...
#ifdef MNL_SO
MTK_GPS_SYS_FUNCTION_POINT_T porting_layer_callback = 
{
...
linux_gps_init (void)
{
	...
	{
		case 0x00:
			MNL_MSG("TCXO, buffer 2\n");
			init_cfg.u1ClockType = 0xFF;
			if(property_set(GPS_CLOCK_TYPE_P, "20") != 0)
				MNL_ERR("set GPS_CLOCK_TYPE_P %s\n", strerror(errno));
			break;
		case 0x10:
			MNL_MSG("TCXO, buffer 1\n");
			init_cfg.u1ClockType = 0xFF;
		...
		case 0x21:
			...
			break;
		case 0x31:
			MNL_MSG("GPS coclock, buffer 3\n");
			init_cfg.u1ClockType = 0xFE;
			if(property_set(GPS_CLOCK_TYPE_P, "31") != 0)
				MNL_ERR("set GPS_CLOCK_TYPE_P %s\n", strerror(errno));
			break;
		case 0x41:
			MNL_MSG("GPS coclock, buffer 4\n");
			init_cfg.u1ClockType = 0xFE;
		...
		default:
			MNL_ERR("unknown clock type, clocktype = %x\n",clock_type);
	...
	MTK_GPS_SYS_FUNCTION_POINT_T*  mBEE_SYS_FP = &porting_layer_callback;
    if(mtk_gps_sys_function_registery(mBEE_SYS_FP) != MTK_GPS_SUCCESS)
...
static int mnlcmd_handler(int fd) /*sent from mnld*/
{
    ...
    else if (cmd == MNL_CMD_INJECT_NW_LOCATION)
    {
        NetworkLocation* nw_location = (NetworkLocation*)malloc(sizeof(NetworkLocation));
        
        do {
            ret = read(fd, nw_location, sizeof(NetworkLocation) );
            MNL_MSG("read: %d, loc.lat = %lf, loc.lon = %lf, loc.acc = %f", ret, nw_location->latitude, nw_location->longitude, nw_location->accuracy);
            usleep(100000);
            retry++;
        } while ((ret < 0) && (retry < 5));
		
        //ret = read(fd, &nw_location, sizeof(nw_location));	
        if(ret == sizeof(NetworkLocation)){
            MNL_MSG("Read location OK");
            MTK_GPS_REF_LOCATION* RefLcation =(MTK_GPS_REF_LOCATION*)malloc(sizeof(MTK_GPS_REF_LOCATION)) ;
...
void get_gps_version()
{
    ...
    property_set("gps.gps.version", "MTK_GPS_MT6592");
