...
linux_gps_init (void)
{
    ...
    else
    {
  	init_cfg.hw_Clock_Freq = 26000000;             // 26MHz TCXO
   	init_cfg.hw_Clock_Drift = 2000;                 // 2.0ppm TCXO
	...
	init_cfg.Int_LNA_Config = 1; /*for 3332, this configuration is hardcode, high Gain. else may have low SNR*/
    /*setting 1Hz/5Hz */
    if(g_is_1Hz)
    {
    ...
    init_cfg.Int_LNA_Config = 1; //by Jerry
    init_cfg.C0 = 0;
    init_cfg.C1 = 0;
    init_cfg.initU = 0;
    init_cfg.lastU = 0;
    MNL_MSG("GNSSOPMode: %d, Int_LNA_Config = %d\n", init_cfg.GNSSOPMode, init_cfg.Int_LNA_Config);
