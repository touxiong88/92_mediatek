    ...
    *eof = 0;
    ...
    if(get_prop()){
     ...
     if(get_prop()){
        ...
        char buf[] = {cmd};
        ret = mnl_send_cmd(buf, sizeof(buf));
        if (ret < 0){
            MND_ERR("send MNL_CMD_INJECT_NW_LOCATION to libmnlp error");
            return ret;
        }
        ...
        do {
            ret = read(sock, nw_location, sizeof(NetworkLocation) );
            MND_MSG("read: %d, loc.lat = %lf, loc.lon = %lf, loc.acc = %f", ret, nw_location->latitude, nw_location->longitude, nw_location->accuracy);
            usleep(100000);
            retry++;
        } while ((ret < 0) && (retry < 2));        	
        if(ret == sizeof(NetworkLocation)){
            MND_MSG("Receive location and send to libmnlp");
            mnl_send_cmd(nw_location, sizeof(NetworkLocation));
            MND_MSG("Inject location to libmnlp");
        } else {
            MND_ERR("Read location fail");
        }
        return 0;
    } 
    else 
