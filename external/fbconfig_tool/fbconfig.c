...
static char mipi_help[] =
	"\n"
	"\nUsage Example: ./fbconfig mipi_set_timing HS_ZERO 0x23\n"	
	...
	printf("%s",_help);
		...
		if(!strcmp(argv[1],"lcm_get_id"))
		   ...
		   if(id_num == 0)
			...
			if(argc !=4)
			{
				printf("\nUsage: <./fbconfig lcm_get_esd Address Parameter_number>\n");
			}
			else{
				int addr,para_num;
				ESD_PARA esd_para;
				...
				sscanf(argv[2],"0X%x",&addr);
				para_num = atoi(argv[3]);
				printf("lcm_get_esd:addr=0x%x  para_num=%d\n",addr,para_num);		
				cmd = LCM_GET_ESD ;	
				esd_para.addr = addr ;
