...
void main(void)
{
    ...
    if(fcntl(fb, F_SETFD, FD_CLOEXEC) == -1)
    {
	perror("fcntl call error");//print errno message;
	goto done;
	}
	else
		ALOGI("lcdc_screen_cap==>fcntl ok\n");

	
	ret_ioctl = ioctl(fb, FBIOGET_VSCREENINFO, &vinfo) ;
    if(ret_ioctl < 0) {
    	ALOGI("[DDMSCap]FBIOGET_VSCREENINFO FAILED!==>%d\n",ret_ioctl);		
    	goto done;
    }else
    ALOGI("[DDMSCap]FBIOGET_VSCREENINFO Successful!!ret_ioctl==>%d\n",ret_ioctl);	
    ...
    }
    {
        ...
        capture_buffer_size_align = capture_buffer_size + 64;//32; //for M4U 32-byte alignment
        ...
        base = (void *)((unsigned long)base_align + 64 - ((unsigned long)base_align & 0x3F));//for M4U v addr 64-bit alignment
