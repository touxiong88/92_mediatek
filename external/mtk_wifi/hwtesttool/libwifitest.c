...
bool WIFI_TEST_OpenDUT(void)
{
    ...
    if (!check_driver_loaded()) {
        LOGD("[%s] loading wifi driver ... ...\n", __func__);

        if (insmod(DRIVER_MODULE_PATH, DRIVER_MODULE_ARG) < 0) {
            LOGD("[%s] failed to load wifi driver!!\n", __func__);
            goto error;
        }
    }
    ...
    if (retval == false) {
        ...
        /* initialize skfd */
        if ((skfd = socket(PF_INET, SOCK_DGRAM, 0)) < 0) {
            LOGE("[%s] failed to open net socket\n", __func__);
            goto error;
        }
    }
