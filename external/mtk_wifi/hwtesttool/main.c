...
{
    printf("%s -I <interface> -I <interface> [-E][-e][-M][-g][-G][-B][-R][-N][-i][-S][-s][-p][-t][-h][-w][-v][-l][-c][-r][-n]\n\n", PACKAGE);
...
static unsigned int sleep_time = 10;
...
int main(int argc, char *argv[]) 
{
    ...
    if (argc == 1) {
        ...
        while ((opt = getopt(argc, argv, "E:e:M:g:G:I:B:R:N:T:m:i:S:s:p:b:t:hVw:v:l:f:c:rn:")) != -1) {
...
void wifi_sensitivity(int times, int channel)
{
    ...
    for(i = 0 ; i < times ; i++) {
		...
		if (rxOk+rxErr == 0){
			fprintf(stdout, "[%d] (%d)RX OK: %d / (%d)RX ERR: %d",i,ret[0],rxOk,ret[1],rxErr);
		}else{
