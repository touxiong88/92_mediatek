    ...
    switch(evt) {
        ...
        case CMD_RESET_END:
            if(false == isRunningReset) {
				...
				/* re-open WiFi only when reset success */
            ...
            break;
        case CMD_RESET_FAIL:
			/* we will not receive CMD_RESET_END */
			if (system("wpa_cli -iwlan0 -p/data/misc/wpa_supplicant send_whole_chip_reset_fail") < 0)
				ALOGI("Fail to send WHOLE_CHIP_RESET_FAIL");
			isRunningReset = false;
			pthread_mutex_unlock(&(mCl->mLock));
			ALOGI("====WHOLE-CHIP RESET FAIL!  ====");
			break;
        case CMD_UNKNOWN:
