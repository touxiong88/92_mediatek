...
namespace {
  class IfConverter : public MachineFunctionPass {
    ...
    // TODO: C++98/03 forbids local type to be used as a template argument. So
    // we move LocalStackFrame to class-level scope.
    struct LocalStackFrame {
      MachineBasicBlock *BB;
      BBInfo *TrueBBI;
      LocalStackFrame(MachineBasicBlock* bb = NULL)
        : BB(bb), TrueBBI(NULL) {}
    };
...
IfConverter::BBInfo &IfConverter::AnalyzeBlock(MachineBasicBlock *BB,
                                             std::vector<IfcvtToken*> &Tokens) {
  // TODO: C++98/03 forbids local type to be used as a template argument. So we
  // move LocalStackFrame to class-level scope.
  std::stack<LocalStackFrame> LocalStack;
  LocalStack.push(LocalStackFrame(BB));

  BBInfo *StackReturn = NULL;
  while (!LocalStack.empty()) {
    LocalStackFrame &frame = LocalStack.top();
    BB = frame.BB;
    BBInfo &BBI = BBAnalysis[BB->getNumber()];

    BBInfo *FalseBBIPtr = NULL;
    if (StackReturn != NULL) {
      if (frame.TrueBBI == NULL)
        frame.TrueBBI = StackReturn;
      else
        FalseBBIPtr = StackReturn;
      StackReturn = NULL;
    }
    ...
    if (frame.TrueBBI == NULL) {
      ...
      LocalStack.push(LocalStackFrame(BBI.TrueBB));
    } else if (FalseBBIPtr == NULL) {
      LocalStack.push(LocalStackFrame(BBI.FalseBB));
    } else {
      BBInfo &TrueBBI = *frame.TrueBBI;
      BBInfo &FalseBBI = *FalseBBIPtr;
      ...
      if (ValidTriangle(TrueBBI, FalseBBI, false, Dups, Prediction) &&
          MeetIfcvtSizeLimit(*TrueBBI.BB,
                             TrueBBI.NonPredSize + TrueBBI.ExtraCost,
      ...
      if (CanRevCond) {
        ...
        if (ValidTriangle(FalseBBI, TrueBBI, false, Dups,
          ...
          Tokens.push_back(new IfcvtToken(BBI, ICTriangleFalse,
                                          FNeedSub, Dups));
          Enqueued = true;
        }
