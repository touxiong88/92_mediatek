...
static BOOLEAN u4WorkqEverRun = FALSE;
...
wlanSetMulticastListWorkQueue (struct work_struct *work) {
	...
	DBGLOG(INIT, WARN, ("wlanSetMulticastListWorkQueue start...\n"));
	...
	DBGLOG(INIT, WARN, ("wlanSetMulticastListWorkQueue end\n"));
    return;
...
*/
	...
	if (u4WorkqEverRun == TRUE) /* if never run, do not need to flush */
	{
