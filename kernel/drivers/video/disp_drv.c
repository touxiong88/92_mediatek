...
extern void mtkfb_release_fence(unsigned int layer,int fence);
extern void mtkfb_release_layer_fence(unsigned int layer);
...
static int _DISP_CleanUpKThread(void *data) {
	...
	while(1) {
		...
		DISP_LOG_D("cleanup r %d, c %d \n",ret,clean_up_task_wakeup);
		clean_up_task_wakeup = 0;
		for (i = 0; i < HW_OVERLAY_COUNT; i++) {
			if (realtime_layer_config[i].isDirty) {
				layerId = realtime_layer_config[i].layer;
				if(isAEEEnabled && (layerId == DISP_DEFAULT_UI_LAYER_ID)) {
					DISP_LOG("aee on\n");
					mtkfb_release_layer_fence(layerId);
				} else {
					...
					if(fence > 0) {
						mtkfb_release_fence(layerId, fence);
					}
				}
			}
		}
...
/* Vanzo:maxiaojun on: Tue, 26 Nov 2013 15:12:59 +0800
 * board device name support.
 */
{
    extern void v_set_dev_name(int id, char *name);
    v_set_dev_name(1, disp_lcm_name);
}
...
extern void smi_dynamic_adj_hint(unsigned int dsi2smi_total_pixel);
	...
	if (try_cnt < 5000) {
		...
		if (dirty_flag->ovl_dirty) {
			...
			for (i = 0; i < DDP_OVL_LAYER_MUN; i++) {
				...
				DISP_LOG("config 0x%08x idx %d\n",
						(captured_layer_config[i].layer << 24)
...
static int _DISP_ConfigDlinkDatapath (disp_path_config_dirty *dirty_flag, struct disp_path_config_mem_out_struct *mem_out_config) {
	...
	if (dirty_flag->ovl_dirty) {
		...
		// add set GPU limition func here
		{
			...
			// size
			smi_dynamic_adj_hint(total_pixel_size);
...
{
	if (lcm_params->dsi.mode != CMD_MODE ||
	 	lcm_params->dsi.mode == CMD_MODE && TARGET_LINE == DISP_Get_MergeTrigger_Mode())
	{
