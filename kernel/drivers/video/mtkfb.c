...
static struct sync_info fence_info[HW_OVERLAY_COUNT] =
{
...
static void mtkfb_ion_init() {
	if (!ion_client && g_ion_device) {
		...
		if (!ion_client) {
			MTKFB_ERR("create ion client failed!\n");
			return;
		}
		MTKFB_LOG("create ion client 0x%p\n", ion_client);
	}
}
...
static void mtkfb_ion_deinit() {
	if (ion_client) {
		...
		ion_client = NULL;
		MTKFB_LOG("destroy ion client 0x%p\n", ion_client);
	}
}
...
static struct ion_handle * mtkfb_ion_import_handle(struct ion_client *client, int fd) {
	...
	if (fd == MTK_FB_NO_ION_FD) {
		MTKFB_LOG("NO NEED ion support\n");
		return handle;
	}

	if (!ion_client) {
		MTKFB_ERR("invalid ion client!\n");
		return handle;
	}
	if (fd == MTK_FB_INVALID_ION_FD) {
		MTKFB_ERR("invalid ion fd!\n");
		return handle;
	}
	handle = ion_import_dma_buf(client, fd);
	if (!handle) {
		MTKFB_ERR("import ion handle failed!\n");
		return handle;
	}
	...
	{
		MTKFB_ERR("configure ion buffer failed!\n");
	}

	MTKFB_LOG("import ion handle fd=%d,hnd=0x%p\n", fd, handle);
...
static void mtkfb_ion_free_handle(struct ion_client *client, struct ion_handle *handle) {
	...
	MTKFB_LOG("free ion handle 0x%p\n",  handle);
}
...
static size_t mtkfb_ion_phys_mmu_addr(struct ion_client *client, struct ion_handle *handle, unsigned int *mva) {
	...
	MTKFB_LOG("alloc mmu addr hnd=0x%p,mva=0x%08x\n",  handle, (unsigned int)*mva);
	return size;
}
...
static void mtkfb_ion_cache_flush(struct ion_client *client, struct ion_handle *handle) {
	...
	if (ion_kernel_ioctl(client, ION_CMD_SYSTEM, &sys_data)) {
		MTKFB_ERR("ion cache flush failed!\n");
	}
}

static void mtkfb_m4u_cache_flush(unsigned int va, unsigned int size) {
	m4u_dma_cache_maint(0, // OVL
	    (const void*)va,
	    size,
	    DMA_BIDIRECTIONAL);
}
...
static unsigned int mtkfb_query_buf_mva(unsigned int layer, unsigned int idx) {
	...
	if (layer != info->layer_id) {
		MTKFB_ERR("wrong layer id %d(rt), %d(in)!\n", info->layer_id, layer);
		return 0;
	}
	...
	if (mva != 0x0) {
		...
		MTKFB_LOG("query buf mva: layer=%d, idx=%d, mva=0x%08x\n", layer, idx, buf->mva);
	} else {
		//FIXME: non-ion buffer need cache sync here?
		MTKFB_ERR("cannot find this buf, layer=%d, idx=%d, fence_idx=%d, timeline_idx=%d, cur_idx=%d!\n", layer, idx, info->fence_idx, info->timeline_idx, info->cur_idx);
	}
	return mva;
}
 ...
 */
static unsigned int mtkfb_get_fence_index (unsigned int layer) {
	struct sync_info *info;
...
static unsigned int mtkfb_inc_fence_index (unsigned int layer) {
	struct sync_info *info;
	unsigned int index = 0;
	ASSERT(layer < HW_OVERLAY_COUNT);
	info = &fence_info[layer];
...
static void mtkfb_init_fence(void) {
	struct sync_info *info;
    int i =0;
	MTKFB_LOG_D("mtkfb init fences\n");
...
static struct sync_info* mtkfb_init_fence_info(unsigned int layer) {
	MTKFB_LOG_D("mtkfb init layer %d fence\n",layer);
	struct sync_info *info;
	ASSERT(layer < HW_OVERLAY_COUNT);
...
struct buf_info* mtkfb_init_buf_info(struct buf_info *buf) {
 ...
 */
static struct buf_info* mtkfb_get_buf_info (void) {
	mutex_lock(&fence_buffer_mutex);
	struct buf_info *info;
	if (!list_empty(&info_pool_head)) {
		info = list_first_entry(&info_pool_head, struct buf_info, list);
		...
		info = kzalloc(sizeof(struct buf_info), GFP_KERNEL);
		mtkfb_init_buf_info(info);
		MTKFB_LOG("create new buf_info node 0x%p\n", info);
		mutex_unlock(&fence_buffer_mutex);
		return info;
	}
	mutex_unlock(&fence_buffer_mutex);
...
static void mtkfb_free_buf_pool (void) {
	struct buf_info *pos, *n;
	mutex_lock(&fence_buffer_mutex);
 ...
 */
void mtkfb_release_buf(unsigned int layer) {
	struct sync_info *info = NULL;
	struct buf_info *buf = NULL;
	struct buf_info *n = NULL;
	...
	if(timeline != NULL ) // make sure fence has been initialized
	{
    	...
    	list_for_each_entry_safe(buf,n,&info->buf_list, list) {
    		if (buf->idx <= timeline_idx) {
    			MTKFB_LOG("release fence buffer-%d-%d\n", layer, buf->idx);
 ...
 */
void mtkfb_signal_fence(unsigned int layer,int fence) {
	struct sync_info *info;
	struct buf_info *buf;
	struct buf_info *n;
	...
	if(timeline != NULL) //make sure fence has been initialized
    {
		...
		{
		    MTKFB_ERR("layer%d fence =%d timevalue %d\n", layer, fence, timeline->value);
		}
		MTKFB_LOG("layer%d  fence =%d timeidx %d timevalue %d\n", layer, fence, info->timeline_idx,timeline->value);
    }
}

static struct sw_sync_timeline* mtkfb_create_timeline (unsigned int layer) {
	struct sw_sync_timeline *timeline;
	char name[32];
	...
	if (timeline == NULL) {
		MTKFB_ERR("cannot create timeline! \n");
	}
	MTKFB_LOG("create timeline: %s\n", name);
	return timeline;
}

static struct fence_data mtkfb_create_fence(unsigned int layer) {
	struct sync_info *info;
	struct sw_sync_timeline *timeline;
	...
	// Init invalid fd
	data.fence = fenceFd;
	data.value = mtkfb_get_fence_index(layer);
	...
	if (timeline == NULL) {
		timeline = mtkfb_create_timeline(layer);
		mtkfb_init_fence_info(layer);
	}
	...
	if (timeline != NULL) {
		if (fence_create(timeline, &data)) {
			MTKFB_ERR("layer%d create Fence Object failed!\n", layer);
		} else {
			mtkfb_inc_fence_index(layer);
			fenceFd = data.fence;
		}
		MTKFB_LOG("create fence %s(%d)\n", data.name, fenceFd);
	} else {
		MTKFB_ERR("layer%d has no Timeline!\n", layer);
	}
 ...
 */
void mtkfb_release_fence(unsigned int layer,int fence) {
	struct sync_info *info;
	struct buf_info *buf;
	struct buf_info *n;
	struct sw_sync_timeline *timeline;
	...
	if(timeline != NULL)
    {
		...
		mutex_unlock(&info->mutex_lock);
		MTKFB_LOG_D_IF((fence != timeline->value),"layer%d fence =%d timevalue %d\n", layer, fence, timeline->value);
		MTKFB_LOG_D("layer%d f %d tl %d inc %d max %d\n", layer, fence, info->timeline_idx, num_fence,info->fence_idx);
    }
}

void mtkfb_release_layer_fence(unsigned int layer) {
	struct sync_info *info;
	int fence = 0;
	ASSERT(layer < HW_OVERLAY_COUNT);
	info = &fence_info[layer];
	...
	mutex_unlock(&info->mutex_lock);
	MTKFB_LOG_D("layer%d release all fence %d\n", layer, fence);
	mtkfb_release_fence(layer,fence);
}
 ...
 * @return struct @buf_info
 */
struct buf_info* mtkfb_prepare_buf(struct fb_overlay_buffer buf) {
	struct sync_info *info;
	struct sw_sync_timeline *timeline;
	...
	// Fence Object
	data = mtkfb_create_fence(buf.layer_id);
	// ION MVA
#if defined (MTK_FB_ION_SUPPORT)
	if (!ion_client) {
		mtkfb_ion_init();
 ...
 */
#if 0
static void mtkfb_hw_sync_init(void) {
    unsigned int layer_id = 0;

    for (layer_id = 0; layer_id < HW_OVERLAY_COUNT; layer_id++) {
    	if (sync_timelines[layer_id]) {
    		printk("warn: Layer %d already has timeline created!\n", layer_id);
    		continue;
    	}
    	sync_timelines[layer_id] = mtkfb_create_timeline(layer_id);
    	// Reset all counter to 0
    	mutex_lock(&FenceMutex);
    	timeline_counter[layer_id] = 0;
    	timeline_counter_inc[layer_id] = 0;
    	fence_counter[layer_id] = 0;
    	mutex_unlock(&FenceMutex);
    }
}
#endif
static void mtkfb_hw_sync_destroy(void) {
	struct sw_sync_timeline *timeline;
	unsigned int layer = 0;
    for (layer = 0; layer < HW_OVERLAY_COUNT; layer++) {
    	timeline = sync_timelines[layer];
    	if (timeline != NULL) {
    		MTKFB_LOG("destroy timeline %s(%d)\n", timeline->obj.name, timeline->value);
    		timeline_destroy(timeline);
    		sync_timelines[layer] = NULL;
    	}
    	mtkfb_release_buf(layer);
    	mtkfb_init_fence_info(layer);
    }
    mtkfb_free_buf_pool();
    mtkfb_ion_deinit();
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
    ...
    // pan display modify layer3, so disable isAEEEnable
    if(isAEEEnabled==1)
    {
        MTKFB_LOG("pan_display reset aee layer\n ");
        isAEEEnabled = 0;
    }
...
static int mtkfb_set_overlay_layer(struct fb_info *info, struct fb_overlay_layer* layerInfo, unsigned int locked)
{
    ...
    if (layerInfo->src_phy_addr != NULL) {
    	...
    	temp[id].addr = mtkfb_query_buf_mva(layerInfo->layer_id, (unsigned int)layerInfo->next_buff_idx);
    }
#else
    temp[id].addr = (unsigned int)layerInfo->src_phy_addr;
#endif
...
static int mtkfb_capture_framebuffer(struct fb_info *info, unsigned int pvbuf)
{
    ...
    // DO NOT trigger hw if in suspend state
    if (is_early_suspended)
    {
...
#endif
{
    ...
    case MTKFB_SET_MULTIPLE_LAYERS:
    {
    	...
    	if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
    		...
    		for (i = 0; i < HW_OVERLAY_COUNT; ++i) {
				...
				if (layerInfo[i].layer_enable && (layerInfo[i].next_buff_idx <= fence_info[i].timeline_idx))
				{
    		    	MTKFB_ERR("layerId(%d) HWC set the old buffer(%d),timeline_idx(%d)!\n", layerInfo[i].layer_id, layerInfo[i].next_buff_idx,fence_info[i].timeline_idx);
					//aee_kernel_warning("MTKFB","layerId(%d) HWC set the old buffer(%d)!\n", layerInfo[i].layer_id, layerInfo[i].next_buff_idx);
					continue;
				}
				if(is_early_suspended)
				{
					MTKFB_ERR("in early suspend layer(0x%x),idx(%d),time_idx(%d)!\n", layerInfo[i].layer_id<<16|layerInfo[i].layer_enable, layerInfo[i].next_buff_idx,fence_info[i].timeline_idx);
	...
	case MTKFB_PREPARE_OVERLAY_BUFFER:
    {
		...
		if (copy_from_user(&overlay_buffer, (void __user *)arg, sizeof(overlay_buffer))) {
			...
			unsigned int layer = overlay_buffer.layer_id;
			if (overlay_buffer.layer_en) {
				buf = mtkfb_prepare_buf(overlay_buffer);
			    if (buf != NULL) {
