...
static long fm_ops_ioctl(struct file *filp, fm_u32 cmd, unsigned long arg)
{
	...
	{
		fm_full_cqi_log_t parm;
		WCN_DBG(FM_NTC | MAIN, "......FM_IOCTL_FULL_CQI_LOG......\n");
		
		if (copy_from_user(&parm, (void*)arg, sizeof(fm_full_cqi_log_t))){
			WCN_DBG(FM_ALT | MAIN,"copy_from_user error\n");
			ret = -EFAULT;
			goto out;
		}
		ret = fm_full_cqi_logger(&parm);
		if(ret < 0){
			WCN_DBG(FM_ERR | MAIN,"FM_IOCTL_FULL_CQI_LOG failed\n");
		}
...
//    fm_close(fm);
    ...
    WCN_DBG(FM_NTC | MAIN, "fm_ops_release:1 pid:%d tgid: %d\n", current->pid, current->tgid);
    return ret;
}
...
static fm_s32 fm_ops_flush(struct file *filp,fl_owner_t Id)
{
    ...
    WCN_DBG(FM_NTC | MAIN, "fm_ops_flush:0 current pid:%d tgid: %d\n", current->pid, current->tgid);    
    //fm_close(fm);
	
    fm->ref--;
    fm->ref = (fm->ref < 0) ? 0 : fm->ref;
