...
{
    fm_s32 ret = 0;
	fm_u32 g_dbg_level_backup = g_dbg_level;
    fm_u16 freq_backup = fm_cur_freq_get();
	FMR_ASSERT(fm_low_ops.bi.cqi_log);

	g_dbg_level = 0xffffffff;	//trigger conditions
	fm_cur_freq_set(10000);
	
	if (FM_LOCK(fm_ops_lock)) 
		return (-FM_ELOCK);
	ret = fm_low_ops.bi.cqi_log(setting->lower,setting->upper,setting->space,setting->cycle);
    FM_UNLOCK(fm_ops_lock);
	
	g_dbg_level = g_dbg_level_backup;
	fm_cur_freq_set(freq_backup);
	return ret;
}
