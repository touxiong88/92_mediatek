...
static fm_s32 mt6630_full_cqi_get(fm_s32 min_freq, fm_s32 max_freq, fm_s32 space, fm_s32 cnt)
{
	...
	if((10000 == orig_freq) && (0xffffffff == g_dbg_level))
    {
		...
		sprintf(&cqi_log_path[strlen(FM_CQI_LOG_PATH)], "%d.txt", cnt);
...
static fm_s32 mt6630_I2s_Setting(fm_s32 onoff, fm_s32 mode, fm_s32 sample)
{
    ...
    ret = mt6630_set_bits(0x9B, tmp_state, 0xFFFE);
    if (ret)
    {
        goto out;
    }
		                        ...
		                        fm_u16 space)
{
    ...
    WCN_DBG(FM_NTC | CHIP, "[freq=%d], [max_freq=%d],[min_freq=%d],[scan BTL size=%d],[scandir=%d],[space=%d]\n", 
			freq, max_freq, min_freq, *ScanTBLsize, scandir, space);
    ...
    {
        for (i=((*pFreq-min_freq)/step+1); i<total_no; i++)
        {
