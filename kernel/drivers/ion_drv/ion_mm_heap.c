...
void ion_mm_heap_free_bufferInfo(struct ion_buffer *buffer)
{
    ...
    if (pBufferInfo)
    {
        ...
        if (t > 5000)
        	pr_err("ion_mm_heap_free_bufferInfo: %luus\n", t);
                            ...
                            ion_phys_addr_t *addr, size_t *len)
{
    ...
    *len = buffer->size;
    ...
    t = (tv_e.tv_sec - tv_s.tv_sec)*1000000 + (tv_e.tv_usec - tv_s.tv_usec);
    if (t > 5000)
    	pr_err("ion_mm_heap_phys: %luus, size = %d.\n", t, *len);
                                      ...
                                      void *unused)
{
    ...
    {
    	seq_printf(s, "mm_heap defer free disabled\n");
    }
    ...
    case ION_MM_CONFIG_BUFFER:
        {
            ...
            handle = Param.config_buffer_param.handle;
            if (handle)
            {
                mutex_lock(&client->lock);
                if (!ion_handle_validate(client, handle)) {
                    IONMSG("invalid handle to config buffer 0x%x.\n", handle);
                    mutex_unlock(&client->lock);
            ...
            {
                IONMSG("[ion_mm_heap]: Error. Configure buffer with invalid handle.\n");
                ret = -EFAULT;
            }
    ...
    case ION_MM_SET_DEBUG_INFO:
        {
            ...
            handle = Param.buf_debug_info_param.handle;
            if (handle)
            {
                mutex_lock(&client->lock);
                if (!ion_handle_validate(client, handle)) {
                    IONMSG("invalid handle to set debug info 0x%x.\n", handle);
                    mutex_unlock(&client->lock);
                    return ERR_PTR(-EINVAL);
                }
                buffer = handle->buffer;
    ...
    case ION_MM_GET_DEBUG_INFO:
        {
            ...
            if (handle)
            {
                mutex_lock(&client->lock);
                if (!ion_handle_validate(client, handle)) {
                    IONMSG("invalid handle to get debug info 0x%x.\n", handle);
                    mutex_unlock(&client->lock);
                    return ERR_PTR(-EINVAL);
                }
                buffer = handle->buffer;
