...
static long ion_sys_cache_sync(struct ion_client *client, ion_sys_cache_sync_param_t* pParam)
{
    ...
    if (pParam->sync_type < ION_CACHE_CLEAN_ALL)
    {
        ...
        else
        {
            start = pParam->va;
            size = pParam->size;
            printk(KERN_INFO"[ion_dbg]: sync user va: va=0x%x, size=0x%x\n", start, size);
            if(start < 0x1000)
            {
                IONMSG("cannot do cachesync, va=0x%x, size=0x%x\n", start, size);
                return -EFAULT;
            }
        }

        do_gettimeofday(&tv_e);
        t = (tv_e.tv_sec - tv_s.tv_sec)*1000000 + (tv_e.tv_usec - tv_s.tv_usec);
        if(t>3000)
            IONMSG("map kernel: %luus, size=0x%x\n", t, size);
...
static long ion_sys_ioctl(struct ion_client *client, unsigned int cmd, unsigned long arg, int from_kernel)
{
    ...
    case ION_SYS_RECORD:
	{
		...
		if(ion_debugger)
		{
				...
				for(i = 0 ; i < Param.record_param.backtrace_num;i++)
				{
					if(Param.record_param.mapping_record[i].size > 0 )
					{
						...
						if(string_len > 0 && string_len < 60)
						{
							...
							if(tmp_string == NULL)
							{
								printk(ION_DEBUG_ERROR "[ION_FUNC%d][ion_sys_ioctl]mapping info error can't get enough memory string len %d \n",Param.record_param.action,string_len);
								break;
							}
						...
						{
							printk(ION_DEBUG_ERROR "[ION_FUNC%d][ion_sys_ioctl]mapping info error can't get right string len %d \n",Param.record_param.action,string_len);
						}
					}
				}
			}
			...
			if(Param.record_param.handle != NULL)
			{
				...
				{
					printk(ION_DEBUG_ERROR "[ION_FUNC%d]buffer is NULL\n",Param.record_param.action);
				}
			} 
    ...
    case ION_SYS_SET_HANDLE_BACKTRACE:
    {
	     ...
	     unsigned int i;
         struct ion_handle *handle = Param.record_param.handle;

        handle->dbg.pid = (unsigned int) current->pid;
        handle->dbg.tgid = (unsigned int)current->tgid;
        handle->dbg.backtrace_num = Param.record_param.backtrace_num;

        for(i=0; i<Param.record_param.backtrace_num; i++)
        {
            handle->dbg.backtrace[i] = Param.record_param.backtrace[i];
        }
#endif
