...
/* Vanzo:yangzhihong on: Sat, 17 May 2014 13:44:27 +0800
 */
static int _isButtonLightOn = 1;
...
/* Vanzo:yangzhihong on: Sat, 17 May 2014 13:45:08 +0800
 */
	if(strcmp(led_data->cust.name,"button-backlight") == 0) {
        if(!_isButtonLightOn)
        {
            level = 0;
        }
...
/* Vanzo:yangzhihong on: Sat, 17 May 2014 13:43:14 +0800
 */

static ssize_t store_button_light(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    if(size == 0)
        return;

    if(buf[0] == '1' && _isButtonLightOn == 0){
        _isButtonLightOn = 1;
    }

    if(buf[0] == '0' && _isButtonLightOn == 1){
        _isButtonLightOn = 0;
    }
...
static ssize_t show_button_light(struct device *dev,struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(button_light_on, 0666, show_button_light, store_button_light);
// End of Vanzo:yangzhihong
...
/* Vanzo:yangzhihong on: Sat, 17 May 2014 13:42:52 +0800
 */
		if(strcmp(g_leds_data[i]->cdev.name,"button-backlight") == 0)
        {
			rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_button_light_on);
            if(rc)
            {
                LEDS_DRV_DEBUG("[LED]device_create_file button_light_on fail!\n");
            }
        }
// End of Vanzo:yangzhihong
