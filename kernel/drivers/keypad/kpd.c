...
/* Vanzo:zhangxinyu on: Wed, 16 Oct 2013 15:37:34 +0800
 * TODO: replace this line with your comment
	#define MTK_KP_WAKESOURCE//this is for auto set wake up source
  */
#if (defined(VANZO_KP_WAKE_UP_SYS))
#else
	#define MTK_KP_WAKESOURCE//this is for auto set wake up source
#endif
...
/* Vanzo:yangzhihong on: Tue, 17 Sep 2013 17:49:51 +0800
 * TODO: eint home key
 */
#ifdef VANZO_HOME_KEY_USE_EINT
#define CUST_EINT_KPD_HOME_POLARITY 0
static void kpd_homekey_handler(unsigned long data);
static DECLARE_TASKLET(kpd_homekey_tasklet, kpd_homekey_handler, 0);
static u8 kpd_homekey_state = !CUST_EINT_KPD_HOME_POLARITY;
#endif
...
#endif
/* Vanzo:yangzhihong on: Tue, 17 Sep 2013 17:52:13 +0800
 * TODO: replace this line with your comment
 */
#ifdef VANZO_HOME_KEY_USE_EINT
...
void kpd_homekey_handler(unsigned long data)
{
  bool pressed;
  ...
  pressed = (kpd_homekey_state == !!CUST_EINT_KPD_HOME_POLARITY);
  ...
  input_report_key(kpd_input_dev, KEY_HOME, pressed);
  input_sync(kpd_input_dev);
  kpd_print("report Linux keycode = %u\n", KEY_HOME);
  /* for detecting the return to old_state */
  mt_eint_set_polarity(CUST_EINT_KPD_HOME_NUM, old_state);
  mt_eint_unmask(CUST_EINT_KPD_HOME_NUM);
}

void kpd_homekey_eint_handler(void)
{
  	mt_eint_mask(CUST_EINT_KPD_HOME_NUM);
	tasklet_schedule(&kpd_homekey_tasklet);
}

void mt_eint_homekey_register()
{
  printk("%s line %d\n",__func__,__LINE__);

  mt_set_gpio_mode(GPIO_KPD_HOMEKEY_EINT_PIN, GPIO_KPD_HOMEKEY_EINT_PIN_M_EINT);
  mt_set_gpio_dir(GPIO_KPD_HOMEKEY_EINT_PIN, GPIO_DIR_IN);
  //mt_set_gpio_pull_enable(GPIO_KPD_MUTEKEY_EINT_PIN, GPIO_PULL_DISABLE);
  mt_set_gpio_pull_enable(GPIO_KPD_HOMEKEY_EINT_PIN, GPIO_PULL_ENABLE);
  mt_set_gpio_pull_select(GPIO_KPD_HOMEKEY_EINT_PIN, GPIO_PULL_UP);

  mt_eint_set_sens(CUST_EINT_KPD_HOME_NUM, CUST_EINT_KPD_HOME_TYPE);
  mt_eint_set_hw_debounce(CUST_EINT_KPD_HOME_NUM, CUST_EINT_KPD_HOME_DEBOUNCE_EN);

  mt_eint_registration(CUST_EINT_KPD_HOME_NUM, CUST_EINT_KPD_HOME_TYPE,kpd_homekey_eint_handler, false);
}
#endif
// End of Vanzo:yangzhihong
/*********************************************************************/
...
/* Vanzo:yangzhihong on: Tue, 17 Sep 2013 18:07:40 +0800
 * TODO: replace this line with your comment
 */
#ifdef VANZO_HOME_KEY_USE_EINT
	__set_bit(KEY_HOME, kpd_input_dev->evbit);
#endif
// End of Vanzo:yangzhihong
...
/* Vanzo:zhangxinyu on: Wed, 16 Oct 2013 15:42:41 +0800
	* TODO: replace this line with your comment
*/
#if (defined(VANZO_KP_WAKE_UP_SYS))
