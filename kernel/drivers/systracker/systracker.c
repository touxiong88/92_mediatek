...
//static int enable_systracker(void);
//static int disable_systracker(void);
  ...
  //if(is_systracker_device_registered)
  {
    /* Get tracker info and save to buf */
	  ...
	  for (i = 0; i < 8; i++) {
            ...
            entry_valid         = extract_n2mbits(reg_value,14,14);
            entry_id            = extract_n2mbits(reg_value,7,13);
...
static ssize_t set_wp_address_store(struct device_driver * driver, const char *buf,
			   size_t count)
{
        unsigned int value;
...
static ssize_t tracker_entry_dump_store(struct device_driver * driver, const char *buf,
			   size_t count)
{
	return count;
}
...
{
/*workaround for 6592*/
   *(volatile u32 *)BUS_DBG_CON &= ~(1<<0);
   *(volatile u32 *)BUS_DBG_CON |= (1<<13);
   *(volatile u32 *)BUS_DBG_CON &= ~(1<<13);
...
static irqreturn_t systracker_isr(void)
{
        ...
        reg_val = readl(IOMEM(BUS_DBG_CON));
        writel(reg_val | (1<<7), IOMEM(BUS_DBG_CON));
        if (reg_val & (1 << 10)){ //IRQ_STA[10] watch point interrupt
...
int enable_watch_point(void)
{
    ...
    if (request_irq(BUS_DBG_TRACKER_IRQ_ID0, systracker_isr, IRQF_TRIGGER_LOW, "SYSTRACKER", NULL )) {
    ...
    writel(readl(IOMEM(BUS_DBG_CON))|(1 << 3),IOMEM(BUS_DBG_CON));
   return 0;
}
...
int enable_systracker(void)
    //static int enable_systracker(int use_irq_mode, int timeout_ms, u32 mask, u32 val)
{
    u32 con;
    u32 val;
    u32 timer_control_value; 
    /*prescale= 16625000/1s, 10^6 * 266 / 16 */
    timer_control_value = 16625 * track_config.timeout_ms; 

    writel(timer_control_value,IOMEM(BUS_DBG_TIMER_CON));

    con = DEFAULT_CONTROL_VAL;
...
int systracker_probe(struct platform_device *pdev)
{
    ...
    track_config.nr_entry_per_rw = 8;
    ...
    //reset_systracker();
    enable_systracker();
    //enable_systracker(1, 3, 0xfffffffb, 0x00);
    printk("systracker probe\n");
    /*Create sysfs entry*/
    ret = driver_create_file(&systracker_driver.driver,
