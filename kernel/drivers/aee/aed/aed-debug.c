...
static int proc_write_generate_oops(struct file* file,
				    const char __user *buf, unsigned long count,
				    void *data)
{
	char msg[6];
...
static int proc_read_generate_nested_ke(char *page, char **start,
			     off_t off, int count,
			     int *eof, void *data)
{
	int len;
...
int aed_proc_debug_init(struct proc_dir_entry *aed_proc_dir)
{
	...
	struct proc_dir_entry *aed_proc_generate_nested_ke_file;
	...
	aed_proc_generate_oops_file = create_proc_entry("generate-oops",
	...
	aed_proc_generate_oops_file->write_proc = proc_write_generate_oops;
	aed_proc_generate_oops_file->read_proc = proc_read_generate_oops;

	aed_proc_generate_nested_ke_file = create_proc_read_entry("generate-nested-ke", 
							     0400, aed_proc_dir, 
							     proc_read_generate_nested_ke,
							     NULL);
	if (aed_proc_generate_nested_ke_file == NULL) {
	  xlog_printk(ANDROID_LOG_ERROR, AEK_LOG_TAG, "aed create_proc_read_entry failed at generate-nested-ke\n");
	  return -ENOMEM;
	}
