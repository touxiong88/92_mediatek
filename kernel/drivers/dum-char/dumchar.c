...
#endif
	total_retlen= sd_single_read(filp, pbuf2,buflen,startaddr2,part);
		set_fs(curr_fs);
...
#endif
		total_retlen= sd_single_read(filp, buf,count,addr,part);
	}
	*pos += total_retlen;
	return total_retlen;
} /* mtd_read */
...
#endif
	total_retlen= sd_single_write(filp, buf,count,addr,part);
	*pos += total_retlen;
	return total_retlen;
} 
#endif
