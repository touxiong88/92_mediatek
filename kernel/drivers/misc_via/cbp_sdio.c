...
static void data_ack_wait_event(struct cbp_wait_event *pdata_ack)
{
	...
	wait_event(cbp_data_ack->wait_q, MODEM_ST_READY == atomic_read(&cbp_data_ack->state));
	//wait_event_timeout(cbp_data_ack->wait_q, MODEM_ST_READY == atomic_read(&cbp_data_ack->state),msecs_to_jiffies(1));
}
...
static void flow_ctrl_wait_event(struct cbp_wait_event *pflow_ctrl)
{
	...
	wait_event_timeout(cbp_flow_ctrl->wait_q, FLOW_CTRL_DISABLE == atomic_read(&cbp_flow_ctrl->state),msecs_to_jiffies(20));
}
