...
#endif
    {
        ...
        if (fn->transfer_id == RAWBULK_TID_MODEM) {
            if (check_enable_state(fn))
                modem_dtr_send(simple_strtoul(buf, NULL, 10));
        }
...
#define print_request_data(req) { \
	if (req->status < 0) { \
		printk(KERN_DEBUG "%s: request failed %d\n", __func__, req->status); \
	} else { \
		int n; \
		printk(KERN_DEBUG "%s: data(%d):", __func__, req->actual); \
...
static void rawbulk_tty_rx_complete(struct usb_ep *ep, struct usb_request *req) {
    ...
    /*while ((r = int_get_req(&fn->rx_free, &fn->rx_lock))) {
        int_put_req(r, &fn->rx_inproc, &fn->rx_lock);
        if(rawbulk_usb_state_check())
            ret = usb_ep_queue(fn->bulk_out, r->request, GFP_ATOMIC);
        if (ret < 0) {
            int_move_req(r, &fn->rx_free, &fn->rx_lock);
            break;
        }
    }*/
}
