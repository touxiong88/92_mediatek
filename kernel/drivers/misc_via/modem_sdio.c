...
int modem_on_off_ctrl_chan(unsigned char on)
{
	...
	wait_event(ctrl_port->sflow_ctrl_wait_q, SFLOW_CTRL_DISABLE == atomic_read(&ctrl_port->sflow_ctrl_state));
...
int modem_dtr_query(int *status,const char query_mode)
{
	...
	LOGPRT(LOG_NOTICE,  "%s: enter, query_mode = %d\n", __func__, query_mode);
	port = sdio_modem_port_get(0);
	ret = check_port(port);
	if (ret < 0){
		LOGPRT(LOG_ERR,  "%s %d check_port failed\n", __func__,__LINE__);
	...
	if(ctrl_port->chan_state == 1){
		if(query_mode)/*ask cbp for status*/
		{	
			wait_event(ctrl_port->sflow_ctrl_wait_q, SFLOW_CTRL_DISABLE == atomic_read(&ctrl_port->sflow_ctrl_state));
			if(down_interruptible(&modem->sem)){
				LOGPRT(LOG_ERR,  "%s %d down_interruptible failed.\n", __func__,__LINE__);
...
static void destroy_modem_port(struct kref *kref)
{
	int index;
	struct sdio_modem *modem;
...
static void modem_tty_close(struct tty_struct *tty, struct file * filp)
{
	...
	if (port->port.count == 1){
		...
		kref_put(&port->kref, destroy_modem_port);
		return ;
	}
...
static void sdio_write_port_work(struct work_struct *work)
{
	...
	left = count;
	do{
		wait_event(port->sflow_ctrl_wait_q, SFLOW_CTRL_DISABLE == atomic_read(&port->sflow_ctrl_state));
		if(down_interruptible(&modem->sem)){
			LOGPRT(LOG_ERR,  "%s %d down_interruptible failed.\n", __func__,__LINE__);
			ret =  -ERESTARTSYS;
			goto down_sem_fail;
...
static void modem_sdio_irq(struct sdio_func *func)
{
	...
	if (ret){
		...
		if(GPIO_OEM_VALID(GPIO_VIATEL_CRASH_CBP)){
			LOGPRT(LOG_ERR,  "let cbp die now\n");
	...
	{   
		...
		if(port->inception){
			...
			if (tty && modem->data_length) {
				if(port->sdio_buf_in ==1){
					mutex_lock(&port->mutex);/*make sure data in list bufeer had been pushed to tty buffer*/
					mutex_unlock(&port->mutex);
				}
	...
	else if(modem->msg->head.chanInfo ==0)/*control message analyze*/
	{
		...
		LOGPRT(LOG_ERR,  "%s %d: after ctrl_msg_analyze\n",__func__, __LINE__);
	}
	else{
...
LOGPRT(LOG_ERR,  "%s %d: before wait_ack\n",__func__, __LINE__);	
wait_ack:
	//LOGPRT(LOG_ERR,  "%s %d: port%d data ack before!,modem->cbp_data->cbp_data_ack=%d\n", __func__, __LINE__, modem->cbp_data->cbp_data_ack);
	LOGPRT(LOG_ERR,  "%s %d: before data_ack_enable\n",__func__, __LINE__);
	if(modem->cbp_data->data_ack_enable){
		modem->cbp_data->data_ack_wait_event(modem->cbp_data->cbp_data_ack);
	}
	LOGPRT(LOG_ERR,  "%s %d: port data ack after!\n", __func__, __LINE__);
out:
	return;
}
...
LOGPRT(LOG_ERR,  "%s %d: if cbp_data->flow_ctrl_enable£¡£¡£¡\n", __func__, __LINE__);		
	...
	if((index == SDIO_AT_CHANNEL_NUM) || sdio_log_level > LOG_NOTICE) {
...
static void sdio_modem_remove(struct sdio_modem *modem)
{
	...
	for (index = 0; index < SDIO_TTY_NR; index++) {
		...
		if (tty) {
			LOGPRT(LOG_NOTICE,"%s %d index=%d port->index=%d\n",__func__,__LINE__,index,port->index);
			tty_hangup(tty);
			tty_kref_put(tty);
		}
	}
			     ...
			     char *buf)
{
   ...
   ret=modem_dtr_query(&status,query_mode);
   if(ret <0){
...
int sdio_rawbulk_intercept(int port_num, unsigned int inception) {
    ...
    if(port_num >= 2)
		port_num++; 
    port = sdio_modem_port_get(port_num);
...
int modem_buffer_push(int port_num, void *buf, int count)
{
	...
	spin_unlock_irqrestore(&port->write_lock, flags);
    if(data_len < count) {
        LOGPRT(LOG_DEBUG, "%s line %d: SDIO driver buffer is full!\n", __FUNCTION__,__LINE__);
        return -ENOMEM;
    } 
	ret = kfifo_in_locked(&port->transmit_fifo, buf, count, &port->write_lock);
	queue_work(port->write_q, &port->write_work);
