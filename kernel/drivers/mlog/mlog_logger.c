...
#define M_FILTER_ALL        (M_MEMFREE | M_SWAPFREE | M_CACHED | M_GPUUSE | M_MLOCK | M_ZRAM | M_ACTIVE | M_INACTIVE)
    ...
    if (buddyinfo_filter)
    {
        ...
        for (i = 0; i < 2; ++i) {
	        ...
	        for (j = 0; j < MAX_ORDER - 2; ++j) {
	            ...
	            //printk("zmj: insert oder string format: %s\n", order_str);
	        }
...
static void mlog_meminfo(void)
{
    ...
    unsigned long meminfo_active, meminfo_inactive;
...
/*	
    ...
    meminfo_active = P2K(pages[LRU_ACTIVE_ANON]   + pages[LRU_ACTIVE_FILE]),
    meminfo_inactive = P2K(pages[LRU_INACTIVE_ANON] + pages[LRU_INACTIVE_FILE]),
    MLOG_PRINTK("meminfo active: %lu, inactive: %lu\n", meminfo_active, meminfo_inactive);
*/
...
void mlog_buddyinfo(void)
{
    ...
    spin_lock_bh(&mlogbuf_lock);
    printk("[mlog]: normal zone:\n");
    for (order = 0; order < MAX_ORDER; ++order) {
        printk("%6lu ", normal_nr_free[order]);
        mlog_emit_32(normal_nr_free[order]);
    }
    printk("\n");

    printk("[mlog]: high zone:\n");	
    for (order = 0; order < MAX_ORDER; ++order) {
        mlog_emit_32(high_nr_free[order]);
        printk("%6lu ", high_nr_free[order]);
    }
    printk("\n");
    spin_unlock_bh(&mlogbuf_lock);
}
EXPORT_SYMBOL(mlog_buddyinfo); // for testing
