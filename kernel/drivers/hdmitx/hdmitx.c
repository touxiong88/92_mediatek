...
#define MTK_HDMI_INVALID_ION_FD	   -1
...
{
#ifdef MTK_HDMI_MULTI_DISPLAY_SUPPORT
    return;
#endif
...
static int hdmi_overlay_config_kthread(void *data)
{
    ...
    if (wdma1_bpp == 2)
    {
        wdma1Config.outFormat = 4;    ////WDMA_OUTPUT_FORMAT_RGB888;
    }
    else
    {
        wdma1Config.outFormat = 1;
    }
...
void hdmi_update_buffer_switch(void)
{
    ...
    ///if (hdmi_rdma_buffer_switch_mode == 0)
    {
...
#ifndef MTK_HDMI_MULTI_DISPLAY_SUPPORT
...
int hdmi_rdma_address_config(bool enable, hdmi_video_buffer_info buffer_info)
{
    ///HDMI_FUNC();
#if 0
    buffer_info.src_width = ALIGN_TO(p->hdmi_width , 4);
    buffer_info.src_height = ALIGN_TO(p->hdmi_height , 4);
    buffer_info.src_pitch = buffer_info.src_width;
#endif
...
int hdmi_display_path_overlay_config(bool enable)
{
    ...
    if (enable)
    {
        ...
        if (wdma1_bpp == 2)
        {
            wdma1Config.outFormat = 4;    ///WDMA_OUTPUT_FORMAT_UYVY;
        }
        else
        {
            wdma1Config.outFormat = 1;
...
/*static*/ void hdmi_resume(void)
{
    ...
    if (wdma1_bpp == 2)
    {
        outFormat = 4;    ///WDMA_OUTPUT_FORMAT_UYVY;
    }
...
static void dpi_setting_res(u8 arg)
{
    switch (arg)
    {
        ...
        case HDMI_VIDEO_1280x720p_60Hz:
        {
            ...
            vsync_back_porch    = 20;
            vsync_front_porch   = 5;
            p->hdmi_width = 1280;//1280  1366
            p->hdmi_height = 720;//720;  768
