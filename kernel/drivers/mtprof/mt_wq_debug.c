...
#ifdef CONFIG_MT_SCHED_MONITOR
	wqdbg->wklog[idx].sched_slim[SCHED_MON_ISR].cur_event = __raw_get_cpu_var(ISR_mon).cur_event;
	wqdbg->wklog[idx].sched_slim[SCHED_MON_SIQ].cur_event = __raw_get_cpu_var(SoftIRQ_mon).cur_event;
	wqdbg->wklog[idx].sched_slim[SCHED_MON_TLT].cur_event = __raw_get_cpu_var(tasklet_mon).cur_event;
	wqdbg->wklog[idx].sched_slim[SCHED_MON_HRT].cur_event = __raw_get_cpu_var(hrt_mon).cur_event;
	wqdbg->wklog[idx].sched_slim[SCHED_MON_SFT].cur_event = __raw_get_cpu_var(sft_mon).cur_event;
#endif
...
void mt_dump_real(int cpu, int i) {
	...
	xlog_printk(ANDROID_LOG_ERROR, WQ_LOG_TAG, "wq:cpu:%d,idx:%d,req_cpu:%d,wkaddr:%x,cb:%pf,cbaddr:%x,module:%s,%s,pid:%d,%s\n",
	cpu,
	i,
	...
	per_cpu(wq_debugger,cpu).wklog[i].callee_pid,
	per_cpu(wq_debugger,cpu).wklog[i].callee
	);

#ifdef CONFIG_MT_SCHED_MONITOR
	xlog_printk(ANDROID_LOG_ERROR, WQ_LOG_TAG, "wq: isr:%d, softirq:%d, tasklet:%ppf, hrt:%pf, sft:%pf\n",
	per_cpu(wq_debugger,cpu).wklog[i].sched_slim[SCHED_MON_ISR].cur_event,
	per_cpu(wq_debugger,cpu).wklog[i].sched_slim[SCHED_MON_SIQ].cur_event,
	(void *)per_cpu(wq_debugger,cpu).wklog[i].sched_slim[SCHED_MON_TLT].cur_event,
	(void *)per_cpu(wq_debugger,cpu).wklog[i].sched_slim[SCHED_MON_HRT].cur_event,
	(void *)per_cpu(wq_debugger,cpu).wklog[i].sched_slim[SCHED_MON_SFT].cur_event
	);
#endif
