...
static ssize_t pa_stat(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
  ...
  //PACK Status
  snprintf(tmp_buf, TMP_BUF_LENGTH, "-----------------------PACK status-----------------------------------\n");
	...
	snprintf(tmp_buf, TMP_BUF_LENGTH, "---------------------------------CPU buddy---------------------------------\n");
	strncat(buf, tmp_buf, TMP_BUF_LENGTH);
	...
	snprintf(tmp_buf, TMP_BUF_LENGTH, "-----------------------------------Buddy CPUs-------------------------------\n");
	strncat(buf, tmp_buf, TMP_BUF_LENGTH);
	...
	if(!list_empty(&cpu_domains))
	{
		...
		unsigned int j;
		snprintf(tmp_buf, TMP_BUF_LENGTH, "--------------------------------Cluster info----------------------------\n");
		strncat(buf, tmp_buf, TMP_BUF_LENGTH);
	...
	if(PA_MON_ENABLE) {
		snprintf(tmp_buf, TMP_BUF_LENGTH, "----------------------------------Task MONITOR-----------------------------\n");
		strncat(buf, tmp_buf, TMP_BUF_LENGTH);
		for(i=0;i<4;i++) {
	...
	if (len == 1) {
		...
		while(i<4) {
			...
			if (chr_ptr != (char*) NULL) {
					strncpy(&PA_MON[i][0], &buf[ptr_offset], (u64)chr_ptr - (u64)(&buf[ptr_offset]));
					cut_end(&PA_MON[i][0]);
...
static ssize_t pa_load(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	strncat(buf, tmp_buf, TMP_BUF_LENGTH);
	if(jiffies_diff != 0) {
		snprintf(tmp_buf, TMP_BUF_LENGTH, "--------------------------------CPU work load------------------------\n");
	  strncat(buf, tmp_buf, TMP_BUF_LENGTH);
		for_each_possible_cpu(cpu) {
		...
		*/
			if(jiffies_diff >= idle_time_diff) {
				snprintf(tmp_buf, TMP_BUF_LENGTH, "cpu%d POWER:%04lu BUDDY:%d CFS_RQ:%04u IDLE_TIME_DIFF:%09llu work_load:%04llu\n", 
													cpu, 
													cpu_rq(cpu)->cpu_power,  
													per_cpu(sd_pack_buddy, cpu), 
													cpu_rq(cpu)->cfs.nr_running,
													cputime64_to_clock_t(idle_time_diff), 
													100 - div_u64(100 * cputime64_to_clock_t(idle_time_diff), jiffies_diff));
