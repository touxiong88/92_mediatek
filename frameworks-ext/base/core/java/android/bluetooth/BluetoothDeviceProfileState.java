...
public final class BluetoothDeviceProfileState extends StateMachine {
  ...
  private class IncomingHid extends State {
        ...
        switch(command) {
            ...
            case CONNECT_HFP_INCOMING:
                if (mHeadsetService == null) {
                    ...
                    /// M: ALPS00795471
                    return processIncomingConnectCommand(command);
                }
                break;
            case CONNECT_A2DP_OUTGOING:
                if (mA2dpService != null) {
    ...
    /// M: ALPS00795471
    private boolean processIncomingConnectCommand(int command) {
        ...
        if (mService.getBondState(mDevice.getAddress()) != BluetoothDevice.BOND_BONDED && command == CONNECT_HFP_INCOMING ) {
            /// M: ALPS00795471
            return handleIncomingConnection(command, true);
        }
        ///@ }
        ...
        if (access == BluetoothDevice.CONNECTION_ACCESS_YES) {
            ...
            /// M: ALPS00795471
            return handleIncomingConnection(command, false);
        } else {
