...
bool dumpLayer(Layer* layer)
{
    char procName[256];
    char file[512];

    if (!g_HWUI_debug_dumpLayer)
        return false;

    if (!getProcessName(procName, sizeof(procName)))
        return false;

    sprintf(file, "/data/data/%s/layer_%d_%.1f_%.1f_%p.png", procName, layer->getFbo(), layer->layer.getWidth(), layer->layer.getHeight(), layer);
    return dumpImage(layer->layer.getWidth(), layer->layer.getHeight(), file);
}
