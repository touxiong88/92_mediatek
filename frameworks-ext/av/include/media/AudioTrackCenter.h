...
public:	
	...
	status_t addTrack(uint8_t trackId, uint32_t frameCount, uint32_t sampleRate, void* trackPtr);

	status_t removeTrack(void* trackPtr);

	status_t updateTrackMaps(void* trackPtr, void* sinkPtr);

	status_t updateServer(uint8_t trackId, uint32_t server, bool restore=false);

	uint8_t getTrackId(void* sinkPtr);
	
	status_t getRealTimePosition(uint8_t trackId, int64_t *position);

    status_t setTrackActive(uint8_t trackId, bool active);

	status_t reset(uint8_t trackId);
...
private:
	...
	Mutex mLock;
	KeyedVector<uint8_t, TrackInfo > mTrackList;
