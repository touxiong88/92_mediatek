...
extern "C" sp<MediaExtractor> mtk_flv_extractor_create_instance(const sp<DataSource> &source) {
    FLV_LOGD("[FLV]mtk_flv_extractor_create_instance\n");
...
void flv_parse_aac_info(uint8_t* data,uint32_t* sampleRate, uint32_t* channelCnt,uint32_t* bitRate)
{
    ...
    byteVal = ptr[0];
    channel_cnt = byteVal & 0xF0 >> 4;

   byteVal = ptr[1];
   freq_index = byteVal & 0x0F  ;
   profile =  byteVal & 0xF0 >> 4;

   findAACSampleFreqIndex(freq_index,&index);
  *sampleRate = AACSampleFreqTable[index];
        ...
        int *out_channels = NULL)
{
       ...
       *out_channels = header & 0x01 + 1; //0 mono. 1 stereo
        ...
        tmp = header & 0x02 >> 1;                                 
        if(tmp == 0)        *out_sampling_size =8;
        else if(tmp == 1)   *out_sampling_size =16;   
...
uint64_t flv_io_seek_func_ptr(void *pFlvExtractor, uint64_t aOffset,FLV_SEEK_FLAG flag)
{
    ...
    //FLV_LOGD("flv_io_seek_func_ptr: aOffset=%ld\n",aOffset);
...
status_t FLVSource::read(MediaBuffer **out, const ReadOptions *options) 
{
    ...
    FLV_LOGV("+FLVSource::read-----mType(0Video,1Audio)=%d---------\n",mType);
	...
	//	LOGE("mode=%d",mode); 
		...
		{
		    FLV_LOGD("+FLVSource::read use mode, seek to %lld us (mType=%d, mode=%d)", seekTimeUs, mType, mode);
            if(mType==VIDEO)
            {
            #if 0
			    if((mode == ReadOptions::SEEK_CLOSEST) 
					&& ((mExtractor->video_codec_id != FLV_VIDEO_CODEC_ID_HEVC)
					    && (mExtractor->video_codec_id != FLV_VIDEO_CODEC_ID_HEVC_PPS)
					    && (mExtractor->video_codec_id != FLV_VIDEO_CODEC_ID_HEVC_XL)))
		    #endif
			    if(mode == ReadOptions::SEEK_CLOSEST)
				{
	...
	if (AUDIO == mType) 
    {
	   FLV_LOGV ("FLVSource::read mAudioFrames.size()=%d", mExtractor->mAudioFrames.size());
    	...
    	else if (mExtractor->mAudioFrames.size() >= FLV_CACHE_POOL_LOW) // cache more sample and dequeue
        {
    	...
    	else //<FLV_CACHE_POOL_LOW
        {
    		...
    		else if (FLV_OK == retVal) 
            {   // cache samples for the first time
...
READ_AGAIN:
    ...
    if (VIDEO == mType) 
    {
	   FLV_LOGD ("FLVSource::read mVideoFrames.size()=%d", mExtractor->mVideoFrames.size());
    	...
    	else if (mExtractor->mVideoFrames.size() >= FLV_CACHE_POOL_LOW) // cache more sample and dequeue
        {
    		retVal = mExtractor->CacheMore(CACHE_FRAME);
    		if(FLV_FILE_EOF == retVal) 
            {
...
FLVExtractor::~FLVExtractor()
{
 ...
 
 {
    ...
    #define nal_type_unkown -1
	 ...
	 out = (uint8_t*)malloc((sps_len + pps_len + 28)*sizeof(uint8_t));
	 if(!out)
	 {
	 ...
	 out[6] = 0;
	 out[7] = 0;
	 out[8] = sps[sps_nal_type + 3]; // level_idc
	 ...
	 out[21] = (((sps_len - sps_nal_type))>>8) & 0b11111111;
	 out[22] = ((sps_len - sps_nal_type)) & 0b11111111;
	 memcpy(out + 23, sps + sps_nal_type, sps_len - sps_nal_type);
	 /* add pps to array */
	 out[23 + sps_len - sps_nal_type] = 0b10100001; //sps
	 /* 2 bytes numOfNalus */
	 out[24 + sps_len - sps_nal_type] = 0;
	 out[25 + sps_len - sps_nal_type] = 1; 
	 /* 2 bytes nalUnitLength */
	 out[26 + sps_len - sps_nal_type] = (((pps_len - pps_nal_type)) >> 8) & 0b11111111;
	 out[27 + sps_len - sps_nal_type] = ((pps_len - pps_nal_type)) & 0b11111111;
	 memcpy(out + 28 + sps_len - sps_nal_type, pps + pps_nal_type, pps_len - pps_nal_type);
	 *real_length = 28 + sps_len - sps_nal_type + pps_len - pps_nal_type;
		 ...
		 //find the parseNALSize
		 {
			 ...
			 case FLV_VIDEO_CODEC_ID_HEVC_XL:	
             {
				    ...
				    &&(pTag->tag_data[8] == 1))
				{
					...
					hevc_codec_info_size = pTag2->tag_header.tag_data_size + pTag3->tag_header.tag_data_size + 28;
