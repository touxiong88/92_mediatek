        ...
        if (start >= 0 && ret)
        {
            ...
            if (frameSize + start > length)
            {
                ...
                while(readret == OK)
                {
                    ...
                    {
                        ALOGI ("[ASF_Read]have enough frame size = existSize", existSize);
                        break;
                    }
...
bool ASFExtractor::RetrieveWmvCodecSpecificData(asf_stream_t * pStreamProp, const sp<MetaData>& meta,VC1SeqData* pCodecSpecificData, uint32_t currIdx)
{
    ...
    */
    ...
    else if(!strcasecmp(mime_present,MEDIA_MIMETYPE_VIDEO_MPEG4))
    {
        ...
        //sp<ABuffer> esds = MakeMPEGVideoESDS(bmih->data, _config_size_extra_data);
        //meta->setData(kKeyESDS, kTypeESDS, esds->data(), esds->size());

        meta->setData(kKeyMPEG4VOS , 0, bmih->data, _config_size_extra_data);
    ...
    else if(!strcasecmp(mime_present,MEDIA_MIMETYPE_VIDEO_AVC))
    {
        ...
        while(prefixLen == -1)
        {
            ...
            {
                ALOGI("AVCPos = %d, ptr = %d, offsetStart = %d, prefixLen = %d\n", AVCPos, ptr, offsetStart, prefixLen);
...
bool ASFExtractor::RetrieveWmaCodecSpecificData(asf_stream_t * pStreamProp, const sp<MetaData>& meta)
{
    if(pStreamProp->flags & ASF_STREAM_FLAG_AVAILABLE)
    {
        ...
        case WAVE_FORMAT_AAC_pm:
        {
            ...
            sample_rate_index = switchAACSampleRateToIndex(wfx->nSamplesPerSec);

            codecData[0] |= (profile + 1) << 3;
            codecData[0] |= ((sample_rate_index & 0x0F) >> 1);
            codecData[1] |= ((sample_rate_index & 0x01) << 7);
            codecData[1] |= wfx->nChannels << 3;

            ALOGI("REAL aac codec data %x %x", codecData[0], codecData[1]);
            meta->setData(kKeyCodecConfigInfo, 0, codecData, 2);
            //meta->setInt32(kKeyIsAACADIF, 0);
            break;
        }
        /*
        case //LPCM
...
bool ASFExtractor::ParseASF()
{
    ...
    if (mAsfParser)
    {
        ...
        else
        {
            ...
            entry =mAsfParser->asf_findMetaValueByKey(pASFMetadata,key,6);
            if(entry && entry->value) mFileMeta->setCString(kKeyArtist, entry->value);
            else ASF_DEBUG_LOGE("no kKeyArtist / Author in file meta");

            entry=NULL;
            key="WM/AlbumTitle";
            entry =  mAsfParser->asf_findMetaValueByKey(pASFMetadata,key,13);
            if(entry && entry->value) mFileMeta->setCString(kKeyAlbum,  entry->value);
            else ASF_DEBUG_LOGE("no kKeyAlbum / WM/AlbumTitle  in file meta");

            entry=NULL;
            key="Title";
            entry =  mAsfParser->asf_findMetaValueByKey(pASFMetadata,key,5);
            if(entry && entry->value) mFileMeta->setCString(kKeyTitle,  entry->value);
            else ASF_DEBUG_LOGE("no kKeyTitle / Title in file meta");

            entry=NULL;
            key="WM/Picture";
            entry =  mAsfParser->asf_findMetaValueByKey(pASFMetadata,key,10);
