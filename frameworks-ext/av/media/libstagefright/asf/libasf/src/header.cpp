        ...
        if (current) {
                        ...
                        for (i=0; i<ret->content_count; i++) 
			 {
                ...
                for (i=0; i<ret->extended_count; i++) {
                    ...
                    position += 2;
                    if((position + length)> current->datalen){                    
                         ALOGE("error in extended object key length = %d, tatal object length = %d",length,current->datalen);
                         continue;
                    }
                    ...
                    if (!(ret->extended[i].size) || (position + length)>current->datalen)
                    {
                         ALOGE("error in extended object value length = %d, tatal object length = %d",length,(current->datalen));
