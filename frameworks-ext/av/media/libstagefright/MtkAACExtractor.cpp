...
#if 0
    /* prevent too much time, vbr only below 50m can get accurate duration before play */
    if(fileSize < 50000000){
		uint32_t sr = nSampleRate;
		if (sr == 0){
		    return;
		}
	    off64_t numFrames = 0;
	    ...
	    if (fileSize != 0) {
	         while (meta_offset < fileSize) {
	            if ((frameSize = getAdtsFrameLength(mDataSource, meta_offset, NULL)) == 0) {
	                return;
	            }
	            meta_offset += frameSize;
	            numFrames ++;
	        }

	        // Round up and get the duration
	        int64_t nFrameDurationUs = (1024 * 1000000ll + (sr - 1)) / sr;
	        duration = numFrames * nFrameDurationUs;
			if(duration != 0){
