...
static bool checkVideoResolution(int32_t width, int32_t height, const char* streamtype) {
    ...
    }else if (!strcasecmp(streamtype, MEDIA_MIMETYPE_VIDEO_HEVC)){
...
status_t MtkAVIExtractor::parseSTRN(MtkAVIOffT off, MtkAVIOffT end, sp<MtkAVISource> source) {    
    if (end - off == 0) {
        LOGE("sizeof strn is not correct: end - pos == 0");
        return ERROR_MALFORMED;
	}
...
#ifdef MTK_VIDEO_HEVC_SUPPORT
		source->mIsHEVC = !strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_HEVC);
#endif
...
status_t MtkAVISource::read(
    ...
    *out = NULL;
	...
	}else if(mIsS263 || mIsXVID || mIsDIVX3){
		*out = mBuffer;
	     mBuffer = NULL;
	     return OK;
#ifdef MTK_VIDEO_HEVC_SUPPORT
