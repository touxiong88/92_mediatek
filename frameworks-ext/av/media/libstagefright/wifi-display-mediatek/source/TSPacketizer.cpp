...
TSPacketizer::Track::Track(
    ...
    if (!strcasecmp(mMIME.c_str(), MEDIA_MIMETYPE_VIDEO_AVC)
        ...
        if (!strcasecmp(mMIME.c_str(), MEDIA_MIMETYPE_AUDIO_AAC)) {
		  ...
		  ALOGD("%s has is-adts",mMIME.c_str());
...
status_t TSPacketizer::packetize(
    ...
    size_t PES_packet_length = accessUnit->size() + 8 + numStuffingBytes; // 8 = PES header legnths
...
#ifdef WFD_HDCP_TX_PAYLOAD_ALIGNMENT
      if (flags & IS_ENCRYPTED)  // HDCP encrypted 
        numTSPackets = 1 + ((PES_packet_length - 169) + 175) / 176;  // first PES packet only has 144 payload + 25 private header space, others has 176 payload space
      else
#endif    
    ...
    //ALOGI("PES_packet_length = %d numTSPackets = %d , flags = 0x%x", PES_packet_length, numTSPackets, flags);
    ...
    if (flags & EMIT_PAT_AND_PMT) {
        ...
        *ptr++ = 0x47;
        *ptr++ = 0x40; // payload_unit_start_indicator = 1
        *ptr++ = 0x00;
        ...
        *ptr++ = 0x00;
        ...
        /* HDCP Spec 3.6.2 registration_descriptoer should be defined in the "outer loop" of PMT */
        /* Android defualt put it in the "inner loop" of PMT, i.e. in ES_info */
        if (flags & IS_ENCRYPTED)  // HDCP encrypted 
        {
             ptr--;
            *ptr++ = 0x07;  // program_info_length (7 bytes)  
            ...
            *ptr++ = 0x20;
            ...
            ALOGI("Put HDCP register_descriptoer in PMT");
        }
#endif        
    ...
    padding = (PES_packet_length < (188 - 10));
...
#ifdef WFD_HDCP_TX_PAYLOAD_ALIGNMENT
    if (padding || (flags & IS_ENCRYPTED))
        *ptr++ = 0x30 | track->incrementContinuityCounter();
    else
        *ptr++ = 0x10 | track->incrementContinuityCounter();
#else    
...
#ifdef WFD_HDCP_TX_PAYLOAD_ALIGNMENT
    /* HDCP Spec 3.6.2 the payload (exculding the PES header) is an intergral multiple of 16 bytes */
    /* Note! the padding size here is for video packet only */
    else if ((flags & IS_ENCRYPTED) && (PES_packet_length != 178))  // HDCP encrypted 
    {
        size_t paddingSize = 9; 
        *ptr++ = paddingSize - 1;
        *ptr++ = 0x00;
         memset(ptr, 0xff, paddingSize - 2);
        ptr += paddingSize - 2;
    ...
    *ptr++ = ((PTS & 0x7f) << 1) | 1;
    ...
    while (offset < accessUnit->size()) {
        ...
        if (flags & IS_ENCRYPTED)
            padding = (accessUnit->size() - offset) < 176;
        else
#endif
...
#ifdef WFD_HDCP_TX_PAYLOAD_ALIGNMENT
        /* HDCP Spec 3.6.2 the payload (exculding the PES header) is an intergral multiple of 16 bytes */
        else if (flags & IS_ENCRYPTED)  // HDCP encrypted 
        {
            /* Note! the padding size here is for video packet only */
            size_t paddingSize = 8; // for payload = 176 (16*11), TS_header = 4 bytes, 188-4-176 = 8
            *ptr++ = paddingSize - 1;
            *ptr++ = 0x00;
            memset(ptr, 0xff, paddingSize - 2);
            ptr += paddingSize - 2;
    ...
    //ALOGI("packetDataStart = 0x%x buffer->data() = 0x%x buffer->capacity = 0x%x", packetDataStart, buffer->data(), buffer->capacity());
