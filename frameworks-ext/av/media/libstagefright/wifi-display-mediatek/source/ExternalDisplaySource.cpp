...
//        status_t err = mBufferQueue->acquireBuffer(&item);
    ...
    *buffer = new MediaBuffer(framebuffer, mFrameSize);
    ...
    readEndUs =  GetNowUs();
    ALOGI("[WFDP][video]read done:MediaBuffer = %p, ,mNumFramesReceived=%d,mCurrentTimestamp=%lld ms,readTimeMs=%lld,deltaUs=%lld", 
		        *buffer,mNumFramesReceived,mCurrentTimestamp/1000,(readEndUs-readStartUs)/1000,deltaUs);

    (*buffer)->meta_data()->setInt64('ToMs', (readEndUs-readStartUs)/1000);
    (*buffer)->meta_data()->setInt64('StMs', readStartUs/1000);
