...
void Sender::queuePackets(
   ...
   int32_t dummyNAL;
   if(tsPackets->meta()->findInt32("dummy-nal", &dummyNAL)){
	 dummyNAL =1;
   }
   ...
   if(isVideo){
    ...
    if(dummyNAL == 1){
		udpPackets->meta()->setInt32("dummy-nal", 1);
   }
	      ...
	      int64_t time=-1;
		 int64_t time1=-1;
	       if(tsPackets->meta()->findInt64("EoMs", &time)){
		   	udpPackets->meta()->setInt64("EoMs",  time);
		}
#endif
            ...
            switch (reason) {
		 	...
		 	size_t numBytesQueued;
            		CHECK(msg->findSize("numBytesQueued", &numBytesQueued));
                        ...
                        ALOGI("RTP kWhatNetworkStall is found.");
...
void  Sender::notifyNetworkStall(size_t numBytesQueued) {
    sp<AMessage> notify = mNotify->dup();
...
void Sender::onDrainQueue(const sp<ABuffer> &udpPackets) {
		    ...
		    int64_t delayUs1;
		    CHECK(udpPackets->meta()->findInt64("delayUs", &delayUs1));
		 ...
		 int64_t time1=-1;
		 int64_t CtoSTime =-1;
	       if(udpPackets->meta()->findInt64("EoMs", &time)){
		        time1 = ALooper::GetNowUs();
			 CtoSTime = (time1-time*1000)/1000;
		}
...
#ifndef ANDROID_DEFAULT_CODE	
	  uint32_t rtpTime = (timeUs * 9ll) / 100ll;
#else
        // 90kHz time scale
...
#ifndef ANDROID_DEFAULT_CODE	
	 bool isLastPacket = ((srcOffset + rtpPacketSize) == udpPackets->size());
	...
	if(isVideo)
	{
		...
		int32_t dummy;
		if(!udpPackets->meta()->findInt32("dummy-nal", &dummy)){
			mCountFrames ++;
	...
	debugInfo->addTimeInfoByKey(isVideo , timeUs, "SentFps", sendFps);
	
	debugInfo->addTimeInfoByKey(isVideo , timeUs, "StOt", endTimeUs/1000);
	debugInfo->addTimeInfoByKey(isVideo , timeUs, "NeTc", netTimeUs);
	...
	ALOGI("[WFD_P][%s] timestamp=%lld ms,sendFps=%d,sendTotalSize=%d,[delayUs]=%lld ms,[NetTimeUs]=%lld ms,[ Latency]=%lld ms",
			isVideo ? "video" : "audio",timeUs/1000,sendFps,udpPackets->size(),delayUs1/1000,netTimeUs/1000,latencyTimeMs);
