...
#endif
{
    mSupportedSourceVideoFormats.SetDefaultConfiguration();
}
        ...
        case kWhatPlaybackSessionNotify:
        {
            ...
            if (what == PlaybackSession::kWhatSessionDead) {
                ...
                if (mClient != NULL) {
                    ...
                    int layerWidth = 1280;
                    int layerHeight = 720;
                    int displayRectWidth = mClientInfo.mPlaybackSession->width();
                    int displayRectHeight = mClientInfo.mPlaybackSession->height();
                    ...
                    mSurfaceTexture = mClientInfo.mPlaybackSession->getSurfaceTexture();
...
status_t WifiDisplaySource::sendM4(int32_t sessionID) {
    ...
    AString videoFormatString;
    videoFormatString.append("wfd_video_formats: ");
    ...
    if((mWfdFlags & kSigmaTest) == kSigmaTest){
       ...
       videoFormatString.append("00 00 01 01 00000001 00000000 00000000 00 0000 0000 01 none none");
    }else{
    #ifndef ANDROID_DEFAULT_CODE
        if (mSupportedSinkVideoFormats.supportCBPOnly()){
            videoFormatString.append(chosenVideoFormat.getCBPFormatSpec(true /* forM4Message */));
        } 
        else                   
    #endif
        {
            videoFormatString.append(chosenVideoFormat.getFormatSpec(true /* forM4Message */));
        }
    }
    
    videoFormatString.append("\r\n");    

    ALOGI("videoFormatString:%s", videoFormatString.c_str());
...
///M: Support Video Format @{
    ...
    if (!(value == "none")) {
       ...
       ALOGI("Picked video resolution %u x %u %c%u",
            ...
            ALOGE("Sink doesn't support video at all.");
            return ERROR_UNSUPPORTED;
    }
...
///@}
    ...
    if(uibcCapability.startsWith("input_category_list=GENERIC")){
        ...
        ALOGI("Sink's UIBC:%d", mClientInfo.mUibcSupported);
    }else{
...
///@}
    ...
    else
    {
       ...
       playbackSession =
            new PlaybackSession(
                    mNetSession, notify, mInterfaceAddr, NULL);
    }
#else
    sp<PlaybackSession> playbackSession =
        new PlaybackSession(
    ...
    if((mWfdFlags & kSigmaTest) == kSigmaTest){
        ...
        ALOGI("Init video resolution %u x %u %u",
              width, height, framesPerSecond);
        playbackSession->setWidth(width);
        playbackSession->setHeight(height);
        playbackSession->setFrameRate(framesPerSecond);    
    }

#ifndef ANDROID_DEFAULT_CODE
    if (mSupportedSinkVideoFormats.supportCBPOnly()){
        playbackSession->setProfileLevel(
            (1ul <<VideoFormats::PROFILE_CBP), (1ul <<VideoFormats::LEVEL_31));
        ALOGI("CBP only!");
    }                   
#endif
...
int WifiDisplaySource::getWfdParam(int paramType) {
    ...
    // 4: if video profile is CBP
        ...
        paramValue = mSupportedSinkVideoFormats.supportCBPOnly() ? 1 : 0;
       
    }    
#ifndef ANDROID_DEFAULT_CODE 
    // 5: average latency, 6: sink fps
    else if (paramType == 5 || paramType == 6 || paramType == 7) {
