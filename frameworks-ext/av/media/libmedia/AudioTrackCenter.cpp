...
status_t AudioTrackCenter::addTrack(uint8_t trackId, uint32_t frameCount, uint32_t sampleRate, void* trackPtr) {
...
status_t AudioTrackCenter::updateServer(uint8_t trackId, uint32_t server, bool restore) {
    Mutex::Autolock autoLock(mLock);
...
uint8_t AudioTrackCenter::getTrackId(void* sinkPtr) {
	Mutex::Autolock autoLock(mLock);
    ...
    SXLOGV("%s, return trackId:%d",__FUNCTION__, 0xff);
	return 0xff;
}

status_t AudioTrackCenter::getRealTimePosition(uint8_t trackId, int64_t *position) {
    Mutex::Autolock autoLock(mLock);
	...
	*position = 0;
	...
	if (info.server) {
		...
		*position = info.framePlayed + deltaFrames;
	}
...
status_t AudioTrackCenter::setTrackActive(uint8_t trackId, bool active) {
    Mutex::Autolock autoLock(mLock);
...
status_t AudioTrackCenter::reset(uint8_t trackId) {
	Mutex::Autolock autoLock(mLock);
