...
void SurfaceFlinger::createProtectedImageTextureLocked() {
    SkBitmap bitmap;
    ...
    glGenTextures(1, &mProtectedImageTexName);
    glBindTexture(GL_TEXTURE_2D, mProtectedImageTexName);
...
void SurfaceFlinger::deleteProtectedImageTexture() {
    Mutex::Autolock _l(mProtectedImageLock);
    if (mProtectedImageTexName != -1U) {
        deleteTextureAsync(mProtectedImageTexName);
        mProtectedImageTexName = -1U;
    }
}

GLuint SurfaceFlinger::getProtectedImageTexName() {
    Mutex::Autolock _l(mProtectedImageLock);
    if (mProtectedImageTexName == -1U) {
        createProtectedImageTextureLocked();
    }
