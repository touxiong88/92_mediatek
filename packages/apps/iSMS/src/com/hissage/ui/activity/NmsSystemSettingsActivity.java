...
public class NmsSystemSettingsActivity extends PreferenceActivity {
    ...
    private void updateStatus() {
        ...
        if ((simId1 > 0 && NmsIpMessageApiNative.nmsGetActivationStatus((int) simId1) == NmsSimActivateStatus.NMS_SIM_STATUS_ACTIVATED)
            ...
            if (!(NmsCommonUtils.getSDCardStatus())
                    && NmsCommonUtils.getSysStorageAvailableSpace() >= 5 * 1024 * 1024) {
                // NO SD && phone >= 5M
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        /*
         * if (preference == mProfile) { Intent i = new Intent(this,
         * NmsProfileSettingsActivity.class); this.startActivity(i); } else if
         * (preference == mSimInfo1) { getAllSimInfo() ; boolean isChecked =
         * mSimInfo1.isChecked();
         * 
         * SNmsSimInfo info = NmsIpMessageApiNative .nmsGetSimInfoViaSimId((int)
         * mSimInfo[NmsConsts.SIM_CARD_SLOT_1].sim_id); if (null == info) {
         * mSimInfo1.setChecked(false); Toast.makeText(this,
         * R.string.STR_NMS_ENABLE_FAILED, Toast.LENGTH_SHORT).show(); return
         * true; } if (isChecked && info.status <
         * NmsSimActivateStatus.NMS_SIM_STATUS_ACTIVATED) {
         * mSimInfo1.setChecked(false); if (NmsCommonUtils.isNetworkReady(this))
         * { showActivitionDlg(mSimInfo[NmsConsts.SIM_CARD_SLOT_1].sim_id,
         * NmsBaseActivity.WELCOME); } else { Toast.makeText(this,
         * R.string.STR_NMS_NO_CONNECTION, Toast.LENGTH_SHORT).show(); } return
         * true; }
         * 
         * soltId = 0; if (isChecked) { if((NmsCommonUtils.getSDCardStatus() &&
         * NmsCommonUtils.getSDcardAvailableSpace() < 5*1024*1024 &&
         * NmsCommonUtils.getExternalSdCardAvailableSpace() < 5*1024*1024 &&
         * NmsCommonUtils.getSysStorageAvailableSpace()< 5*1024*1024)
         * ||(!(NmsCommonUtils.getSDCardStatus()) &&
         * NmsCommonUtils.getSysStorageAvailableSpace() < 5*1024*1024)) { // (SD
         * && SD < 5M && Phone < 5M) || (No SD && Phone < 5M )
         * mSimInfo1.setChecked(false); Toast.makeText(this,
         * R.string.STR_NMS_SPACE_NOT_ENOUGH_ADD, Toast.LENGTH_SHORT).show();
         * return true; }else if (NmsSimActivateStatus.NMS_SIM_STATUS_ACTIVATED
         * != info.status) {
         * NmsStatistics.incKeyVal(NmsStatistics.KEY_ENABLE_ISMS) ;
         * NmsIpMessageApiNative .nmsEnableIpService((int)
         * mSimInfo[NmsConsts.SIM_CARD_SLOT_1].sim_id); showWaitDlg(); } } else
         * { if (NmsSimActivateStatus.NMS_SIM_STATUS_DISABLED != info.status) {
         * NmsStatistics.incKeyVal(NmsStatistics.KEY_DISABLE_ISMS) ;
         * NmsIpMessageApiNative .nmsDisableIpService((int)
         * mSimInfo[NmsConsts.SIM_CARD_SLOT_1].sim_id); showWaitDlg(); } } }
         * else if (preference == mSimInfo2) { getAllSimInfo() ; boolean
         * isChecked = mSimInfo2.isChecked(); SNmsSimInfo info =
         * NmsIpMessageApiNative .nmsGetSimInfoViaSimId((int)
         * mSimInfo[NmsConsts.SIM_CARD_SLOT_2].sim_id); if (null == info) {
         * mSimInfo2.setChecked(false); Toast.makeText(this,
         * R.string.STR_NMS_ENABLE_FAILED, Toast.LENGTH_SHORT).show(); return
         * true; } if (isChecked && info.status <
         * NmsSimActivateStatus.NMS_SIM_STATUS_ACTIVATED) {
         * mSimInfo2.setChecked(false); if (NmsCommonUtils.isNetworkReady(this))
         * { showActivitionDlg(mSimInfo[NmsConsts.SIM_CARD_SLOT_2].sim_id,
         * NmsBaseActivity.WELCOME); } else { Toast.makeText(this,
         * R.string.STR_NMS_NO_CONNECTION, Toast.LENGTH_SHORT).show(); } return
         * true; } soltId = 1; if (isChecked) {
         * if((NmsCommonUtils.getSDCardStatus() &&
         * NmsCommonUtils.getSDcardAvailableSpace() < 5*1024*1024 &&
         * NmsCommonUtils.getExternalSdCardAvailableSpace() < 5*1024*1024 &&
         * NmsCommonUtils.getSysStorageAvailableSpace()< 5*1024*1024)
         ...
         * NmsStatistics.incKeyVal(NmsStatistics.KEY_ENABLE_ISMS) ;
         * NmsIpMessageApiNative .nmsEnableIpService((int)
         * mSimInfo[NmsConsts.SIM_CARD_SLOT_2].sim_id); showWaitDlg(); } } else
         * { if (NmsSimActivateStatus.NMS_SIM_STATUS_DISABLED != info.status) {
         * NmsStatistics.incKeyVal(NmsStatistics.KEY_DISABLE_ISMS) ;
         * NmsIpMessageApiNative .nmsDisableIpService((int)
         * mSimInfo[NmsConsts.SIM_CARD_SLOT_2].sim_id); showWaitDlg(); } } }
         * else
