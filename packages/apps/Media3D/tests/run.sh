...
INTERMEDIATES=$SRC_ROOT/out/target/common/obj/APPS/Media3D_intermediates
...
then
  ...
  adb uninstall com.mediatek.media3d
  adb install -r $PRODUCT_OUT/system/app/Media3D.apk
  adb uninstall com.mediatek.media3d.tests
  adb install -r $PRODUCT_OUT/data/app/Media3DTests.apk

  # remove the shader cache to evaluate the first time launch without cache.
  adb shell rm -r /data/data/com.mediatek.media3d/cache

  # Run instrumentation test
  PACKAGE=com.mediatek.media3d
  adb shell am instrument -e coverage true -w $PACKAGE.tests/$TEST_RUNNER
  adb pull /data/data/$PACKAGE/files/junit-report.xml $SRC_ROOT/junit-report-media3d.xml
