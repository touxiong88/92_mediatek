...
public class DmBootstrapHandler implements BootMsgHandler, SessionInitiator {
    ...
    // private int getGprsConnectionSlotId() {
    // mSiminfoList = SimInfoManager.getInsertedSimInfoList(DmService.getInstance());
    // int slotId = -1;
    // long simid = Settings.System.GPRS_CONNECTION_SIM_SETTING_NEVER;
    // if (FeatureOption.MTK_GEMINI_SUPPORT) {
    // simid = Settings.System.getLong(DmService.getInstance().getContentResolver(),
    // Settings.System.GPRS_CONNECTION_SIM_SETTING,
    // Settings.System.DEFAULT_SIM_NOT_SET);
    // } else {
    // ConnectivityManager connService = ConnectivityManager.from(DmService.getInstance());
    // if (connService.getMobileDataEnabled()) {
    ...
    // Xlog.d(TAG, "data connection is closed");
    ...
    // if (size > 1) {
    // SimInfoRecord tempSIMInfo = findSIMInfoBySimId(simid);
    // if (tempSIMInfo == null) {
    // Xlog.d(TAG,
    // "Error can not find the sim id with related siminfo");
    // } else {
    // pos = tempSIMInfo.mSimSlotId;
    // }
    // }
