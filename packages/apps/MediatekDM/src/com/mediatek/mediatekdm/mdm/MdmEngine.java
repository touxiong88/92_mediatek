...
public class MdmEngine {
    ...
    /**
     * Trigger a bootstrap session. To receive notification when a bootstrap session state changes, register as a session
     ...
     * @param message The binary bootstrap data, pre-configured or received from server.
     * @param handler Handler of bootstrap events.
     * @param initiator Session initiator that will be passed as a parameter upon session state notifications.
     * @throws MdmException with a MDMError error code
     */
    public void triggerBootstrapSession(String account, BootProfile profile,
            CpSecurity security,
            String mac, byte[] message, BootMsgHandler handler, SessionInitiator initiator)
            throws MdmException {
    ...
    public synchronized void triggerReportSession(
        ...
        if (!isIdleN()) {
            mLogger.logMsg(MdmLogLevel.DEBUG, "Engine is not idle, enqueue the trigger.");
            TriggerRecord tr = new TriggerRecord(TriggerRecord.TriggerType.DM_REPORT, initiator);
            ...
            triggerReportSessionInternal(correlator, format, type, mark, source, target, data, account, initiator);
        }
        mLogger.logMsg(MdmLogLevel.DEBUG, "[ENGINE] -triggerReportSession()");
    }

    private synchronized void triggerReportSessionInternal(
            String correlator,
            String format,
            String type,
            String mark,
            ...
            SessionInitiator initiator)
            throws MdmException {
        mLogger.logMsg(MdmLogLevel.DEBUG, "triggerReportSessionInternal called");
        mSessionInitiator = initiator;
        if (correlator == null ||
                type == null ||
                source == null ||
        ...
        if (data != null &&
                !(format != null && (format.equals("int") || format.equals("text/plain")))) {
            throw new MdmException(MdmError.BAD_INPUT);
        }

        clearSessionActions();
        if (0 != triggerReportSessionN(
                correlator,
                format,
                type,
                mark,
    ...
    private native int triggerReportSessionN(
            String correlator,
            String format,
            String type,
            String mark,
