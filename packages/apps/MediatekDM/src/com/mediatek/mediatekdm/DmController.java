...
public class DmController {
    ...
    public DmController(
        ...
        mDmConfig = new DmConfig(service);
        mDmConfig.configure();
        ...
        registerDevIdNodeIOHandler(service);

        String mccmnc = null;
        if (Options.USESMSREGISTER) {
            int simSlot = Utilities.getRegisteredSimId(service);
            if (simSlot != -1) {
                TelephonyManagerEx teleMgr = TelephonyManagerEx.getDefault();
                mccmnc = teleMgr.getSimOperator(simSlot);
                Log.w(TAG.CONTROLLER, "reg sim MccMnc: " + mccmnc);
            }
        }

        DmRegister dr = new DmRegister(mService, mccmnc);
        dr.registerCCNodeIoHandler(DmConst.Path.getPathInData(mService, DmConst.Path.DM_TREE_FILE));
     ...
     */
    private void registerSwVNodeIOHandler() {
        try {
            mDmTree.registerNodeIoHandler(DmConst.NodeUri.DEVDETAIL_SWV, new DmDevSwVNodeIoHandler());
            Log.i(TAG.CONTROLLER, "Node : " + DmConst.NodeUri.DEVDETAIL_SWV + "IO handler registered");
        } catch (MdmException e) {
     ...
     * Register node IO handler of getting software version
     */
    private void registerDevIdNodeIOHandler(DmService service) {
        try {
            mDmTree.registerNodeIoHandler(DmConst.NodeUri.DEVINFO_DEVID, new DmDevIdNodeIoHandler(service));
            Log.i(TAG.CONTROLLER, "Node : " + DmConst.NodeUri.DEVINFO_DEVID + "IO handler registered");
        } catch (MdmException e) {
    ...
    private boolean syncDmServerAddr() {
        Assert.assertFalse("syncDmServerAddr MUST NOT called in direct internet", Options.USEDIRECTINTERNET);
    ...
    public static class DmAction {
        ...
        public String toString() {
            return "DmAction = { fumoAction = " + Integer.toHexString(fumoAction)
                    + ", lawmoAction = " + Integer.toHexString(lawmoAction)
                    + ", scomoAction = " + Integer.toHexString(scomoAction) + "}";
        }
    }
