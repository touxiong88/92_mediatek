...
public class ScomoManager extends SessionHandler implements IScomoManager, MdmScomoHandler, MdmScomoDpHandler,
        MdmScomoDcHandler {
    ...
    private static final String SCOMO_RESUME_FILE_NAME = "scomoresume.dat";
    private static final String SCOMO_FILE_NAME = "scomo.zip";
    ...
    public ScomoManager(DmService service) {
        ...
        try {
            mScomo = MdmScomo.getInstance(DmConst.NodeUri.SCOMO_ROOT, this);
            ...
            ArrayList<MdmScomoDp> dps = MdmScomo.getInstance(DmConst.NodeUri.SCOMO_ROOT, this).getDps();
    ...
    public void newDpAdded(String dpName) {
        ...
        // Update Current DP
        try {
            MdmScomo scomo = MdmScomo.getInstance(DmConst.NodeUri.SCOMO_ROOT, this);
    ...
    public void setVerbose(boolean verbose) {
        if (mScomoState == null) {
            Log.d(TAG.SCOMO, "setVerbose with mScomoState is null!");
            return;
        } else {
            Log.d(TAG.SCOMO, "+setVerbose(" + verbose + ")");
            mScomoState.verbose = verbose;
            DmScomoState.store(mService, mScomoState);
        }
    }
    ...
    protected void dmComplete(DmAction action) {
        ...
        DmPLDlPkg.setDeltaFileName(ScomoManager.SCOMO_FILE_NAME);
        ...
        if ((action.scomoAction & ScomoAction.DOWNLOAD_EXECUTED) != 0 ||
                (action.scomoAction & ScomoAction.DOWNLOAD_INSTALL_EXECUTED) != 0 ||
                (action.scomoAction & ScomoAction.DOWNLOAD_INSTALL_INACTIVE_EXECUTED) != 0) {
            // Reset file state
            clearFiles();
            // Cut in line
    ...
    private void clearFiles() {
        ...
        mService.deleteFile(SCOMO_FILE_NAME);
        mService.deleteFile(SCOMO_RESUME_FILE_NAME);
        Log.d(TAG.SCOMO, "-clearFiles()");
    }
    ...
    public void resumeDlPkg() {
        ...
        DmOperation operation = new DmOperation(
                DmOperation.generateId(), DL_TIME_OUT, DL_MAX_RETRY);
    ...
    private void scomoScanPackageInternal() {
        ...
        try {
            ...
            MdmScomo scomoInstance = MdmScomo.getInstance(DmConst.NodeUri.SCOMO_ROOT, null);
