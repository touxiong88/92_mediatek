...
public abstract class DmDBNodeIoHandler implements NodeIoHandler {
    ...
    protected final String getMccMnc() {
        if (Options.USESMSREGISTER) {
            if (mMccMnc == null) {
                int simSlot = Utilities.getRegisteredSimId(mContext);
                if (simSlot != -1) {
                    TelephonyManagerEx teleMgr = TelephonyManagerEx.getDefault();
                    mMccMnc = teleMgr.getSimOperator(simSlot);
