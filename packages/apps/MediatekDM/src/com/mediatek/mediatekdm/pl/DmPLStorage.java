...
public class DmPLStorage implements PLStorage {
    ...
    public void delete(ItemType itemType) {
        ...
        if (ItemType.DMTREE == itemType) {
            ...
            if (operation.getProperty(KEY.TYPE).contains("SCOMO") ||
                    operation.getBooleanProperty(KEY.SCOMO_TAG, false)) {
                fileName = mService.getScomoManager().getDlResumeFilename();
            } else {
                fileName = mService.getFumoManager().getDlResumeFilename();
            }
        } else {
    ...
    public PLFile open(ItemType fileType, AccessMode accessMode) throws IOException {
        ...
        if (ItemType.DMTREE == fileType) {
            ...
            if (operation.getProperty(KEY.TYPE).contains("SCOMO") ||
                ...
                File f = new File(DmConst.Path.getPathInData(mService, DmConst.Path.RESUME_FILE));
                if (!f.exists()) {
