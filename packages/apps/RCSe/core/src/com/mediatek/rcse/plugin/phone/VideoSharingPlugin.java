...
public class VideoSharingPlugin extends SharingPlugin implements SurfaceHolder.Callback,
    ...
    private static final String DEFAULT_VIEDO_FORMAT = "h264";
    private static final String DEFAULT_VIEDO_FORMAT_STORED = "h264";
    private static final String H264_VIEDO_FORMAT = "h264";
    private static final String H263_VIEDO_FORMAT = "h263-2000";
    private static final String SUPPORT_FORMAT_H263 = "263";
    ...
    private static int orientResult = 0;
    ...
    // Stored Video Share
    private String mStoredVideoFormat = null;
    ...
    private String mVideoEncoding = null;
    private int mStoredVideoWidth;
    private int mStoredVideoHeight;
    ...
    /**
     * Start a live/stored video sharing based on current status.
     * Start sharing may not execute.
     * @param isLive True if start a live sharing, false if start a stored sharing.
     */
    private void startVideoShare(boolean isLive) {
        ...
        if (isLive) {
            if (!Utils.isInImageSharing() && allowOutgoingLiveSharing()) {
            int networkType = Utils.getNetworkType(mContext);
                startByNetwork(networkType, isLive);
            } else {
            ...
            if (!Utils.isInImageSharing() && allowOutgoingStoredSharing()) {
                int networkType = Utils.getNetworkType(mContext);
                startByNetwork(networkType, isLive);
            } else {
                Logger.w(TAG, "startVideoShare() Start has been clicked,"
    ...
    /**
     * Start a video sharing(live or stored) or not according to current network
     ...
     * @param isLive True if start a live sharing, false if start a stored
     *            sharing.
     */
    private void startByNetwork(int networkType, boolean isLive) {
        if (isLive) {
            ...
            Logger.d(TAG, "start a stored sharing, modify status");
        ...
        if (mShareStatus == ShareStatus.LIVE_OUT
            ...
            Logger.v(TAG,
                ...
                IVideoPlayerEventListener listener = new IVideoPlayerEventListener.Stub() {
                    ...
                    if(mTimeBar != null)
                    {
                        updatePlayingProgress(mVideoDuration);
                        mTimeBar.setShowScrubber(false);
                        mTimeBar.setScrubbing(false);
                    }
                    if (mControllButton != null) {
                        Logger.v(TAG,"endOfStream mControllButton is null");
                        mControllButton.hideButton();
                    } else {
                        Logger.w(TAG, "displayStoredVideoUI(), mControllButton is null!");
                    }
            ...
            Logger.v(TAG,
                    "startStoredVideoShare() start createPrerecordedVideoPlayer mStoredVideoFormat is: "
                            + mStoredVideoFormat);
                try {
                mOutgoingVideoPlayer = mRichCallApi
                        .createPrerecordedVideoPlayer(
                                (mStoredVideoFormat != null ? mStoredVideoFormat
                                        : DEFAULT_VIEDO_FORMAT_STORED), mVideoFileName,
                                        listener, mStoredVideoWidth, mStoredVideoHeight);
                } catch (ClientApiException e) {
                    e.printStackTrace();
                }
            }
    ...
    /**
     * Check whether allow a incoming stored sharing
     * @return True if allow a outgoing stored sharing
     */
    private boolean allowOutgoingStoredSharing() {
        boolean allow = false;
        if (mShareStatus == ShareStatus.STORED_IN
                || mShareStatus == ShareStatus.STORED_OUT
                || mShareStatus == ShareStatus.LIVE_OUT
        ...
        Logger.d(TAG, "allowOutgoingStoredSharing(): return " + allow);
        return allow;
    }
        ...
        synchronized (mLock) {
            if (mCamera != null) {
                ...
                if(info != null)
                {
                    ...
                    int rotation = mCallScreenHost.getCallScreenActivity().getWindowManager().getDefaultDisplay()
                            .getRotation();
                    int degrees = 0;
                    switch (rotation) {
                    ...
                    if (info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
                        orientResult = (info.orientation + degrees) % 360;
                        orientResult = (360 - orientResult) % 360;  // compensate the mirror
                    } else {  // back-facing
                        orientResult = (info.orientation - degrees + 360) % 360;
                    }
                    mCamera.setDisplayOrientation(orientResult);
                }
    ...
    private void showSenderStoredView() {
        ...
        int width = mOutgoingDisplayArea.getWidth();
        int height = mOutgoingDisplayArea.getHeight();
        Logger.v(TAG, "showSenderStoredView(), width = " + width + ",height = " + height
                + "mVideoWidth = " + mVideoWidth + ",mVideoHeight = " + mVideoHeight);
        LayoutParams params =
                new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                        ViewGroup.LayoutParams.MATCH_PARENT);
        mOutgoingLocalVideoSurfaceView.setLayoutParams(params);
        RelativeLayout.LayoutParams layoutParams =
                new RelativeLayout.LayoutParams(QVGA_WIDTH,
                        QVGA_HEIGHT);
        layoutParams.addRule(RelativeLayout.CENTER_IN_PARENT);
        mOutgoingDisplayArea.setGravity(Gravity.CENTER);
        mOutgoingDisplayArea.addView(mOutgoingLocalVideoSurfaceView, layoutParams);
        Logger.w(TAG, "showSenderStoredView((), DEFAULT_VIEDO_FORMAT_STORED"+ DEFAULT_VIEDO_FORMAT_STORED);
        try {
            mOutgoingLocalVideoRenderer =
    ...
    private void updatePlayingProgress(final long currentDuration) {
        ...
        mMainHandler.post(new Runnable() {
            @Override
            public void run() {
                if (mTimeBar != null && (currentDuration <= mVideoDuration)) {
                    mTimeBar.setTime((int) currentDuration, (int) mVideoDuration);
                }
                if(currentDuration >= mVideoDuration)
                {
                    mVideoFinished = true;
                    if (mControllButton != null) {
                        Logger.v(TAG,"endOfStream mControllButton is null");
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent != null) {
                ...
                if (VIDEO_SHARING_INVITATION_ACTION.equals(action)) {
                    ...
                    Logger.v(TAG, "onReceive Received Duration is()"+ mVideoDuration);
                    mVideoEncoding = intent.getStringExtra(VIDEO_ENCODING);
                    mStoredVideoWidth = intent.getIntExtra(VIDEO_WIDTH, 0);
                    mStoredVideoHeight = intent.getIntExtra(VIDEO_HEIGHT, 0);
                    if(mVideoEncoding.indexOf(SUPPORT_FORMAT_H264) != -1){
                        mStoredVideoFormat = H264_VIEDO_FORMAT;
                    }else if(mVideoEncoding.indexOf(SUPPORT_FORMAT_H263) != -1){
                        mStoredVideoFormat = H263_VIEDO_FORMAT;
                    }
                    Logger.d(TAG, "onReceive(), mVideoFileName: " + mVideoFileName+"mVideoDuration: " +mVideoDuration+"mVideoEncoding: "+mVideoEncoding);
                    initAudioButton();
                    //Start stored sharing
                    startVideoShare(false);
        ...
        public void showSelectVideoDialog() {
            ...
            if (mCallScreenHost != null) {
                ...
                if (context != null) {
                    ...
                    mSelectionDialog.setCancelListener(
