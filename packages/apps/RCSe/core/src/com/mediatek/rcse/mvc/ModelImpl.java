...
public class ModelImpl implements IChatManager {
            ...
            if (messageApi != null) {
                try {
                    ...
                    if (isChatExisted(fromSessionParticipant)) {
                        ...
                        ((One2OneChat) chat).addReceiveFileTransfer(fileTransferSession, isAutoAccept);
    ...
    public void handleMessageDeliveryStatus(String contact, String msgId, String status,
        ...
        IChat chat = getOne2OneChat(new Participant(remoteContact, remoteContact));
        if (null != chat) {
...
//                onFileTransferFinished(IOnSendFinishListener.Result.REMOVABLE);
        ...
        public void onNetworkStatusChanged(boolean isConnected) {
            Logger.d(TAG, "onNetworkStatusChanged isConnected = " + isConnected);
            onCancel();
            onFileTransferFinished(IOnSendFinishListener.Result.REMOVABLE);
        }
    }
    ...
    @Override
    public void reloadMessages(final String tag, final List<Integer> messageIds) {
        Logger.d(TAG, "reloadMessages() messageIds: " + messageIds + " tag is " + tag);
        final ContentResolver contentResolver = AndroidFactory.getApplicationContext()
                .getContentResolver();
                ...
                for (IChat chat : chats) {
                    if (chat instanceof One2OneChat
                            && ((One2OneChat) chat).getChatTag().toString().equals(tag)) {
        
                         reloadOne2OneMessages(messageIds, contentResolver);
                        break;
                    } else if (chat instanceof GroupChat
                            && ((GroupChat) chat).getChatTag().toString().equals(tag)) {

                        reloadGroupMessages(messageIds, contentResolver,tag,chat);
                        break;
            }
        }
