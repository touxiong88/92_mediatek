...
public class RichCallApi extends ClientApi {
     ...
     * @param int Video file height
     * @param int Video file width
     */
    public IMediaPlayer createPrerecordedVideoPlayer(String codec, String filename,
            IVideoPlayerEventListener listener, int width, int height) throws ClientApiException {
        if (coreApi != null) {
            try {
                return coreApi.createPrerecordedVideoPlayer(codec, filename, listener, width, height);
            } catch (RemoteException e) {
                throw new ClientApiException(e.getMessage());
            }
        } else {
