...
public class SessionTimerManager extends PeriodicRefresher {
    ...
    private void sendReInvite(SipRequest reInvite) throws SipException, CoreException {
        ...
        if (ctx.isSipResponse()) {
            ...
            /**
             * M: Add to handle the sip 422 response. @{
             */
            
            else if(ctx.getStatusCode() == 422) {
            	// 422 response received
    	    	if (logger.isActivated()) {
    	    		logger.info("422 response received. send reinvite with  new session expire value");
    	    	}
    	        ...
    	        if (minExpire == -1) {
    	            if (logger.isActivated()) {
        	    		logger.info("422 response received. Can't read the Min-SE value ");
        	    	}
    	        	return;
    	        }
    	        
    	     // Set the min expire value
    	        session.getDialogPath().setMinSessionExpireTime(minExpire);

    	        // Set the expire value
    	        session.getDialogPath().setSessionExpireTime(minExpire);
                ...
                // Set the Authorization header
                session.getAuthenticationAgent().setAuthorizationHeader(newReInvite);

                // Reset initial request in the dialog path
                session.getDialogPath().setInvite(newReInvite);
