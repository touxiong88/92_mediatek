...
public class ImsServiceDispatcher extends Thread {
    ...
    private void dispatch(SipRequest request) {
        ...
        String localIpAddress = imsModule.getCurrentNetworkInterface().getNetworkAccess()
                .getIpAddress();
        /**
      ...
      // see http://code.google.com/p/android-rcs-ims-stack/issues/detail?id=63 
          if (imsNetItf.getRegistrationManager().isBehindNat()) 
          { 
        ...
        if (request.getMethod().equals(Request.OPTIONS)) {
            ...
            if (isTagPresent(sdp, "msrp")
                ...
                if (RcsSettings.getInstance().isImageSharingSupported()) {
                    ...
                    // Service not supported: reject the invitation with a 603
                    // Decline
                    if (logger.isActivated()) {
             ...
             */
                ...
                if (SipUtils.getAssertedIdentity(request).contains(StoreAndForwardManager.SERVICE_URI)&&
                  (!request.getContentType().contains("multipart"))) {
                    ...
                    imsModule.getInstantMessagingService()
                    ...
                    imsModule.getInstantMessagingService().receiveStoredAndForwardPushMessages(
                            request);
                } else {
                    // 1-1 chat session
                    if (logger.isActivated()) {
                ...
                if (intentMgr.isSipIntentResolved(request)) {
                    ...
                    // Unknown service: reject the invitation with a 606 Not
                    // Acceptable
                    if (logger.isActivated()) {
            ...
            try {
                ...
                SipResponse response = SipMessageFactory.createResponse(request, IdGenerator
                        .getIdentifier(), 200);
