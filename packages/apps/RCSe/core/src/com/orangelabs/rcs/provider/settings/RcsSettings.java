...
public class RcsSettings {
	...
	public String readParameter(String key) {
	     ...
	     */
        String curIcci = StartService.getIcciFromSimCard();
        if (curIcci == null) {
            if (LOGGER.isActivated()) {
                LOGGER.debug("readParameter()-curIcci is null");
            }
            return null;
                    }
        String result = null;
        String where = RcsSettingsData.KEY_KEY + "='" + key + "'";
        ArrayList<String> lastUserIcciList = StartService.getLastUserIcciList();
        boolean curIcciExist = lastUserIcciList.contains(curIcci);
                if (LOGGER.isActivated()) {
            LOGGER.debug("readParameter()-curIcciExist is "
                    + (curIcciExist ? "exist" : "not exist"));
                }
        if (curIcciExist) {
            for (int i = 0; i < lastUserIcciList.size(); i++) {
                String lastUser = lastUserIcciList.get(i);
                if (lastUser != null && lastUser.equals(curIcci)) {
                            Uri uri = null;
                            switch (i) {
                                case 0:
                            ...
                            if(LOGGER.isActivated()){
                        LOGGER.debug("readParameter(), uri = " + (uri == null ? "null" : uri));
                            }
                            if (uri != null) {
            ...
            Cursor c = cr.query(RcsSettingsData.FIRST_USER_ACCOUNT_CONTENT_URI, null, where,
                    null, null);
                        if ((c != null) && (c.getCount() > 0)) {
                            if (c.moveToFirst()) {
                                result = c.getString(2);
	...
	public void writeParameter(String key, String value) {
        ...
        if (curIcci == null) {
            if (LOGGER.isActivated()) {
                LOGGER.debug("writeParameter()-curIcci is null");
            }
            return;
            }
        ...
        boolean curIcciExist = lastUserIcciList.contains(curIcci);
                            if (LOGGER.isActivated()) {
            LOGGER.debug("writeParameter()-curIcciExist is "
                    + (curIcciExist ? "exist" : "not exist"));
                            }
        if (curIcciExist) {
            // Only for debug mode
                ...
                swapTable(RcsSettingsData.SECOND_USER_ACCOUNT_CONTENT_URI,
                        RcsSettingsData.THIRD_USER_ACCOUNT_CONTENT_URI);
                swapTable(RcsSettingsData.FIRST_USER_ACCOUNT_CONTENT_URI,
                        RcsSettingsData.SECOND_USER_ACCOUNT_CONTENT_URI);
                                    }
    ...
    private void swapTable(Uri from, Uri to) {
        Cursor cursor = null;
                        try {
            cursor = cr.query(from, null, null, null, null);
                    ...
                    cr.update(to, oldValue, RcsSettingsData.KEY_KEY + "='" + rowName + "'", null);
                                    cursor.moveToNext();
                                }
                            }
                        } finally {
     ...
     * */
	/**
     * Set the RCS service activation state in the preference file off our joyn
     *
     * @param state State
     */
	private void setServiceStatePreference(boolean registrationState){
    	...
    	SharedPreferences pref = mContext.getApplicationContext().getSharedPreferences("gsma.joyn.preferences", Activity.MODE_WORLD_READABLE);
    ...
    /**
     * M: Added to set the default user profile information to a User.@{
     */
    ...
    private void notifyRcseOnlyApnStateChanged(boolean status) {
        if (!LauncherUtils.DEBUG_ENABLE_ONLY_APN_FEATURE) {
            return;
        }
