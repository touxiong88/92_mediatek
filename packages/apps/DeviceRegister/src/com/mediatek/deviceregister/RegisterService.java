...
public class RegisterService extends Service {
    ...
    // ****ITelephony API for read/write UIM*****
    // public byte[] transmitIccSimIOExGemini(int fileID, int command,
    // int p1, int p2, int p3, String filePath, String data, String pin2,
    // int simId)

    // private void testUIMIO() {
    // String meid = "A0000036DBD438";
    // byte[] meidByte = hexStringToBytes(meid);
    // byte[] meidByteReverse = getReverseBytes(meidByte);
    // String meidReverse = bytesToHexString(meidByteReverse);
    //
    ...
    //
    // int length = 8;
    // Log.w(SUB_TAG, "write content:" + pEsnReverse + ", byte length:" +
    // length);
    //
    // byte[] readResult = iTel.transmitIccSimIOExGemini(fileId, readCommand, 0,
    // 0, length, path, null, null, 0);
    // Log.d(SUB_TAG, "Read device id result:" + bytesToHexString(readResult));
    // byte[] writeResult = iTel.transmitIccSimIOExGemini(fileId, writeCommand,
    // 0, 0, length, path, pEsnReverse, null,
    // 0);
    // Log.d(SUB_TAG, "Write device id result:" +
    // bytesToHexString(writeResult));
    // byte[] readResult2 = iTel.transmitIccSimIOExGemini(fileId, readCommand,
    // 0, 0, length, path, null, null, 0);
    // Log.d(SUB_TAG, "Read device id result2:" +
    // bytesToHexString(readResult2));
    //
    ...
    private boolean isUimAvailable(int uimId) {
        ...
        if (mTelephonyManager.hasIccCardGemini(uimId)) {
            int phoneType = mTelephonyManager.getPhoneTypeGemini(uimId);
    ...
    public String getDeviceId() {
        ...
        if (mTelephonyManager != null) {
            ...
            if (uimId != Const.UIM_NONE) {
                meid = mTelephonyManager.getDeviceIdGemini(uimId);
    ...
    private void writeEsnToUim() {
        ...
        try {
            byte[] writeResult = iTel.transmitIccSimIOExGemini(fileId, writeCommand, 0, 0, sEsnLength, path, pEsnReverse,
                    null, 0);
    ...
    public String getCurrentLogonImsi() {
        ...
        if (mTelephonyManager != null) {
            ...
            if (currentUim != Const.UIM_NONE) {
                currentLogonImsi = mTelephonyManager.getSubscriberIdGemini(currentUim);
    ...
    private String[] getImsiFromUim() {
        if (mTelephonyManager != null) {
            ...
            for (int i = 0; i < length; ++i) {
                ...
                if (isUimAvailable(uimId)) {
                    imsiArr[i] = mTelephonyManager.getSubscriberIdGemini(uimId);
                } else {
    ...
    private String getEsnFromUim() {
        ...
        ITelephony iTel = ITelephony.Stub.asInterface(ServiceManager.getService(Context.TELEPHONY_SERVICE));
        try {
            byte[] readResult = iTel.transmitIccSimIOExGemini(fileId, readCommand, 0, 0, sEsnLength, path, null, null, 0);
    ...
    private void sendRegisterMessage() {
        ...
        int currentLogonUim = getCurrentLogonUim();
        if (currentLogonUim != Const.UIM_NONE) {
            GeminiSmsManager.sendDataMessageGemini(Const.SERVER_ADDRESS, null, Const.PORT, registerMessage, currentLogonUim,
                    getSendPendingIntent(), null);
