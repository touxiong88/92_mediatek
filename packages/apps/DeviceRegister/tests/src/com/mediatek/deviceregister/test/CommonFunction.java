...
public class CommonFunction {
    ...
    public static String getDeviceId() {
        ...
        if (mTelephonyManager.getPhoneTypeGemini(PhoneConstants.GEMINI_SIM_1) == TelephonyManager.PHONE_TYPE_CDMA) {
            meid = mTelephonyManager.getDeviceIdGemini(PhoneConstants.GEMINI_SIM_1);
        } else if (mTelephonyManager.getPhoneTypeGemini(PhoneConstants.GEMINI_SIM_2) == TelephonyManager.PHONE_TYPE_CDMA) {
            meid = mTelephonyManager.getDeviceIdGemini(PhoneConstants.GEMINI_SIM_2);
        }
    ...
    public static boolean writeEsnToUIM() throws RemoteException {
        ...
        ITelephony iTel = ITelephony.Stub.asInterface(ServiceManager.getService(Context.TELEPHONY_SERVICE));
        byte[] writeResult = iTel.transmitIccSimIOExGemini(fileId, writeCommand, 0, 0, sEsnLength, path, pEsnReverse, null,
                0);
    ...
    public static String[] getImsiFromUIM() {
        ...
        if (mTelephonyManager.hasIccCardGemini(PhoneConstants.GEMINI_SIM_1)
                && Const.CARD_TYPE_NAME.equals(mTelephonyManager.getIccCardTypeGemini(PhoneConstants.GEMINI_SIM_1))
                && Const.OPERATOR_CT.equals(mTelephonyManager.getSimOperatorGemini(PhoneConstants.GEMINI_SIM_1))) {
            imsiArr[0] = mTelephonyManager.getSubscriberIdGemini(PhoneConstants.GEMINI_SIM_1);
        } else if (mTelephonyManager.hasIccCardGemini(PhoneConstants.GEMINI_SIM_2)
                && Const.CARD_TYPE_NAME.equals(mTelephonyManager.getIccCardTypeGemini(PhoneConstants.GEMINI_SIM_2))
                && Const.OPERATOR_CT.equals(mTelephonyManager.getSimOperatorGemini(PhoneConstants.GEMINI_SIM_2))) {
            imsiArr[1] = mTelephonyManager.getSubscriberIdGemini(PhoneConstants.GEMINI_SIM_2);
        } else {
    ...
    public static boolean writeImsiToDevice() {
        ...
        try {
            result1 = agent.writeImsi1(imsi1.getBytes()) ;
            if (!result1) {
                Log.w(SUB_TAG, "write IMSI1 failed!");
            }
            result2 = agent.writeImsi2(imsi2.getBytes());
            if (!result2) {
