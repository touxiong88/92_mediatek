...
public class ImportContactsActivity extends OOBEStepPreferenceActivity 
    ...
    */
    private void setLayout() {
        ...
        title.setText(mExt.customizeSimDisplayString(getString(R.string.oobe_title_import_contacts), Utils.SLOT_ALL));
        ...
        for (SimInfoRecord siminfo : simList) {
            ...
            try {
                if (PhoneConstants.GEMINI_SIM_NUM == 1) {
                    ...
                    status = mITelephony.getSimIndicatorStateGemini(siminfo.mSimSlotId);
                }
    ...
    */
    private void updateSimInfo() {
        ...
        if (simList != null && mSimInfoPreMap.size() > 0) {
            for (SimInfoRecord simInfo : simList) {
                mSimInfoPreMap.get(simInfo.mSimInfoId).setEnabled(true);
            }
        }
    }
    /*
