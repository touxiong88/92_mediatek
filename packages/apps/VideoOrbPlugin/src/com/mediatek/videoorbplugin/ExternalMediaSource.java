...
//    }
    ...
    public int getMediaCount(int parameter) {
        query();
        if (mCursor != null) {
            mCounts = mCursor.getCount();
            Log.v(TAG, "mCursor.getCount() : " + mCounts);
            // do not limit the result (e.g. 8)
            // the invalid video would be ignored in next step, and limited 8 available ones
            // return mCounts > MAX_VIDEO_CONTENT ? (mCounts = MAX_VIDEO_CONTENT) : mCounts;
            return mCounts;
        }
        return 0;
    }
