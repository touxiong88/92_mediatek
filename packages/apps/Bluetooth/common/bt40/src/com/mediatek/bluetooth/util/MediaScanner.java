...
public class MediaScanner implements MediaScannerConnectionClient {
    ...
    public MediaScanner( Context context, String path, String mimeType, Handler callback, int callbackId ) {
        ...
        //sendbroadcast
        int currentUserId = SystemUtils.getCurrentUserId(context);
        if(currentUserId != 0) {
            Log.d( TAG, "[BT][MMI][MediaScanner]: before sendbroadcast ACTION_MEDIA_SCANNER_SCAN_FILE, userid = " + currentUserId);
            String uriPath = "file://" + path;
            Intent intent = new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.parse(uriPath));
