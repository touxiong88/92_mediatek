...
public class OppNotificationFactory {
    ...
    public static Notification getOppIncomingNotification(Context context, BluetoothShareTask task) {
        ...
        // prepare Intent
        Intent intent = new Intent(BluetoothIntent.OppsAccessRequest.ACTION_PUSH_REQUEST);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK); // required for
                                                        // startActivity()
                                                        // outside the Activity
        intent.putExtra(BluetoothIntent.OppsAccessRequest.EXTRA_PEER_NAME, task.getPeerName());
        intent.putExtra(BluetoothIntent.OppsAccessRequest.EXTRA_OBJECT_NAME, task.getObjectName());
        intent.putExtra(BluetoothIntent.OppsAccessRequest.EXTRA_TOTAL_BYTES, task.getTotalBytes());
        intent.putExtra(BluetoothIntent.OppsAccessRequest.EXTRA_TASK_ID, task.getId());
