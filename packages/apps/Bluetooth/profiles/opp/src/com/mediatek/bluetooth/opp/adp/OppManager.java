...
import com.mediatek.bluetooth.BluetoothIntent;
...
public class OppManager implements Callback {
    ...
    public void oppsStartService() {
        ...
        intent.setAction(BluetoothIntent.OppService.ACTION_OPPS_START);
        ...
        switch (task.getState()) {
            case BluetoothShareTask.STATE_ONGOING:
                if (task.isHandover()) {
                    ...
                    Intent intent = new Intent(BluetoothIntent.ACTION_BT_OPP_TRANSFER_PROGRESS);
                    if (task.getDirection() == Direction.in) {
                        intent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_DIRECTION,
                                BluetoothIntent.DIRECTION_BLUETOOTH_INCOMING);
                    } else {
                        intent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_DIRECTION,
                                BluetoothIntent.DIRECTION_BLUETOOTH_OUTGOING);
                    }
                    intent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_ID, task.getId());
                    intent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_PROGRESS, progress);
                    intent.putExtra(BluetoothIntent.EXTRA_BT_OPP_ADDRESS, task.getPeerAddr());
                    this.mApplicationContext.sendBroadcastAsUser(intent, UserHandle.ALL, BluetoothIntent.HANDOVER_STATUS_PERMISSION);
                } else {
    ...
    private void cancelOngoingNotification(BluetoothShareTask task, int pid) {
        ...
        if (task.isHandover()) {
            ...
            Intent handoverIntent = new Intent(BluetoothIntent.ACTION_BT_OPP_TRANSFER_DONE);
            if (task.getDirection() == Direction.in) {
                handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_DIRECTION,
                        BluetoothIntent.DIRECTION_BLUETOOTH_INCOMING);
            } else {
                handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_DIRECTION,
                        BluetoothIntent.DIRECTION_BLUETOOTH_OUTGOING);
            }
            handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_ID, task.getId());
            handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_ADDRESS, task.getPeerAddr());

            if (BluetoothShareTask.STATE_SUCCESS == task.getState()) {
                handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_STATUS,
                        BluetoothIntent.HANDOVER_TRANSFER_STATUS_SUCCESS);
                handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_URI, task.getData());
                handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_MIMETYPE, task.getMimeType());
            } else {
                handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_STATUS,
                        BluetoothIntent.HANDOVER_TRANSFER_STATUS_FAILURE);
                handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_MIMETYPE, task.getMimeType());
                handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_OBJECT_NAME, task.getObjectName());
                handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_FILE_SIZE, task.getTotalBytes());
                handoverIntent.putExtra(BluetoothIntent.EXTRA_BT_OPP_TRANSFER_DONE_SIZE, task.getDoneBytes());                
            }
            this.mApplicationContext.sendBroadcastAsUser(handoverIntent, UserHandle.ALL, BluetoothIntent.HANDOVER_STATUS_PERMISSION);
        } else {
            this.mNotificationManager.cancel(pid);
        }
    }
    ...
    public class OppcTaskTransferThread extends Thread {
        ...
        public synchronized void oppcHandleTask() {
            ...
            intent.setAction(BluetoothIntent.OppService.ACTION_OPPC_START);
