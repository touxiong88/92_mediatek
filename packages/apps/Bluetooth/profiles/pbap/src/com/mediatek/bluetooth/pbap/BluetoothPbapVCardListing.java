...
public class BluetoothPbapVCardListing {
    ...
    private int mSimSearchAttrib = 0;

    private String mSimSearchValue = null;
    ...
    public int list(int type, int order, int searchAttr, String searchVal, int listOffset,
        ...
        switch (type) {
            ...
            case BluetoothPbapPath.FOLDER_TYPE_SIM1_PB:
                ...
                if (mSimDirty || !isSameSimAttribute(searchAttr, searchVal)) {
                    mSimSearchAttrib = searchAttr;
                    mSimSearchValue = searchVal;
    ...
    private int listSimPb(boolean relist, int listOffset, int maxListCount, ArrayList<String> result) {
        ...
        ContentResolver resolver = mContext.getContentResolver();
        Cursor cursor = null;
        boolean ownerInc = (TextUtils.isEmpty(mSimSearchValue));
        printLog("[API] listSimPb(" + String.valueOf(relist) + "," + listOffset + ","
                + maxListCount + ")");
        printLog("SearchVal=" + mSimSearchValue + ", mSimSearchAttrib=" + mSimSearchAttrib);
        try {
            if (resolver == null) {
                errorLog("[ERR] resolver is null");
                return ResponseCodes.OBEX_HTTP_INTERNAL_ERROR;
            ...
            if (relist) {
                ...
                mSimIDList = null;
                if (!TextUtils.isEmpty(mSimSearchValue)) {
                    if (mSimSearchAttrib != VCARD_SEARCH_NUMBER) {
                        uri = Uri.withAppendedPath(Contacts.CONTENT_FILTER_URI, Uri
                                .encode(mSimSearchValue));
                        projection = PB_PROJECTION;
                    } else {
                        uri = Phone.CONTENT_URI;
                        projection = PB_NUMBER_PROJECTION;
                        selection += " AND " + Phone.NUMBER + " LIKE '%" + mSimSearchValue + "%'";
                    }
                } else {
                    uri = Contacts.CONTENT_URI;
                    projection = PB_PROJECTION;
...
//        }
    ...
    private boolean isSameSimAttribute(int searchAttr, String searchVal) {

        boolean b = true;

        if (!TextUtils.isEmpty(searchVal) || !TextUtils.isEmpty(mSimSearchValue)) {
            if ((searchAttr != mSimSearchAttrib) || !TextUtils.equals(searchVal, mSimSearchValue)) {
                b = false;
            }
        }
        return b;
