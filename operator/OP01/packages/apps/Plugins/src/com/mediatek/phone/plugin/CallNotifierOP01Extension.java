...
public class CallNotifierOP01Extension  extends CallNotifierExtension {

    private static final String LOG_TAG = "CallNotifierOP01Extension";
    ...
    public void onPhoneStateChanged(CallManager cm,Context context) {
        if ((cm.getActiveFgCallState() == Call.State.ACTIVE)
            ...
            if(!powerManager.isScreenOn()) {
                log("plugin pulse screen lock");
                powerManager.wakeUp(SystemClock.uptimeMillis());
            }
        }
        mPreviousCallState = cm.getActiveFgCallState();
    }
