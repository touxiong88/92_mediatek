...
public class ContactBackupComposer extends Composer {
    ...
    private String getCondition() {
        if (mParams != null) {
            ...
            List<SimInfoManager.SimInfoRecord> simInfoList = SimInfoManager.getInsertedSimInfoList(mContext);
            if (simInfoList != null && simInfoList.size() > 0) {
                for (SimInfoManager.SimInfoRecord simInfo : simInfoList) {
                    if (mParams.contains(Long.toString(simInfo.mSimSlotId))) {
            ...
            if (len > 0) {
                ...
                MyLogger.logD(CLASS_TAG, "condition" + condition.toString());
