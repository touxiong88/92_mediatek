...
static void mag_work_func(struct work_struct *work)
{
	...
	{
	    if(1 == atomic_read(&cxt->early_suspend))
	    {
	       // slow down polling rate at early suspend  let system have chance to sleep
	       mod_timer(&cxt->timer, jiffies + (HZ/7));
		   MAG_LOG("mag_dev early suspend work polling\n");
	    }
		else
		{
