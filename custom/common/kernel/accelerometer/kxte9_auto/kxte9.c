...
static int kxte9_i2c_detect(struct i2c_client *client, struct i2c_board_info *info);

static int kxte9_local_init(void);
static int kxte9_remove(void);

static int kxte9_init_flag =0; // 0<==>OK -1 <==> fail
...
/*----------------------------------------------------------------------------*/
static struct sensor_init_info kxte9_init_info = {
		.name = "kxte9",
		.init = kxte9_local_init,
		.uninit = kxte9_remove,
...
static int kxte9_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	new_client = client;
	data = 0x0f;
	if((i2c_master_send(new_client,&data, 1))<0)
	{
		GSE_ERR("I2C transfer error, line: %d\n", __LINE__);
		kxte9_init_flag = -1;
		return -1;
	}

	data = 0xFF;
	if((i2c_master_recv(new_client,&data, 1))<0)
	{
		GSE_ERR("I2C transfer error, line: %d\n", __LINE__);
		kxte9_init_flag = -1;
		return -1;
	...
	if(data != 0x00)
	{
		GSE_ERR("KXTE9 Read ID error, line: %d\n", __LINE__);
		kxte9_init_flag = -1;
		return -1;
	}
    ...
    kxte9_i2c_client->addr = (kxte9_i2c_client->addr & I2C_MASK_FLAG) | I2C_ENEXT_FLAG;
	...
	if(err = kxte9_create_attr(&(kxte9_init_info.platform_diver_addr->driver)))
	{
...
static int kxte9_i2c_remove(struct i2c_client *client)
{
	...
	if(err = kxte9_delete_attr(&(kxte9_init_info.platform_diver_addr->driver)))
	{
...
static int  kxte9_local_init(void)
{
   struct acc_hw *hw = kxte9_get_cust_acc_hw();
	GSE_FUN();
...
//	kxte9_force[0] = hw->i2c_num;
	...
	if(-1 == kxte9_init_flag)
	{
		GSE_ERR("kxte9_local_init fail.\n");
		i2c_del_driver(&kxte9_i2c_driver);
		return -1;
	}
...
{
	GSE_FUN();
	i2c_register_board_info(2, &i2c_kxte9, 1);
//	if(platform_driver_register(&kxte9_gsensor_driver))
//	{
//		GSE_ERR("failed to register driver");
//		return -ENODEV;
//	}
	hwmsen_gsensor_add(&kxte9_init_info);
	return 0;    
}
/*----------------------------------------------------------------------------*/
