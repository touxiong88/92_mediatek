...
//re_read:
	...
	{
		IMM_GetOneChannelValue(0, data, adc);
		sum+=(data[0]*100+data[1]);
	}
	adc_vol = sum/10;
	#if defined(BUILD_LK)
	printf("sunxing test adc_vol is %d\n",adc_vol);
	#else
	printk("sunxing test adc_vol is %d\n",adc_vol);
	#endif
	if(adc_vol<30)
		return 0;
...
static void lcm_get_params(LCM_PARAMS * params)
{
	...
	params->dsi.fbk_div = 17;	//12;    // fref=26MHz, fvco=fref*(fbk_div+1)*2/(div1_real*div2_real)
...
// ---------------------------------------------------------------------------
LCM_DRIVER otm1283a_cmi50_ykl_hd_lcm_drv= {
	.name = "otm1283a_cmi50_ykl_hd",
