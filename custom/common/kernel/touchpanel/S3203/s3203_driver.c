...
#ifdef TPD_DCLICK_WAKEUP
static int _is_double_click_wakup_enable = 0;
#endif
...
static unsigned int tp_id = 0;

#ifdef TPD_HAVE_BUTTON 
static int tpd_keys_local[TPD_KEY_COUNT] = TPD_KEYS;
static int tpd_keys_dim_local[TPD_KEY_COUNT][4] = TPD_KEYS_DIM;
u8 tpd_res_2d[4]={(u8)CUSTOM_MAX_WIDTH, (u8)(CUSTOM_MAX_WIDTH>>8),\
			(u8)(CUSTOM_MAX_HEIGHT + TPD_BUTTON_HEIGH), (u8)((CUSTOM_MAX_HEIGHT + TPD_BUTTON_HEIGH)>>8)};
u8 tpd_res_0d[4]={(u8)CUSTOM_MAX_WIDTH, (u8)(CUSTOM_MAX_WIDTH>>8),\
			(u8)(CUSTOM_MAX_HEIGHT), (u8)((CUSTOM_MAX_HEIGHT)>>8)};   //3202 0Dbutton
#endif
...
#endif
#ifdef TPD_KEY_SUPPORT
static const u16 touch_key_array[] = { KEY_MENU, KEY_HOMEPAGE, KEY_BACK, KEY_SEARCH };
#define CTP_MAX_KEY_NUM ( sizeof( touch_key_array )/sizeof( touch_key_array[0] ) )
#endif
...
static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO("S3203", (0x20))};
...
#define TPD_KEY_REG_PAGE 0x02
#define TPD_KEY_REG 0x00
#define PAGE_LEN 2

static int s3203_set_page(unsigned int page)
{
...
{   
	...
	u8 data5 = 0x00;	
	u8 data6 = 0x00;	    
	if(_is_double_click_wakup_enable)
	{
		tpd_i2c_read_data(ts->client, 0x4B, &data6, 1);
		printk("%s data6=[%x]\n", __func__,data6);
		if (data6&0x01==0x01)
		{
			...
			input_sync(tpd->dev);
		tpd_i2c_write_byte_data(ts->client, fd_01.commandBase, 0x01);/*touchscreen reset*/
        msleep(200);
		}		
	}
#endif
...
#ifdef TPD_KEY_SUPPORT
	tpd_i2c_read_data(ts->client, 0x56, &data7, 1);
	//read key value start
	data8= data7&0x07;
	if(data8==0x00)
	{
		s3203_set_page(0x02);//Page 2
		tpd_i2c_read_data(ts->client, TPD_KEY_REG, &key_value, 1);
		s3203_set_page(0x00);//Page 0
		//printk("%s key_value=[%x]\n", __func__, key_value);
		//read key value end
	}
#endif
...
static int tpd_power(struct i2c_client *client, int on)
{
    ...
    {       
       ...
       tpd_i2c_read_data(ts->client, TPD_KEY_PRODUCT_ADDR, &tp_id, 1);//add by lichengcheng    
          
	//send custom resolution
	for(i = 0 ; i < 4; i++)
	{
		if(tp_id == 49)
			ret = tpd_i2c_write_byte_data(client, (fd_11.controlBase + 6 + i), tpd_res_0d[i]);
		else
		  ret = tpd_i2c_write_byte_data(client, (fd_11.controlBase + 6 + i), tpd_res_2d[i]); /* set touchscreen x/y resolution*/
		if (ret < 0)
        	{
...
#ifdef TPD_DCLICK_WAKEUP
static ssize_t gtp_dclick_wakeup_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	ssize_t num_read_chars = 0;

	num_read_chars = snprintf(buf, PAGE_SIZE, "%d\n", _is_double_click_wakup_enable);

	return num_read_chars;
}

static ssize_t gtp_dclick_wakeup_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
    u8 wr_buf[2];
	...
	if(buf[0] == '1' && _is_double_click_wakup_enable == 0){
		_is_double_click_wakup_enable = 1;
	}
	if(buf[0] == '0' && _is_double_click_wakup_enable == 1){
		_is_double_click_wakup_enable = 0;
	}
...
static DEVICE_ATTR(dclick_en, 0777, gtp_dclick_wakeup_show, gtp_dclick_wakeup_store);

static struct attribute *sn3203_attributes[] = {
	&dev_attr_dclick_en.attr,
    NULL
};

static struct attribute_group sn3203_attribute_group = {
    .attrs = sn3203_attributes
};
#endif
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hwPowerOn(MT6322_POWER_LDO_VGP3, VOL_1800, "TP");
...
#ifdef TPD_KEY_SUPPORT
	for (idx = 0; idx < CTP_MAX_KEY_NUM; idx++)
	{
    ...
    if ((ret = sysfs_create_group(&client->dev.kobj, &sn3203_attribute_group))) {
...
static int tpd_local_init(void)
{
	...
	if(i2c_add_driver(&tpd_i2c_driver)!=0)
	{
        ...
        {
            TPD_DMESG("s3203 add error touch panel driver.\n");
...
{ 
	...
	if(_is_double_click_wakup_enable)
	{	
		tpd_i2c_read_data(ts->client, 0x56, &data2, 1);
		data3 = data2 & 0xF8;
		TPD_DEBUG("TPD wake up\n");
		tpd_up(0,0);
		tpd_i2c_write_byte_data(ts->client, 0x56, data3);
		return;
	}
#endif	
...
static void tpd_suspend(struct early_suspend *h)
{
	...
	{
		//mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
		tpd_i2c_write_byte_data(ts->client, 0x56, data1);
		//tpd_power(ts->client, 0);
//		tpd_i2c_read_data(ts->client, 0x56, &data1, 1);
//		printk("%s data1=[%x]\n", __func__,data1);
