...
#if GTP_HIGH_SENSITIVITY_SUPPORT
static int _is_in_high_sensitivity_mode = 0;

#if GTP_HIGH_SENSITIVITY_SEND_CFG
s32 gtp_high_sensitivity_send_cfg(struct i2c_client *client)
{
    s32 ret = 1;
    ...
    u8 switch_config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
            = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
    
    u8 default_config[] = CTP_CFG_DEFAULT;
    u8 hi_config[] = CTP_CFG_HS;
    
    memset(&switch_config[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);

    if(_is_in_high_sensitivity_mode)
    {
        cfg_len = CFG_GROUP_LEN(hi_config);
        memcpy(&switch_config[GTP_ADDR_LENGTH], hi_config, cfg_len);
    }
    else
    {
        cfg_len = CFG_GROUP_LEN(default_config);
        memcpy(&switch_config[GTP_ADDR_LENGTH], default_config, cfg_len);
    }

    printk("xcz enter gtp_high_sensitivity_send_cfg, mode: %d\n", _is_in_high_sensitivity_mode); //xcz

    GTP_INFO("Driver Send Config");
    for (retry = 0; retry < 5; retry++)
    {
        ret = gtp_i2c_write(client, switch_config, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);

        if (ret > 0)
        {
            printk("Derive send config success! line %d",__LINE__);
            break;
        }
    }
...
static ssize_t gt9xx_highsensitivity_show(struct device *dev,struct device_attribute *attr,char *buf)
{
...
static ssize_t gt9xx_highsensitivity_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
    u8 wr_buf[2];
    ...
    #if GTP_HIGH_SENSITIVITY_SEND_CFG
        gtp_high_sensitivity_send_cfg(i2c_client_point);
    #else        
        wr_buf[0]=0x0B;
        i2c_write_bytes(i2c_client_point, 0x8046, wr_buf, 1);
        i2c_write_bytes(i2c_client_point, 0x8040, wr_buf, 1);
        msleep(5);
        wr_buf[0]=0x03;
        //i2c_write_bytes(i2c_client_point, 0x8046, wr_buf, 1);
        i2c_write_bytes(i2c_client_point, 0x8040, wr_buf, 1);
...
/* Vanzo:wangyi on: Wed, 02 Apr 2014 16:57:33 +0800
 * #73940, modify permissions
static DEVICE_ATTR(highsensitivity, S_IRUGO | S_IWUSR, gt9xx_highsensitivity_show, gt9xx_highsensitivity_store);
 */
static DEVICE_ATTR(highsensitivity, 0777, gt9xx_highsensitivity_show, gt9xx_highsensitivity_store);
// End of Vanzo: wangyi
#endif

#if GTP_GLOVE_MODE_SUPPORT 
static int _is_in_glove_mode = 0;

static ssize_t gt9xx_glove_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	ssize_t num_read_chars = 0;
...
static ssize_t gt9xx_glove_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
    u8 wr_buf[2];
	...
	if(buf[0] == '1' && _is_in_glove_mode == 0){
		_is_in_glove_mode = 1;
        wr_buf[0]=0x0B;
        i2c_write_bytes(i2c_client_point, 0x8046, wr_buf, 1);
        i2c_write_bytes(i2c_client_point, 0x8040, wr_buf, 1);
	...
	if(buf[0] == '0' && _is_in_glove_mode == 1){
		_is_in_glove_mode = 0;
        wr_buf[0]=0x0A;
        i2c_write_bytes(i2c_client_point, 0x8046, wr_buf, 1);
        i2c_write_bytes(i2c_client_point, 0x8040, wr_buf, 1);
...
static DEVICE_ATTR(glove, 0777, gt9xx_glove_show, gt9xx_glove_store);
#endif
#if GTP_GESTURE_WAKEUP
...
static ssize_t gt9xx_fwversion_show(struct device *dev,struct device_attribute *attr,char *buf)
{
...
#else
    u8 reg[3] = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
	...
	num_read_chars = snprintf(buf, PAGE_SIZE, "FW VERSION: %d\n",reg[2]);
#endif
	return num_read_chars;
}
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (!int_type)
    {
		...
		mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINTF_TRIGGER_RISING, tpd_eint_interrupt_handler, 1);
    }
    else
    {
...
static int touch_event_handler(void *unused)
{
    ...
    do
    {
        ...
        if (DOZE_ENABLED == doze_status && tpd_halt == 1)
        {
            ...
            if (ret > 0)
            {               
                //printk("YangZhihong %s line %d doze_buf[2] %d\n",__func__,__LINE__,doze_buf[2]);    
                isValuedKey = TRUE;
...
static void tpd_resume(struct early_suspend *h)
{
...
#if GTP_GLOVE_MODE_SUPPORT
    {
        ...
        if(_is_in_glove_mode)
        {
            ...
            {
                wr_buf1[0]=0x0B;
                i2c_write_bytes(i2c_client_point, 0x8046, wr_buf1, 1);
                i2c_write_bytes(i2c_client_point, 0x8040, wr_buf1, 1);
                msleep(5);
                wr_buf1[0]=0x03;
                //i2c_write_bytes(i2c_client_point, 0x8046, wr_buf, 1);
                i2c_write_bytes(i2c_client_point, 0x8040, wr_buf1, 1);
