...
static int ft5x0x_read_Touchdata(void)
{
   ...
   {
       coordinate_x[i] = ((((s16) buf[0 + (4 * i)]) & 0x0F) << 8 | (((s16) buf[1 + (4 * i)])& 0xFF));//X靠
       coordinate_y[i] = ((((s16) buf[2 + (4 * i)]) & 0x0F) << 8 | (((s16) buf[3 + (4 * i)]) & 0xFF));//Y靠
...
//sunhouzan 2014-10-13	TP无反应,重新上电复位
	mutex_lock(&i2c_access);
	reg = 0xa8;
	fts_i2c_Read(i2c_client, &reg, 1, data, 1);
	mutex_unlock(&i2c_access);
	TPD_DMESG("ft5306 reg 0xa8 = %x\n",data[0]);
	
	if(0x5f != data[0])	//模组厂ID 编号为0x5f
	{
		TPD_DMESG("ft5306 reg_a8= %x tp will reset\n",data[0]);
