...
static unsigned short force[] = {1,0x80,I2C_CLIENT_END,I2C_CLIENT_END};
...
static void gsl_report_point(struct gsl_touch_info *ti)
{
	...
	else 
	{
		...
		for (tmp = 0; ti->finger_num > tmp; tmp++) 
		{
			...
			if(boot_mode!=NORMAL_BOOT && (strncmp(MTK_LCM_PHYSICAL_ROTATION, "270", 3)==0 
				|| strncmp(MTK_LCM_PHYSICAL_ROTATION, "90", 3)==0) )
			{
				int temp;
				temp = ti->y[tmp];
				ti->y[tmp] = ti->x[tmp];
...
static void gsl_report_work(void)
{
	...
	if(gsl_halt_flag == 1 && gsl_gesture_flag == 1){
		...
		printk("%s line %d tmp_c = %d\n",__func__,__LINE__,tmp_c);
		switch(tmp_c){
		case (int)'C':
			keyValue = KEY_C;
			break;			
		...
		*/
		...
		printk("%s line %d flag %d gsl_lcd_flag %d\n",__func__,__LINE__, flag, gsl_lcd_flag);
		gsl_gesture_c = (char)(tmp_c & 0xff);
		if(flag==1){
			//if(gsl_lcd_flag == 0){
				printk("%s line %d keyValue %d\n",__func__,__LINE__, keyValue);
				input_report_key(gsl_power_idev,keyValue,1);
				input_sync(gsl_power_idev);
				input_report_key(gsl_power_idev,keyValue,0);
				input_sync(gsl_power_idev);
			//}
			gsl_lcd_flag = 1;			
		}
...
#ifdef MT6592
#if defined(DCT_K8)
	hwPowerOn(MT6322_POWER_LDO_VGP2, VOL_2800, "TP");
#endif
#endif
	/* reset ctp gsl1680 */
...
#define GSL_C		100
#define GSL_CHIP_1	0xff804003  //sanfengda 
#define GSL_CHIP_2	0xff801003  //xingzhen
#define GSL_CHIP_3	0xff800c00
...
static ssize_t gsl_sysfs_tpgesturet_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(buf[0] == '1' && gsl_gesture_flag == 0){
		gsl_gesture_flag = 1;
	}
	if(buf[0] == '0' && gsl_gesture_flag == 1){
		gsl_gesture_flag = 0;
	}
...
//static DEVICE_ATTR(gesture, S_IRUGO|S_IWUSR, gsl_sysfs_tpgesture_show, gsl_sysfs_tpgesturet_store);
...
static void gsl_request_power_idev(void)
{
	...
	{
		input_set_capability(idev, EV_KEY, tpd_gestrue_keys[i]);
	}
...
static int __devinit gsl_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = sysfs_create_file(&client->dev.kobj, &dev_attr_gesture.attr);
    if (ret)
    {
        printk("%s: sysfs_create_version_file failed\n", __func__);
        return ret;
...
#endif
#ifdef GSL_GESTURE
	print_info("suspend:gsl_gesture_flag = %d\n",gsl_gesture_flag);
	 if(gsl_gesture_flag == 1){
		u8 buf[4];
		unsigned int temp;
		gsl_lcd_flag = 0;
		...
		//gsl_load_fw();
		gsl_load_fw(ddata->client,GSLX68X_FW_GESTURE,temp);
		gsl_start_core(ddata->client);
	#endif
		msleep(1000);
		buf[0] = 0xa;
...
static void gsl_resume(struct i2c_client *client)
{
    print_info();
#ifdef GSL_GESTURE
	print_info("resume:gsl_gesture_flag = %d\n",gsl_gesture_flag);
	if(gsl_gesture_flag == 1){
		u8 buf[4];
		unsigned int temp;
		mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ZERO);
