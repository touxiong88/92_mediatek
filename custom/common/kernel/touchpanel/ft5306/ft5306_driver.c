...
struct Upgrade_Info fts_updateinfo[] =
{
    ...
    {FT5X36_UPGRADE_AA_DELAY, FT5X36_UPGRADE_55_DELAY, FT5X36_UPGRADE_ID_1, FT5X36_UPGRADE_ID_2, FT5X36_UPGRADE_READID_DELAY, FT5X36_UPGRADE_EARSE_DELAY},
};
...
int fts_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf,u32 dw_lenth)
{
	...
	//printk("[CCI TP DEBUG] %s chip_id = %x\n", __func__, chip_id);
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if(chip_curr == FTS_FT6206){
			msleep(upgradeinfo.delay_55+(i+1)*5);
		}else{
...
#ifdef TP_GLOVE_MODE_SUPPORT
static ssize_t ft5x0x_glove_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    ssize_t num_read_chars = 0;
...
/*upgrade from app.bin*/
static ssize_t ft5x0x_glove_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
    if(count == 0)
        return;
    ...
    if(buf[0] == '1' && _is_in_glove_mode == 0){
        ft5x0x_write_reg(i2c_client, 0xC0, 0x01);
        _is_in_glove_mode = 1;
    }
    if(buf[0] == '0' && _is_in_glove_mode == 1){
        ft5x0x_write_reg(i2c_client, 0xC0, 0x0);
        _is_in_glove_mode = 0;
    }
...
static DEVICE_ATTR(glove, S_IRUGO | S_IWUSR, ft5x0x_glove_show, ft5x0x_glove_store);
#endif
...
/* Vanzo:maxiaojun on: Wed, 03 Jul 2013 21:53:49 +0800
 * firmware auto upgrade
 */
extern BOOTMODE get_boot_mode(void);
static unsigned char UPG_PBT_BUF[]=
{
#include "auto_upg_data.i"
};
static void fts_ctpm_auto_upg(void)
{
    u8 fwver = 0;
    u8 new_fwver = 0;
    int fwsize;
    ...
    if(boot_mode != NORMAL_BOOT)
        return;
    fwsize = sizeof(UPG_PBT_BUF);
    new_fwver = UPG_PBT_BUF[fwsize-2];
    ...
    if (fts_ctpm_fw_upgrade(i2c_client, UPG_PBT_BUF, fwsize) != 0) {
        dev_dbg(&i2c_client->dev, "%s() - ERROR:[FTS] upgrade failed..\n",__func__);
    } else {
        //#ifdef AUTO_CLB
        fts_ctpm_auto_clb(i2c_client);      /*start auto CLB*/
        //#endif
    }
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
    ...
    if(PS_STATUS)
    {
        i2c_smbus_read_i2c_block_data(i2c_client, 0xb0, 1, &psregister);
        if(!psregister)
        {
            buffer[0] = 0xb0;
            buffer[1] = 0x01;
...

{
    ...
    /* Vanzo:maxiaojun on: Tue, 26 Nov 2013 15:30:28 +0800
     * board device name support.
     */
    {
        extern void v_set_dev_name(int id, char *name);
        v_set_dev_name(2, "ft5306");
    }
    // End of Vanzo:maxiaojun
    return 0;
}
