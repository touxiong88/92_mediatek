...
#ifdef TOUCH_PS
#define TOUCH_PS_VALID_REG                   0x814E
#define TOUCH_PS_ENABLE_REG                  0x8042
...
static s32 gt9xx_ps_enable(s32 enable)
{
    u8  state;
    s32 ret = -1;
    ...
    ret = i2c_write_bytes(i2c_client_point, TOUCH_PS_ENABLE_REG, &state, 1);
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    u8 *send_cfg_buf[6] = {cfg_info_group0, cfg_info_group1, cfg_info_group2, cfg_info_group3, cfg_info_group4, cfg_info_group5};
	u8 cfg_info_len[6] = {sizeof(cfg_info_group0) / sizeof(cfg_info_group0[0]),
...
static void force_reset_guitar(void)
{
    ...
    //Power off TP
#if 1//(defined(MT6575) || defined(mt6577))
    msleep(30);
    //Power on TP
#ifdef GPIO_CTP_EN_PIN
	mt_set_gpio_mode(GPIO_CTP_EN_PIN, GPIO_CTP_EN_PIN_M_GPIO);                                                                                                        
...
static int touch_event_handler(void *unused)
{
    ...
    do
    {
        ...
        if (tpd_proximity_flag == 1)
        {
            ...
            printk("REG INDEX[0x814E]:0x%02X\n", proximity_status);
