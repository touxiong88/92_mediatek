...
#include "coolpad_7296_eachopto.h"
#include "coolpad_7296_ofilm.h"
int synaptics_fw_updater(int Product_ID)//add by liyongfeng
{
    int retval;
    printk("inter synaptics_fw_updater\n");
    if(Product_ID==1)
    {
     fwu->ext_data_source = synaImage_ofilm;
     fwu->config_area = UI_CONFIG_AREA;
     retval = fwu_start_reflash();
    }
    if(Product_ID==2)
    {
     fwu->ext_data_source = synaImage_eachopto;
     fwu->config_area = UI_CONFIG_AREA;
     retval = fwu_start_reflash();
    }
    return retval;
...
//add by liongfeng start 
//device firmware id
fwu->fn_ptr->read(fwu->rmi4_data,
				fwu->f01_fd.query_base_addr+11+5,
				firmware_id,
				10);
firmware_id[10]=0;
printk("synaptics_needfw_updater firmware_id=%s\n",firmware_id);
fwu->fn_ptr->read(fwu->rmi4_data,fwu->f34_fd.ctrl_base_addr,config_id,4);
printk("config_id[1]=%x,config_id[3]=%x\n",config_id[1],config_id[3]);
//if(firmware_id[3]==0x61&&firmware_id[4]==0x35)//truly
if(firmware_id[3]==0x61&&firmware_id[4]==0x30)//ofilm:0xa0
{
     if(synaImage_ofilm[0xb100+3]>config_id[3])
      return 1;
     else return 0;
     
}
//if(firmware_id[3]==0x61&&firmware_id[4]==0x63)//biel
if(firmware_id[3]==0x61&&firmware_id[4]==0x31)//eachopto:0xa1
{
     if(synaImage_eachopto[0xb100+3]>config_id[3])
      return 1;
     else return 0;
}
...
return 0;
#if 0
	//fwu->ext_data_source = fw_data;//del by liyongfeng
	//fwu->config_area = UI_CONFIG_AREA;//del by liyongfeng
	////////////////////////////////////////////////////////////////////////////
	...
	device_configID = ConvertConfigID(config_id);
        tpd_i2c_read_data(ts->client, fwu->f34_fd.ctrl_base_addr, &data[0],4);
        //tpd_i2c_read_data(ts->client, 0x37,               &data[0],4);//f34.ctrl_base
        if(data[1]==1)
	image_configID = ConvertConfigID(synaImage+0xb100);
        if(data[1]==2)
        image_configID = ConvertConfigID(synaImage+0xb100);
    
        printk("synaptics device_configID ==%x image_configID==%x\n",device_configID,image_configID);
	if(image_configID > device_configID){
	   //retval = fwu_start_reflash();//del by liyongfeng
	   //return retval;
           mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
           return 1;//del by liyongfeng
	}
	////////////////////////////////////////////////////////////////////////////
         mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
	return 0;
...
EXPORT_SYMBOL(synaptics_fw_needupdater);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	//retval = synaptics_fw_updater(fwu->ext_data_source);//modif by liyongfeng
...
/*	list_for_each_entry(fhandler, &rmi->support_fn_list, link) {
 ...
 int synaptics_rmi4_fwu_init(struct i2c_client *client)
{
	int retval;
