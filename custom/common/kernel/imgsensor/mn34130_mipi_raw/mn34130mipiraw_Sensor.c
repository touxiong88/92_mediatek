...
#define MN34130_RG_TYPICAL_VALUE    0x1D3
#define MN34130_BG_TYPICAL_VALUE    0x1f2

static kal_uint16 mn34130_otp_gain_to_reg[37][2] =
{
	{1000, 0x100},
	{1020, 0x101},
	{1030, 0x102},
...
kal_uint16 MN34130_calc_reg(kal_uint16 gain)
{
	kal_uint16 i, length;
	...
	for (i=0; i<length; i++)
	{
		if (gain < mn34130_otp_gain_to_reg[i][0])
		{
			break;
		}
	}

	return mn34130_otp_gain_to_reg[i-1][1];
}

void MN34130_calc_gain(kal_uint16 goldenRG, kal_uint16 goldenBG, kal_uint16 currentRG, kal_uint16 currentBG,
					   kal_uint16 *R_gain, kal_uint16 *G_gain, kal_uint16 *B_gain)
{
	// Calculate R, G, B gain of current module
	kal_uint32 ratioR;
	kal_uint32 ratioB;
	kal_uint32 ratioG;
	kal_uint32 ratioBase;

	ratioR = 1000 * goldenRG / currentRG;
	ratioB = 1000 * goldenBG / currentBG;
	ratioG = 1000;
	...
	ratioR = 1000 * ratioR / ratioBase;
	ratioG = 1000 * ratioG / ratioBase;
	ratioB = 1000 * ratioB / ratioBase;
...
void MN34130_update_awb(void)
{
	kal_uint16 rg_ratio;
	kal_uint16 bg_ratio;
	kal_uint16 R_gain, G_gain, B_gain;

	kal_uint16 reg_l, reg_h;

	reg_l = MN34130_read_cmos_sensor(0x35D6);
	reg_h = MN34130_read_cmos_sensor(0x35D7);
	rg_ratio = (reg_h << 8) | reg_l;

	reg_l = MN34130_read_cmos_sensor(0x35D8);
	reg_h = MN34130_read_cmos_sensor(0x35D9);
	bg_ratio = (reg_h << 8) | reg_l;

	MN34130DB("rg_ratio:0x%x, bg_ratio:0x%x", rg_ratio, bg_ratio);

	/* update digital gain */
	MN34130_calc_gain(MN34130_RG_TYPICAL_VALUE, MN34130_BG_TYPICAL_VALUE,
		rg_ratio, bg_ratio, &R_gain, &G_gain, &B_gain);

	MN34130DB("R_gain:%d, B_gain:%d, G_gain:%d", R_gain, B_gain, G_gain);

	R_gain = MN34130_calc_reg(R_gain);
	B_gain = MN34130_calc_reg(B_gain);
	G_gain = MN34130_calc_reg(G_gain);

	MN34130DB("regval R_gain:0x%x, B_gain:0x%x, G_gain:0x%x", R_gain, B_gain, G_gain);

	//gr
	MN34130_write_cmos_sensor(0x020E, G_gain >> 8);
	MN34130_write_cmos_sensor(0x020F, G_gain & 0xff);

	//r
	MN34130_write_cmos_sensor(0x0210, R_gain >> 8);
	MN34130_write_cmos_sensor(0x0211, R_gain & 0xff);

	//b
	MN34130_write_cmos_sensor(0x0212, B_gain >> 8);
	MN34130_write_cmos_sensor(0x0213, B_gain & 0xff);

	//gb
	MN34130_write_cmos_sensor(0x0214, G_gain >> 8);
	MN34130_write_cmos_sensor(0x0215, G_gain & 0xff);
}

void MN34130_update_otp(void)
{
	kal_uint16 reg_val;

	reg_val = MN34130_read_cmos_sensor(0x35FE);

	if (0x5F != reg_val)
	{
		MN34130_write_cmos_sensor(0x3045, 0x00);
		MN34130DB("failed read otp 0x%x, disable LSC", reg_val);
		return;
	}
