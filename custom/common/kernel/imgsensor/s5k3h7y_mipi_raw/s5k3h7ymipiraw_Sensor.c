...
#define S5K3H7_TEST_PATTERN_CHECKSUM (0xdc0c1113)
...
static inline void SetFramelength(kal_uint16 u2Framelength)
{
	...
	S5K3H7Y_bytewrite_cmos_sensor(0x0104, 0x00);	 //Grouped parameter release	
	spin_lock(&s5k3h7ymipiraw_drv_lock);
	S5K3H7Y_FRAME_LENGTH_COUNT = u2Framelength;
	spin_unlock(&s5k3h7ymipiraw_drv_lock);
}
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:  
            pSensorInfo->SensorGrabStartX = S5K3H7Y_PV_X_START+1;
            pSensorInfo->SensorGrabStartY = S5K3H7Y_PV_Y_START+1;
		break;
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:         
			pSensorInfo->SensorGrabStartX = S5K3H7Y_VIDEO_X_START+1;
			pSensorInfo->SensorGrabStartY = S5K3H7Y_VIDEO_Y_START+1;     
        break;
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		case MSDK_SCENARIO_ID_CAMERA_ZSD:
            pSensorInfo->SensorGrabStartX = S5K3H7Y_FULL_X_START+1;	//2*S5K3H7Y_IMAGE_SENSOR_PV_STARTX;
            pSensorInfo->SensorGrabStartY = S5K3H7Y_FULL_Y_START+1;	//2*S5K3H7Y_IMAGE_SENSOR_PV_STARTY;           
        break;
        default:
            pSensorInfo->SensorGrabStartX = S5K3H7Y_PV_X_START+1;
            pSensorInfo->SensorGrabStartY = S5K3H7Y_PV_Y_START+1;
