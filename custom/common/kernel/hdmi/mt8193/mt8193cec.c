...
#define CLR_CEC_IRQ() 	    (mt8193_write(0x1504,0x20))//(mt65xx_eint_unmask(13))
...
u8 _CEC_TX_Enqueue(CEC_FRAME_DESCRIPTION* frame)
{
	 	...
	 	if (frame->size ==1)
	 	{
	 		MT8193_CEC_LOG("[CEC] Polling LA = 0x%x \n" , (u8)((frame->blocks.header.initiator << 4) | frame->blocks.header.destination));
	 	}
		else
		{
 ...
 u8 mt8193_cec_isrprocess(u8 u1rxmode)
{
	...
	if (IsCECStatus(STATE_TXING_FRAME))
	{
		...
		if (IS_INT_LOW())
		{
			...
			if (!IS_INT_RB_RDY())
			{
				MT8193_CEC_LOG( "[CEC] FW is slow to trigger the following blocks\n");
