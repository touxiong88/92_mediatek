...
static size_t breaghlight_show_rgb(struct device *dev,
				    struct device_attribute *attr,
				    char *buf)
{
	BL_DBG();
...
static size_t breathlight_store_rgb(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t size)
{
...
static DEVICE_ATTR(rgb, 0666, breaghlight_show_rgb,
		   breathlight_store_rgb);
