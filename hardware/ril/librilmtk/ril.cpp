...
static int handleSpecialRequestWithArgs(int argCount, char** args){
    ...
    if (1 == argCount)
    {
        ...
        if(strcmp(cmd, "EAP_SIM") == 0){
            ...
            int ret = send(s_fdDebug_command, &len, sizeof(int), 0);
            LOGD("AT+ERFTX=1, fdDebug:%d", s_fdDebug_command);
            if (ret != sizeof(int)) {
                LOGD("Socket write Error: when sending arg length");
            } else {                
                ret = send(s_fdDebug_command, result, len, 0);
                if (ret != len) {
