...
extern int max_pdp_support_23G;
    ...
    if (fd >= 0) { /* the device doesn't exist on the emulator */
        ...
        if (!isEmulatorRunning()) { //since emulator would not send PDN urc, we do not use IPv6 handler
                ...
                if (strcmp(profile, "1002") == 0) {
                    ...
                    requestSetupDataCallOverIPv6(apn, username, passwd, request_authType, request_protocolType, 1, 1, profile, t);
                } else {
                    requestSetupDataCallOverIPv6(apn, username, passwd, request_authType, request_protocolType, 2, 2, profile, t);
                }
#else
        ...
        /* AT+CGDATA=<L2P>,<cid> */
#if defined(PURE_AP_USE_EXTERNAL_MODEM)
        asprintf(&cmd, "AT+CGDATA=\"M-MBIM\",%d", request_cid);
#else
...
void requestDeactiveDataCall(void * data, size_t datalen, RIL_Token t)
{
...
#if defined(PURE_AP_USE_EXTERNAL_MODEM)
    if (cid_value == 2) {
        LOGD("Start shutdown pppd");
        shutdownPPP(2, t);
        max_pdp_support_23G ++;
        setPropCidStatus(max_pdp_support_23G);
        LOGD("Shutdown send response");    
    ...
    //[ALPS01025656] Deactivation timing issue start
    //Skip this deactivate
    if (result < 0){
        LOGD("Has deactivated the same time, skip this time");
        goto skip;
    }
    //[ALPS01025656] Deactivation timing issue end
