...
int worldPhoneFlag = 0x00000000;
CfunParam cfunParamInQ;
...
extern int triggerCCCIIoctlEx(int request, int *param)
{
    ...
    ccci_sys_fd = open(CCCI_MD2_POWER_IOCTL_PORT, O_RDWR);
#else
...
#if defined(MTK_WORLD_PHONE) && defined(MTK_CMCC_WORLD_PHONE_TEST)
void clearWorldPhoneFlagCallback(void* param)
{
    LOGW("[WorldPhoneATCI]clearWorldPhoneFlagCallback.flag: %d", worldPhoneFlag);
    if(!IS_SET_RIL_FLAG(worldPhoneFlag, FLAG_AT_EFUN_DONE))
        return;
    CLEAR_RIL_FLAG(worldPhoneFlag, FLAG_AT_EFUN_DONE);
    if(IS_SET_RIL_FLAG(worldPhoneFlag, FLAG_AT_CFUN_IN_Q))
    {
        CLEAR_RIL_FLAG(worldPhoneFlag, FLAG_AT_CFUN_IN_Q);
        postQCfun();
    }
}
...
void postQCfun()
{
    LOGW("[WorldPhoneATCI]Post cfun in Q");
    requestOemHookRaw(cfunParamInQ.cfun, sizeof(cfunParamInQ.cfun), cfunParamInQ.token);
}
#endif//MTK_WORLD_PHONE
