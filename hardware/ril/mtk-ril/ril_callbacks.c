...
extern void initCidStatus(RILId rid);
...
static void initializeCallback(void *param)
{
	...
	// ALPS00465815, fix leakage issue.
    ...
    if (isInternationalRoamingEnabled()
        ...
        if ((!isDualTalkMode() && rid == MTK_RIL_SOCKET_2) ||
            (isDualTalkMode() && ((rid == MTK_RIL_SOCKET_2 && RIL_is3GSwitched()) || (rid == MTK_RIL_SOCKET_1 && !RIL_is3GSwitched())))
        ){
            flightModeBoot();
            if (isDualTalkMode()) {
                ...
                bootupGetBasebandVersion(MTK_RIL_SOCKET_1);
                bootupGetBasebandVersion(MTK_RIL_SOCKET_2);
                bootupGetCalData(MTK_RIL_SOCKET_2);
            }
