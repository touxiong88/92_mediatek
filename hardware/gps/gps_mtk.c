...
mt3326_gps_inject_location(double latitude, double longitude, float accuracy)
{
    ...
    nw_location->accuracy = accuracy;
    nw_location->latitude = latitude;
    nw_location->longitude = longitude;
    
    DBG("Start to inject location command");
    
    char buf[] = {MNL_CMD_INJECT_NW_LOCATION};
    if ((err = mt3326_daemon_send(buf, sizeof(buf))))
        ERR("Send location cmd fail\n");
    
    DBG("Start to inject location");
    
    if ((err = mt3326_daemon_send((char*)nw_location, sizeof(NetworkLocation))))
        ERR("Send location fail\n");    
    DBG("err = %d, nw_location len = %d", err, sizeof(NetworkLocation));	    
...
mt3326_gps_cleanup(void)
{
    ...
    //pthread_join(s->thread_epo, NULL);
