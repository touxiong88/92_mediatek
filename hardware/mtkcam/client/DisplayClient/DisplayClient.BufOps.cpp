...
#if 1
			int w = pStreamImgBuf->getImgWidth();
			int h = pStreamImgBuf->getImgHeight();

			if(needInitFace  && width ==  w && height == h)
//			if(needInitFace )
			{
				needInitFace = 0;
				int size = pStreamImgBuf->getBufSize();
				estar_face_initialize(w, h, 1,size);

				dbmsg(" exec_only_once:%d",size);
			}
			else if(!needInitFace  && width ==  w &&   height == h)
			{
		//		frames++;
		//		if(frames %2){
					int num_face = estar_face_detect((char*)pStreamImgBuf->getVirAddr());
		//		}
			//	estar_face_get_face();
			//	dbmsg("num_face:%d",num_face);

			}
			//void estar_face_destroy();
#endif
