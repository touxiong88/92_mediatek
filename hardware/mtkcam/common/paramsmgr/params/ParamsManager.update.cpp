...
updateDefaultParams0()
{
    ...
    //  3A related
    mParameters.set(CameraParameters::KEY_FOCAL_LENGTH, "3.5"); // default, may be overwrited by the one queried from 3A
...
{
    MtkCamUtils::Property::set( String8(PROPERTY_KEY_HAL_FORCEAPPMODE), mode);
    return true;
}
/******************************************************************************
*
...
evaluateHalAppModeUtility(String8& mode) const
{
    ...
    //  Force Hal App mode
    if  ( MtkCamUtils::Property::tryGet(String8(PROPERTY_KEY_HAL_FORCEAPPMODE), mode ) )
    {
        if( mode != "" )
        {
            MY_LOGD("force mode: %s", mode.string());
            //clear property
            MtkCamUtils::Property::set( String8(PROPERTY_KEY_HAL_FORCEAPPMODE), String8());
            goto lbExit;
        }
    }
