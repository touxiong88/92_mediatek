...
int textview_key_handler(int key, void *priv)
{
	...
	case UI_KEY_UP:
	case UI_KEY_VOLUP:
		LOGD("textview_key_handler: key up\n");
		LOGD("textview_key_handler: avail_lines=%d, tv->m_nr_lines=%d, tv->m_start=%d, tv->m_end=%d,", \
			avail_lines, tv->m_nr_lines, tv->m_start, tv->m_end);
		if (avail_lines < tv->m_nr_lines && tv->m_start > 0) {
			tv->m_start--;
			tv->m_end--;
			tv->m_redraw = 1;
			LOGD("textview_key_handler: key up redraw\n");
		}
		break;
	case UI_KEY_DOWN:
	case UI_KEY_VOLDOWN:
		LOGD("textview_key_handler: key down\n");
		LOGD("textview_key_handler: avail_lines=%d, tv->m_nr_lines=%d, tv->m_start=%d, tv->m_end=%d,", \
			avail_lines, tv->m_nr_lines, tv->m_start, tv->m_end);
		if (avail_lines < tv->m_nr_lines && tv->m_end < tv->m_nr_lines) {
			...
			tv->m_end++;
			tv->m_redraw = 1;
			LOGD("textview_key_handler: key down redraw\n");
		}
		break;
	default:
		handled = -1;
