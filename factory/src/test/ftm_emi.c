...
static unsigned int clk_range[] = {208, 266, 293,367};
...
static int get_dram_type(void)
{
  ...
  if  (fd < 0) {
      ...
      ret = -1; 
        ...
        {
          printf("Failed to get dram type\n");
          ret = -1;
        }
   get_dram_type_end:
   close(fd);   
...
static int emi_entry(struct ftm_param *param, void *priv)
{
    ...
    if(dram_type==LPDDR2)
    {
     ret = dram_overclock(clk_range[MHz_293], ptr);       
    }
    else if (dram_type==LPDDR3)
    {
     ret = dram_overclock(clk_range[MHz_367], ptr);
    }
    else
    {
     printf("can not get correct dram type\n");
     goto emi_entry_exit;
    }
    if (ret < 0) {
