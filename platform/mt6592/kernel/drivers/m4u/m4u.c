...
int m4u_dma_cache_maint(M4U_MODULE_ID_ENUM eModuleID, const void *va, size_t size, int direction)
{
    // By range operation
    unsigned int page_num;
    int ret = 0;
...
struct page* m4u_alloc_pages() {
    ...
    // free unused pages
    // frame buffer pa is > 0xf0000000
    start = PT_TOTAL_ENTRY_NUM * sizeof(unsigned int) / M4U_PAGE_SIZE ;
...
//    gM4U_align_page_va = (unsigned int)page_address(tmp_page);
    ...
    if(IS_ERR_OR_NULL(pM4URegBackUp))
    {
