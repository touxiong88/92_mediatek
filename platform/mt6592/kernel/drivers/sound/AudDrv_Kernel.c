...
//    len += sprintf(buf + len , "CID  = 0x%x\n", Ana_Get_Reg(CID));
    ...
    PRINTK_AUDDRV("AudDrv_Read_Procmem len = %d Max = %d\n", len,count);

    AudDrv_Clk_Off();
#if 0
    if(len > count)
    {
        PRINTK_AUDDRV("AudDrv: ASSERT [%d] > [%d]",len,count);
        aee_kernel_exception_api(__FILE__, __LINE__, DB_OPT_FTRACE, "proc_fs read buffer overflow", "audio blocked dump ftrace");
    }
#endif
    PRINTK_AUDDRV("-AudDrv_Read_Procmem \n");
    return len;
...
void Auddrv_Handle_Mem_context(AFE_MEM_CONTROL_T *Mem_Block)
{
    ...
    {
        PRINTK_AUDDRV("[Auddrv] Mem_Block ==[%x] AudDrvSuspendStatus = [%d]\n",Mem_Block,AudDrvSuspendStatus);
        return;
    }
...
void Auddrv_DL_Interrupt_Handler(void)  // irq1 ISR handler
{
    ...
    if (Afe_Block->u4DataRemained < Afe_consumed_bytes || Afe_Block->u4DataRemained <= 0 || Afe_Block->u4DataRemained  > Afe_Block->u4BufferSize || AudIrqReset)
    {
        ...
        PRINTK_AUDDRV("-DL_Handling underflow ReadIdx:0x%x WriteIdx:0x%x, DataRemained:0x%x, Afe_consumed_bytes 0x%x \n",
...
/* Vanzo:zhangxinyu on: Mon, 25 Nov 2013 15:50:18 +0800
 * TODO: control headphone en pin
  */
#ifdef GPIO_HEADPHONE_EN_PIN
			mt_set_gpio_out(GPIO_HEADPHONE_EN_PIN,1);
#endif
 ...
 * TODO: control headphone en pin
  */
#ifdef GPIO_HEADPHONE_EN_PIN
			mt_set_gpio_out(GPIO_HEADPHONE_EN_PIN,0);
#endif
// End of Vanzo:zhangxinyu
...
static ssize_t AudDrv_write(struct file *fp, const char __user *data, size_t count, loff_t *offset)
{
    ...
    while (count)
    {
        ...
        {
            //PRINTK_DEBUG_MODE("AudDrv_write copy_size:0x%x (freespace=%d)\n", count, copy_size);  // (free  space of buffer)
            copy_size = count;
        }
