...
#define CCCI_IOC_GET_MD_IMG_EXIST			_IOR(CCCI_IOC_MAGIC, 30, unsigned int[4])
#define CCCI_IOC_GET_MD_TYPE				_IOR(CCCI_IOC_MAGIC, 31, unsigned int)
	...
	else if(pLocalBuf->type == FT_MODEM_OP_SET_MODEMTYPE)
	{
		unsigned int modem_type = pLocalBuf->cmd.set_modem_type_req.modem_type;

		int fd = open("/dev/ccci_ioctl4", O_RDWR|O_NOCTTY|O_NDELAY );
		if(fd < 0)
		{
			ft_cnf.status = META_FAILED;
			FT_LOG("[FTT_Drv:]Open /dev/ccci_ioclt4 fail %d",fd );		
		}
		else
		{		
