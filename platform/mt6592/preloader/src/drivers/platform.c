...
int usb_cable_in(void)
{
    ...
    if ((g_boot_reason == BR_USB) || usb_accessory_in()) {
        ...
        if (ret == STANDARD_HOST || ret == CHARGING_HOST) {
            ...
            print("\n%s USB cable in - NONSTANDARD CHARGE or STANDARD CHARGER\n", MOD);
            #if CFG_USBIF_COMPLIANCE
            platform_set_chrg_cur(450);
            #endif
...
//************************************************
	...
	mt_set_gpio_mode(GPIO19,0);
	mt_set_gpio_pull_enable(GPIO19,1);
	mt_set_gpio_pull_select(GPIO19,1);
	mt_set_gpio_dir(GPIO19,1);
	mt_set_gpio_out(GPIO19,0);

	fan5405_hw_init();
    mdelay(500);
	mt_set_gpio_out(GPIO19,1);
#endif
}
