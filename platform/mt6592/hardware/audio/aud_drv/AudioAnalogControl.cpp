...
#ifdef SWITCH_BEFORE_HPAMP //HP switch
    mFd = 0;
...
#if 0 //HP switch
uint32_t AudioAnalogControl::GetHeadPhoneGainReg(AudioAnalogType::VOLUME_TYPE VoleumType)
{
    ALOGD("GetHeadPhoneGainReg VoleumType = %d", VoleumType);
    return mAudioMachineDevice->GetHeadPhoneGainReg(VoleumType);
}
#endif
...
/* Vanzo:maxiaojun on: Thu, 14 Nov 2013 10:52:04 +0800
 */
#ifdef FM36
    if(AudioAnalogType::DEVICE_IN_DIGITAL_MIC == DeviceType){
        system("echo 9 > /sys/bus/platform/drivers/fm36_driver/fm36_state");
        usleep(10 * 1000);
    }
#endif
...
#ifdef SWITCH_BEFORE_HPAMP //HP switch
    if (DeviceType == AudioAnalogType::DEVICE_OUT_HEADSETR || DeviceType == AudioAnalogType::DEVICE_OUT_HEADSETL)
    {
        ALOGD("AnalogOpen open external HPAmp \n");
...
status_t AudioAnalogControl::AnalogClose(AudioAnalogType::DEVICE_TYPE DeviceType, AudioAnalogType::DEVICE_TYPE_SETTING Type_setting)
{
    ...
    if (DeviceType == AudioAnalogType::DEVICE_OUT_HEADSETR || DeviceType == AudioAnalogType::DEVICE_OUT_HEADSETL)
    {
        ALOGD("AnalogClose close external HPAmp \n");
        ::ioctl(mFd, SET_EXTHEADPHONE_AMP_OFF, NULL); // GPIO turn OFF TPA6141 in kernel (similar to cust_matv_gpio_on()), TPA6141 gain is fixed at 0dB or +6dB?
    }
#endif
    mAudioMachineDevice->AnalogClose(DeviceType);
    mAudioPlatformDevice->AnalogClose(DeviceType);
/* Vanzo:maxiaojun on: Thu, 14 Nov 2013 10:52:27 +0800
 */
#ifdef FM36
    if(AudioAnalogType::DEVICE_IN_DIGITAL_MIC == DeviceType){
        system("echo 0 > /sys/bus/platform/drivers/fm36_driver/fm36_state");
    }
#endif
// End of Vanzo:maxiaojun
    return NO_ERROR;
