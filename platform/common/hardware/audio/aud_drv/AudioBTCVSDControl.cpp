...
void AudioBTCVSDControl::btsco_process_RX_MSBC(void *inbuf, uint32_t *insize, void *outbuf, uint32_t *outsize, void *workbuf, const uint32_t workbufsize, uint8_t packetvalid)
{
    ...
    if ((mBTSCOCVSDContext->pRX->PacketBuf[index1][0] == 0x1) && ((mBTSCOCVSDContext->pRX->PacketBuf[index1][1] & 0xF) == 0x8))
    {
        ...
        status = MSBC_DEC_Process(mBTSCOCVSDContext->pRX->pDecHandle, (char *)mBTSCOCVSDContext->pRX->EntirePacket, &iInByteTemp, (int16_t *)outbuf, &iOutSample);
        ALOGD("btsco_process_RX_MSBC() MSBC_DEC_Process(-) status=%d", status);


#ifdef DUMP_CVSDRX_OUTPUT
        memcpy((void *)RXout_dumpbuf + Rxcnt * MSBC_PCM_FRAME_BYTE, (void *)outbuf, iOutSample << 1);
        if (Rxcnt == 1)
        {
            if (mBTCVSDRXDumpFile)
            {
    ...
    if (mBTSCOCVSDContext->pRX->fEnablePLC)
    {
        ...
        else
        {
            ...
            g711plc_dofe_v2(mBTSCOCVSDContext->pRX->pPLCHandle, (short *)outbuf, dwBtEv3HalfBad);
        }
    }
    ...
    if (mBTSCOCVSDContext->pRX->fEnableFilter)
    {
        ...
        Audio_IIRHPF_Process(mBTSCOCVSDContext->pRX->pHPFHandle, (uint16_t *)outbuf, (uint16_t *)&iInSampleTemp, (uint16_t *)outbuf, (uint16_t *)&iOutSampleTemp);
...
{
    ALOGVV("BT_SCO_ExtMDULBufLock (1)");
    mLockUL.lock();
    ALOGVV("BT_SCO_ExtMDULBufLock (2)");
    return true;
}

bool AudioBTCVSDControl::BT_SCO_ExtMDULBufUnLock(void)
{
    ALOGVV("BT_SCO_ExtMDULBufUnLock (1)");
    mLockUL.unlock();
    ALOGVV("BT_SCO_ExtMDULBufUnLock (2)");
    return true;
}

bool AudioBTCVSDControl::BT_SCO_ExtMDDLBufLock(void)
{
    ALOGVV("BT_SCO_ExtMDDLBufLock (1)");
    mLockDL.lock();
    ALOGVV("BT_SCO_ExtMDDLBufLock (2)");
    return true;
}

bool AudioBTCVSDControl::BT_SCO_ExtMDDLBufUnLock(void)
{
    ALOGVV("BT_SCO_ExtMDDLBufUnLock (1)");
    mLockDL.unlock();
    ALOGVV("BT_SCO_ExtMDDLBufUnLock (2)");
    return true;
}
...
#define DL1_BUFFER_SIZE (0x4000)
        ...
        case ExtMD_BTSCO_DL_READTHREAD:
            ...
            {
                ALOGD("threadLoop exit STANDBY_MEMIF_TYPE mThreadType = %d", mThreadType);
                ::ioctl(mFd, STANDBY_MEMIF_TYPE, mThreadType);
            }
...
void AudioBTCVSDControl::AudioExtMDCVSDThread::ExtMD_btsco_cvsd_UL_Read_main(void)
{
    ...
    uint32_t i, Read_Size, outsize, workbufsize, insize, bytes, offset, accuoutsize;

    ALOGV("ExtMD_btsco_cvsd_UL_Read_main(+)");
    Read_Size = ::read(mFd2, mAudioBTCVSDControl->BT_SCO_RX_GetCVSDTempInBuf(), BTSCO_CVSD_RX_TEMPINPUTBUF_SIZE);
    ALOGV("ExtMD_btsco_cvsd_UL_Read_main ::read() done Read_Size=%d", Read_Size);
...
{
    uint32_t DataCount, trycount = 0, i;
    uint8_t *wtmpbuf, *wtmpbuf2;
    uint16_t *pSrc, *pDst;

    ALOGVV("ExtMD_btsco_cvsd_UL_Write_main(+)");

    wtmpbuf = mAudioBTCVSDControl->BT_SCO_ExtMDGetCVSDULWriteTmpBuf();
    ...
    do
    {
        ...
        if (DataCount >= BTSCO_CVSD_RX_INBUF_SIZE * 2)
        {
            mAudioBTCVSDControl->BT_SCO_ExtMDReadDataFromRingBuf(wtmpbuf, BTSCO_CVSD_RX_INBUF_SIZE * 2, ExtMD_BTSCO_UL); //use temp buf to avoid blocking Lock
            mAudioBTCVSDControl->BT_SCO_ExtMDULBufUnLock();
            break;
        }
        else
        {
            ALOGVV("ExtMD_btsco_cvsd_UL_Write_main DataCount=%d < %d,", DataCount, BTSCO_CVSD_RX_INBUF_SIZE * 2);
            mAudioBTCVSDControl->BT_SCO_ExtMDULBufUnLock();
            usleep(10 * 1000);
        }
        trycount++;
    ...
    ALOGVV("ExtMD_btsco_cvsd_UL_Write_main pSrc=0x%x, pDst=0x%x, size=%d", pSrc, pDst, BTSCO_CVSD_RX_INBUF_SIZE);

    for (i = 0; i < (BTSCO_CVSD_RX_INBUF_SIZE); i++)
    {
        *pDst = *pSrc;
        pDst++;
        *pDst = *pSrc;
    ...
    WritePcmDumpData(wtmpbuf2, BTSCO_CVSD_RX_INBUF_SIZE * 2 * 2);

    ALOGD("ExtMD_btsco_cvsd_UL_Write_main ::write to kernel (+),size=%d", BTSCO_CVSD_RX_INBUF_SIZE * 2 * 2);
    ::write(mFd, wtmpbuf2, BTSCO_CVSD_RX_INBUF_SIZE * 2 * 2); //BTSCO_CVSD_RX_INBUF_SIZE*2
    ...
    if (mAFEULStarting == false) //mAudioBTCVSDControl->BT_SCO_RX_ExtMDInitAFEDLHW();
    {
        ...
        ::write(mFd, wtmpbuf2, BTSCO_CVSD_RX_INBUF_SIZE * 2 * 2);
        mAFEULStarting = true;
...
void AudioBTCVSDControl::AudioExtMDCVSDThread::ExtMD_btsco_cvsd_DL_Read_main(void)
{
    ...
    uint32_t Read_Size, FreeSpace;
    uint8_t trycount = 0;

    ALOGD("ExtMD_btsco_cvsd_DL_Read_main(+)");
    ...
    if (mAFEDLStarting == false)
    {
        ...
        if (IrqStatus.mStatus == false)
        {
            ...
            mAudioDigitalControl->SetIrqMcuCounter(AudioDigitalType::IRQ2_MCU_MODE, sizeof(rtmpbuf) >> 1); // 480 samples, 60ms
    ...
    Read_Size = ::read(mFd, rtmpbuf, sizeof(rtmpbuf));
    ALOGV("ExtMD_btsco_cvsd_DL_Read_main ::read() done Read_Size=%d", Read_Size);
    //ASSERT(Read_Size==sizeof(rtmpbuf)); //ccc remove temporarily
    ...
    do
    {
        ...
        if (FreeSpace >= sizeof(rtmpbuf))
        {
            mAudioBTCVSDControl->BT_SCO_ExtMDWriteDataToRingBuf(rtmpbuf, sizeof(rtmpbuf), ExtMD_BTSCO_DL);
            mAudioBTCVSDControl->BT_SCO_ExtMDDLBufUnLock();
            WritePcmDumpData(rtmpbuf, sizeof(rtmpbuf));
            break;
        }
        else
        {
            ALOGVV("ExtMD_btsco_cvsd_DL_Read_main FreeSpace=%d < %d,", FreeSpace, BTSCO_CVSD_TX_OUTBUF_SIZE * 2);
            mAudioBTCVSDControl->BT_SCO_ExtMDDLBufUnLock();
            usleep(10 * 1000);
        }
        trycount++;
...
void AudioBTCVSDControl::AudioExtMDCVSDThread::ExtMD_btsco_cvsd_DL_Write_main(void)
{
    ...
    ALOGVV("ExtMD_btsco_cvsd_DL_Write_main(+)");
    ...
    do
    {
        ...
        if (DataCount >= sizeof(wtmpbuf))
        {
            mAudioBTCVSDControl->BT_SCO_ExtMDReadDataFromRingBuf(wtmpbuf, sizeof(wtmpbuf), ExtMD_BTSCO_DL); //use temp buf to avoid blocking Lock
            mAudioBTCVSDControl->BT_SCO_ExtMDDLBufUnLock();
            break;
        }
        else
        {
            ALOGVV("ExtMD_btsco_cvsd_DL_Write_main DataCount=%d < %d,", DataCount, sizeof(wtmpbuf));
            mAudioBTCVSDControl->BT_SCO_ExtMDDLBufUnLock();
            usleep(10 * 1000);
        }
        trycount++;
    ...
    do
    {
        ...
        WritePcmDumpData(mAudioBTCVSDControl->BT_SCO_TX_GetCVSDOutBuf(), total_outsize);

        ALOGV("ExtMD_btsco_cvsd_DL_Write_main write to kernel(+) total_outsize=%d", total_outsize);
        ...
        ALOGV("ExtMD_btsco_cvsd_DL_Write_main write to kernel(-) remaining bytes=%d", bytes);
    }
    while (bytes > 0);
...
#elif defined(BTCVSD_KERNEL_LOOPBACK) // create MTKRecordthread for test BTCVSD RX
    ALOGD("****************BTCVSD TEST create AudioBTCVSDLoopbackRxThread************** \n");
    mBTCVSDRxTestThread = new AudioBTCVSDLoopbackRxThread(this, AudioDigitalType::MEM_DAI , NULL, 0);
    if (mBTCVSDRxTestThread.get())
    {
...
#elif defined(BTCVSD_LOOPBACK_WITH_CODEC) // close AudioBTCVSDControl::AudioBTCVSDLoopbackRxThread
    int ret = 0;
    if (mBTCVSDRxTestThread.get())
    {
        //ret = mBTCVSDRxTestThread->requestExitAndWait();
...
//#if defined(BTCVSD_KERNEL_LOOPBACK) || defined(BTCVSD_LOOPBACK_WITH_CODEC) //0902
...
#if !defined(BTCVSD_TEST_HW_ONLY) && defined(DUMP_CVSDRX_OUTPUT)
static uint8_t outbuf_dump[BTSCO_CVSD_RX_FRAME *SCO_RX_PCM8K_BUF_SIZE]; //temp buf only for dump to file
#endif
...
#if 0 //0902
#if !defined(BTCVSD_TEST_HW_ONLY) && defined(DUMP_CVSDRX_OUTPUT)
        memcpy((void *)outbuf_dump + dump_size, (void *)outbuf, outsize); // copy to outbuf_dump to reduce file IO
        dump_size += outsize;
#endif
#else
...
#if 0 //0902
#if !defined(BTCVSD_TEST_HW_ONLY) && defined(DUMP_CVSDRX_OUTPUT)
    WritePcmDumpData(outbuf_dump, dump_size);
#endif
#endif
