...
#ifdef MTK_AUDIO
    ||(outputDesc->isUsedByStrategy(STRATEGY_ENFORCED_AUDIBLE) && mPhoneState ==AudioSystem::MODE_RINGTONE) //ALPS01092399
#endif
    ) {
...
#ifdef MTK_AUDIO
    }else if (outputDesc->isUsedByStrategy(STRATEGY_PROPRIETARY_FM)) {
        device = getDeviceForStrategy(STRATEGY_PROPRIETARY_FM, fromCache);
    }else if (outputDesc->isUsedByStrategy(STRATEGY_PROPRIETARY_MATV)) {
                                                       ...
                                                       uint32_t delayMs)
{
    ...
    for (size_t i = 0; i < NUM_STRATEGIES; i++) {
        ...
        bool bFM2MusicStarTempMute = ((outputDesc->strategyRefCount((routing_strategy)STRATEGY_PROPRIETARY_FM)||
                                       outputDesc->strategyRefCount((routing_strategy)STRATEGY_PROPRIETARY_MATV))
                                     &&outputDesc->mRefCount[AudioSystem::MUSIC]&&(i==STRATEGY_MEDIA)&&bFMPreStopSignal);
        if(i==STRATEGY_MEDIA)
            bFMPreStopSignal = false;//Reset
#endif
...
||bFM2MusicStarTempMute
                        ...
                        if((STRATEGY_PROPRIETARY_FM == i||STRATEGY_PROPRIETARY_MATV == i)||(STRATEGY_MEDIA == i && bFM2MusicStarTempMute))//[ALPS00414767] Add STRATEGY_MEDIA case
                        {
                            ...
                            if(desc->mRefCount[AudioSystem::VOICE_CALL])
                               setStrategyMute((routing_strategy)i, false, curOutput,desc->latency() * 60, device);
                            else
...
#ifdef MTK_AUDIO
    if ((samplingRate != 0)&&(device&AUDIO_DEVICE_BIT_IN)==0) {
#else
    if (samplingRate != 0) {
#endif
        size_t i;
...
void AudioMTKPolicyManager::defaultAudioPolicyConfig(void)
{
    ...
    profile->mFormats.add(AUDIO_FORMAT_PCM_32_BIT);
#endif
    ...
    inputprofile->mSupportedDevices = (AUDIO_DEVICE_IN_COMMUNICATION | AUDIO_DEVICE_IN_AMBIENT |
                                                        ...
                                                        AUDIO_DEVICE_IN_AUX_DIGITAL | AUDIO_DEVICE_IN_VOICE_CALL | AUDIO_DEVICE_IN_BACK_MIC | AUDIO_DEVICE_IN_FM | AUDIO_DEVICE_IN_MATV);
