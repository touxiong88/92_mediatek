...
static void WriteAttributeTable(const llvm_2_9::ValueEnumerator &VE,
  ...
  for (unsigned i = 0, e = Attrs.size(); i != e; ++i) {
    ...
    for (unsigned i = 0, e = A.getNumSlots(); i != e; ++i) {
      ...
      // 11 bits.
      uint64_t FauxAttr = A.Raw(Index) & 0xffff;
      if (Attrs.hasAttribute(Index, Attribute::Alignment))
        FauxAttr |= (Attrs.getParamAlignment(Index) << 16);
      FauxAttr |= (Attrs.Raw(Index) & (0xFFFull << 21)) << 11;
  ...
  case Instruction::Br:
    {
      ...
      const BranchInst &II = cast<const BranchInst>(I);
