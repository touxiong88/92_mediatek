...
static void atv_openVideo(JNIEnv *env, jobject thiz,jboolean connect)
{
	...
	if (connect) {
		...
		#endif
	    camera = Camera::connect(0xFF);// 0xFF is the id for atv sensor.
		#ifdef ATV_LOG_OUT
	    ...
	    if (camera == 0) return;
        #ifdef ATV_LOG_OUT
    	XLOGD("camera->setPreviewDisplay in");
		#endif
		if (camera->setPreviewDisplay((sp<Surface>)NULL) != NO_ERROR) {
			jniThrowException(env, "java/io/IOException", "setPreviewDisplay failed");
		}
        #ifdef ATV_LOG_OUT
    	XLOGD("camera->setPreviewDisplay out");
		#endif
	}
...
static void atv_setPreviewDisplay(JNIEnv *env, jobject thiz, jobject jSurface)
{
	...
	#ifdef ATV_LOG_OUT
    XLOGD("camera->setPreviewDisplay(surface) in");
	#endif
    if (camera->setPreviewDisplay(surface) != NO_ERROR) {
        jniThrowException(env, "java/io/IOException", "setPreviewDisplay failed");
    }
	#ifdef ATV_LOG_OUT
    XLOGD("camera->setPreviewDisplay(surface) out");
    XLOGD("setPreviewDisplay out");
	#endif
}
