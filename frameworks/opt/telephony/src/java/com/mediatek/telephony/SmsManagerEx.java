...
public class SmsManagerEx implements ISmsManagerExt {
    ...
    public void sendTextMessage(String destinationAddress, String scAddress,
        ...
        if (FeatureOption.MTK_ONLY_OWNER_SIM_SUPPORT) {
            Log.d(TAG, "sendTextMessage UserId is " + UserHandle.myUserId());
    ...
    public void sendDataMessage(String destinationAddress, String scAddress,
        ...
        if (FeatureOption.MTK_ONLY_OWNER_SIM_SUPPORT) {
            Log.d(TAG, "sendDataMessage UserId is " + UserHandle.myUserId());
            if (UserHandle.myUserId() != UserHandle.USER_OWNER) {
                try {
                    if (sentIntent != null) {
                        sentIntent.send(RESULT_ERROR_GENERIC_FAILURE);
    ...
    public boolean deleteMessageFromIcc(int messageIndex, int slotId) {
        ...
        if (FeatureOption.MTK_ONLY_OWNER_SIM_SUPPORT) {
            Log.d(TAG, "deleteMessageFromIcc UserId is " + UserHandle.myUserId());
            if (UserHandle.myUserId() != UserHandle.USER_OWNER) {                
                return false;
            }
        }
    ...
    public boolean updateMessageOnIcc(int messageIndex, int newStatus,
        ...
        if (FeatureOption.MTK_ONLY_OWNER_SIM_SUPPORT) {
            Log.d(TAG, "updateMessageOnIcc UserId is " + UserHandle.myUserId());
            if (UserHandle.myUserId() != UserHandle.USER_OWNER) {                
                return false;
            }
        }
    ...
    private static SimSmsInsertStatus insertRawMessageToIccCard(int status, byte[] pdu,
        ...
        if (FeatureOption.MTK_ONLY_OWNER_SIM_SUPPORT) {
            Log.d(TAG, "insertRawMessageToIccCard UserId is " + UserHandle.myUserId());
            if (UserHandle.myUserId() != UserHandle.USER_OWNER) {
                return null;
            }
        } 
    ...
    public void sendTextMessageWithExtraParams(String destAddr, String scAddr,
        ...
        if (FeatureOption.MTK_ONLY_OWNER_SIM_SUPPORT) {
            Log.d(TAG, "sendTextMessageWithExtraParams UserId is " + UserHandle.myUserId());
            if (UserHandle.myUserId() != UserHandle.USER_OWNER) {
                try {
                    if (sentIntent != null) {
                        sentIntent.send(RESULT_ERROR_GENERIC_FAILURE);
    ...
    public void sendMultipartTextMessageWithExtraParams(String destAddr,
        ...
        if (FeatureOption.MTK_ONLY_OWNER_SIM_SUPPORT) {
            Log.d(TAG, "sendMultipartTextMessageWithExtraParams UserId is " + UserHandle.myUserId());
            if (UserHandle.myUserId() != UserHandle.USER_OWNER) {
                if (sentIntents != null) {
                    for (PendingIntent sentIntent : sentIntents) {
                        try {
    ...
    private static SmsParameters getSmsParameters(int slotId) {
        ...
        if (FeatureOption.MTK_ONLY_OWNER_SIM_SUPPORT) {
            Log.d(TAG, "getSmsParameters UserId is " + UserHandle.myUserId());
            if (UserHandle.myUserId() != UserHandle.USER_OWNER) {
                return null; 
            }
        }
    ...
    public int copySmsPduToIcc(byte[] smsc, byte[] pdu, int status, int slotId) {
        if (FeatureOption.MTK_ONLY_OWNER_SIM_SUPPORT) {
            Log.d(TAG, "copyMessageToIcc UserId is " + UserHandle.myUserId());
            if (UserHandle.myUserId() != UserHandle.USER_OWNER) {                
                return -1;
            }
        }
     ...
     * @hide
     */
    private static GeminiSmsMessage createFromEfRecordByMode(int index, byte[] data, int simId, int mode) {
    ...
    private static ArrayList<SmsMessage> createMessageListFromRawRecordsByMode(List<SmsRawData> records,
            int slotId, int mode) {
        // impl
        Log.d(TAG, "call createMessageListFromRawRecordsByMode");
        ArrayList<SmsMessage> geminiMessages = null;
        if (records != null) {
            int count = records.size();
            geminiMessages = new ArrayList<SmsMessage>();
    ...
    public ArrayList<SmsMessage> getAllMessagesFromIcc(int slotId, int mode) {
        // impl
        Log.d(TAG, "call getAllMessagesFromIcc, mode="+mode);
        String isms = getSmsServiceName(slotId);
        List<SmsRawData> records = null;
        ...
        try {
            ISms iccISms = ISms.Stub.asInterface(ServiceManager.getService(isms));
            if (iccISms != null) {
                records = iccISms.getAllMessagesFromIccEfByMode(mode);
            }
        } catch (RemoteException ex) {
            Log.d(TAG, "getAllMessagesFromIcc, RemoteException!");
        }
