...
bool GLHelper::createSurfaceTexture(uint32_t w, uint32_t h,
        sp<SurfaceTexture>* glConsumer, EGLSurface* surface,
        GLuint* name) {
...
bool GLHelper::createNamedSurfaceTexture(GLuint name, uint32_t w, uint32_t h,
    ...
    sp<SurfaceTexture> glc = new SurfaceTexture(name, true,
            GL_TEXTURE_EXTERNAL_OES, false, bq);
    glc->setDefaultBufferSize(w, h);
    glc->setDefaultMaxBufferCount(3);
    glc->setConsumerUsageBits(GRALLOC_USAGE_HW_COMPOSER);

    sp<ANativeWindow> anw = new SurfaceTextureClient(static_cast<sp<ISurfaceTexture> >(bq));
