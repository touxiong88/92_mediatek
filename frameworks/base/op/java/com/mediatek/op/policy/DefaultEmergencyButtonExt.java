...
public class DefaultEmergencyButtonExt implements IEmergencyButtonExt {
    ...
    public boolean isSimInService(boolean[] isServiceSupportEcc, int slotId) {
        ...
        for (int i = PhoneConstants.GEMINI_SIM_1; i < simSlotCount; i++) {
            if (DEBUG) Xlog.d(TAG, "isSimInService i = " + i
    ...
    @Override
    public void addSlotIdForEmergencyDialer(Intent intent, int slotId) {
    }

    @Override
    public boolean enableEccOnSlide() {
        if (DEBUG) Xlog.d(TAG, "needShowEccOnSlide return false");
        return false;
    }
}
