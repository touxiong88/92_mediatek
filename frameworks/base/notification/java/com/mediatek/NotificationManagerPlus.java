...
public class NotificationManagerPlus {
    ...
    private NotificationManagerPlus(Parameters p) {
        ...
        // [ALPS00436165] we should never unregister broadcast receiver. Otherwise, we may not be able to cancel notification dialog.
     ...
     * @internal
     *
     * Destroy the instance of NotificationManagerPlus. Note that it's important to destory
     * this object if you don't need it anymore. Otherwise, there will be resource leak.
     * It's suggested to do this in app's onDestroy() callback.
     */
    public void destroy() {
        if(LOG){
            Log.i(TAG, "destroy");
        }
        // Unregister receiver and remove all dialogs
        mHandler.removeMessages(MSG_SHOW);
        mParams.mContext.unregisterReceiver(mReceiver);
        clearAll();

        // Reset mSend to prevent race condition
        mSend = false;
    }
    ...
    public static class ManagerBuilder {
         ...
         * @internal
         *
         * Get the instance of NotificationManagerPlus.
         */
