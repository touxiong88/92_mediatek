...
public class InternationalRoamingController extends Handler implements
    ...
    private static final int EVENT_SIM_START_SWITCH = 100;
    ...
    public InternationalRoamingController(Context context, Object dualModePhone, Object gsmPhone,
        ...
        mDualModePhoneCM = ((PhoneBase) (mDualModePhone.getActivePhone())).mCM;
    ...
    public int switchPhone(int mode, boolean fromUser) {
        ...
        if (mUnderSimSwitching || !mGsmActivePhone.mCM.getRadioState().isAvailable()) {
            logd("switchPhone error, mode = " + mode + ",mUnderSimSwitching = "
                    + mUnderSimSwitching);
        ...
        switch (msg.what) {
            ...
            case EVENT_SERVICE_STATE_CHANGED:
                ...
                if (regState == ServiceState.REGISTRATION_STATE_HOME_NETWORK
                    ...
                    // Registered, save network information and remove no service message.
    ...
    private void startToSwitchPhone() {
        ...
        mGsmActivePhone.mCM
                .setRadioPowerOff(mSimSwithHandler.obtainMessage(EVENT_SIM_START_SWITCH));
    }
    ...
    private void loadDualPhoneImsiCompleted(int dualSimId, String cdmaImsi, String gsmImsi) {
        if (mImsiLoaded) {
            logd("IMSI has already loaded: mCardType = " + mCardType + ", mCdmaImsi = " + mCdmaImsi + ", mGsmImsi = "
                    + mGsmImsi);
            return;
        }
    ...
    private class SimSwithHandler extends Handler {
        ...
        private static final int EVENT_SIM_SWITCH_DONE = 101;
        private static final int EVENT_PHONE_RADIO_OFF = 102;
        private static final int EVENT_SWITCH_RIL_SOCKET_INTERFACE = 103;
        private static final int EVENT_SWITCH_SIM_INTERFACE = 104;
        private static final int EVENT_SWITCH_PHONE = 105;
        ...
        private void setCurrentSimMode(Context context, int mode) {
            logd("[SSH] setCurrentSimMode = "
                    + (mode == InternationalRoamingConstants.SIM_SWITCH_MODE_GSM ? "GSM" : "CDMA"));
            SystemProperties.set(InternationalRoamingConstants.PROPERTY_EVDO_MODE,
                    String.valueOf(mode));
            Settings.System.putInt(context.getContentResolver(), CURRENT_SIM_MODE, mode);
        }
         ...
         * CDMA->GSM: 
         * 1. CDMA off 
         * 2. Switch socket,GSM off 
         * 3. Switch SIM interface
         * 4. GSM on 
         * 
         * GSM-> CDMA: 
         * 1. GSM off 
         * 2. Switch SIM interface 
         * 3. Switch socket 
         * 4. CDMA on
         */
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case EVENT_SIM_START_SWITCH:
                    if (mDualActivePhone.getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
                        logd("[SSH] CDMA -> GSM, Set CDMA radio off.");
                        mDualActivePhone.mCM.setRadioPowerCardSwitch(0,
                                obtainMessage(EVENT_SWITCH_RIL_SOCKET_INTERFACE));
                        mChangeToGemini = true;
                        setCurrentSimMode(mContext,
                                InternationalRoamingConstants.SIM_SWITCH_MODE_GSM);
                    } else {
                        logd("[SSH] GSM -> CDMA, EVENT_SIM_START_SWITCH");
                        setCurrentSimMode(mContext,
                                InternationalRoamingConstants.SIM_SWITCH_MODE_CDMA);
                        mChangeToGemini = false;
                        obtainMessage(EVENT_SWITCH_SIM_INTERFACE, null).sendToTarget();
                    }
                    break;

                case EVENT_SWITCH_RIL_SOCKET_INTERFACE:
                    logd("[SSH] EVENT_SWITCH_RIL_SOCKET_INTERFACE: mChangeToGemini = "
                            + mChangeToGemini);
                    if (mChangeToGemini) {
                        mDualActivePhone.mCM.registerForRadioStateChanged(this,
                                EVENT_SWITCH_SIM_INTERFACE, null);
                        mDualActivePhone.mCM.switchRilSocket(Phone.NT_MODE_WCDMA_PREF, mDualSimId);
                    } else {
                        mDualActivePhone.mCM.registerForRadioStateChanged(this, EVENT_SWITCH_PHONE,
                                null);
                        mDualActivePhone.mCM.switchRilSocket(Phone.NT_MODE_CDMA, mDualSimId);
                    }
                    break;

                case EVENT_SWITCH_SIM_INTERFACE:
                    if (mChangeToGemini) {
                        // Remove extra EVENT_SWITCH_SIM_INTERFACE messages.
                        removeMessages(EVENT_SWITCH_SIM_INTERFACE);
                        if (!mDualActivePhone.mCM.getSimState().isAvailable()) {
                            logd("[SSH] Dual phone SIM is not available when handing EVENT_SWITCH_SIM_INTERFACE.");
                            break;
                        }

                        logd("[SSH] EVENT_SWITCH_SIM_INTERFACE: CDMA -> GSM.");
                        mDualActivePhone.mCM.unregisterForRadioStateChanged(this);
                        mDualActivePhone.mCM.setSimInterfaceSwitch(
                                InternationalRoamingConstants.SIM_SWITCH_MODE_GSM,
                                obtainMessage(EVENT_SWITCH_PHONE));
                    } else {
                ...
                case EVENT_SWITCH_PHONE:
                    ...
                    mDualModePhone.unregisterForServiceStateChanged(InternationalRoamingController.this);
                    ...
                    logd("[SSH] EVENT_SWITCH_PHONE: newTech = " + newTech);

                    // Set modem reset mode.
                    SystemProperties.set("mux.report.case", String.valueOf(1));

                    // The next radio power on message is triggered by SIM switch.
                    mRadioOnFromSimSwitch = true;
                    
                    if (mChangeToGemini) {
                        finishToSwitchPhone();

                        logd("[SSH] Power on GSM modem after phone instance updated.");
                        mGsmActivePhone.mCM.setRadioPowerOn(null);
                    } else {
                        if (!mDualActivePhone.mCM.getSimState().isAvailable()) {
                            logd("[SSH] Dual phone SIM is not available when handing EVENT_POWERON_PHONES.");
                            break;
                        }
                        finishToSwitchPhone();

                        logd("[SSH] Power on GSM modem and CDMA modem after phone instance updated.");
                        // Remove extra EVENT_POWERON_PHONES messages.
                        removeMessages(EVENT_SWITCH_PHONE);
                        mDualActivePhone.mCM.unregisterForRadioStateChanged(this);
                        mDualActivePhone.mCM.setRadioPowerCardSwitch(1, null);
                        mGsmActivePhone.mCM.setRadioPowerOn(null);
                    }
                    break;
